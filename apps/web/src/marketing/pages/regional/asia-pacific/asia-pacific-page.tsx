...
import { asiaPacificEditorial } from "./asia-pacific-page-editorial"

interface SectionProps {
  readonly reduceMotion: boolean
}

function AsiaPacificIdentitySection({ reduceMotion }: SectionProps) {
  const { opening } = asiaPacificEditorial

  return (
    <MarketingPageSection
      className="relative overflow-hidden border-b border-border/70"
      containerClassName="pb-16 pt-28 md:pb-20 lg:pb-24 lg:pt-32"
      aria-labelledby="asia-pacific-hero-heading"
    >
      <MarketingStructuralGridBg sizePx={40} className="opacity-[0.05]" />
      <MarketingStructuralBaseline />
...
function AsiaPacificPostureSection({ reduceMotion }: SectionProps) {
  const { regionalPosture } = asiaPacificEditorial

  return (
    <MarketingPageSection
      className="relative border-b border-border/70"
      aria-labelledby="asia-pacific-posture-heading"
    >
      <MarketingStructuralVerticalRulesBg
        gapPx={108}
      ...
      <div className="relative z-[1]">
        <div className="grid gap-14 lg:grid-cols-[minmax(0,0.94fr)_minmax(0,1.06fr)] lg:items-start lg:gap-16">
          <motion.div {...getMarketingReveal(reduceMotion)}>
            <MarketingSectionHeading
              as="h2"
              id="asia-pacific-posture-heading"
              kicker="Regional posture"
              title={regionalPosture.title}
            ...
            <div className="mt-8 max-w-3xl space-y-6 text-base leading-8 text-pretty text-muted-foreground">
              {regionalPosture.body.map((paragraph) => (
          ...
          <motion.div {...getMarketingReveal(reduceMotion, 0.06)}>
            {regionalPosture.matrix.map((item, rowIndex) => (
              <div
                key={item.title}
...
function AsiaPacificRolloutSection({ reduceMotion }: SectionProps) {
  const { rolloutChain } = asiaPacificEditorial
  const chainLabelId = "asia-pacific-rollout-chain"

  return (
    <MarketingPageSection
      className="relative overflow-hidden border-b border-border/70"
      aria-labelledby="asia-pacific-rollout-heading"
    >
      ...
      <div className="relative z-[1]">
        <motion.div {...getMarketingReveal(reduceMotion)}>
          <MarketingSectionHeading
            as="h2"
            id="asia-pacific-rollout-heading"
            kicker="Rollout chain"
            title={rolloutChain.title}
            description={rolloutChain.intro}
        ...
        >
          ...
          >
            <ol className="flex min-w-[min(100%,72rem)] flex-col divide-y divide-border/70 md:min-w-0 md:flex-row md:divide-x md:divide-y-0">
              {rolloutChain.chain.map((step, index) => (
                <li
                  key={step.label}
        ...
        <motion.aside
          className="mt-12 max-w-4xl border-l-2 border-primary/40 pl-6"
...
function AsiaPacificClosingSection({ reduceMotion }: SectionProps) {
  const { cta } = asiaPacificEditorial

  return (
    <MarketingPageSection aria-labelledby="asia-pacific-closing-heading">
      <motion.div
      ...
      >
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,20rem)] lg:items-start lg:gap-14">
          <div>
            <MarketingSectionHeading
              as="h2"
              id="asia-pacific-closing-heading"
              kicker="Next"
              title={cta.title}
...
export default function AsiaPacificPage() {
  ...
  const { shell } = asiaPacificEditorial

  return (
    <MarketingPageShell title={shell.title} tagline={shell.tagline}>
      <AsiaPacificIdentitySection reduceMotion={reduceMotion} />
      <AsiaPacificPostureSection reduceMotion={reduceMotion} />
      <AsiaPacificRolloutSection reduceMotion={reduceMotion} />
      <AsiaPacificClosingSection reduceMotion={reduceMotion} />
    </MarketingPageShell>
  )
}
