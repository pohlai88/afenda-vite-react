...
import { pdpaEditorial } from "./pdpa-page-editorial"

interface SectionProps {
  readonly reduceMotion: boolean
}

function PdpaIdentitySection({ reduceMotion }: SectionProps) {
  const { opening } = pdpaEditorial

  return (
    <MarketingPageSection
      className="relative overflow-hidden border-b border-border/70"
      containerClassName="pb-16 pt-28 md:pb-20 lg:pb-24 lg:pt-32"
      aria-labelledby="pdpa-hero-heading"
    >
      <MarketingStructuralGridBg sizePx={38} className="opacity-[0.05]" />
      <MarketingStructuralBaseline />
...
function PdpaRegionalFrameSection({ reduceMotion }: SectionProps) {
  const { regionalFrame } = pdpaEditorial

  return (
    <MarketingPageSection
      className="relative border-b border-border/70"
      aria-labelledby="pdpa-frame-heading"
    >
      <MarketingStructuralVerticalRulesBg
        gapPx={104}
      ...
      <div className="relative z-[1]">
        <div className="grid gap-14 lg:grid-cols-[minmax(0,0.94fr)_minmax(0,1.06fr)] lg:items-start lg:gap-16">
          <motion.div {...getMarketingReveal(reduceMotion)}>
            <MarketingSectionHeading
              as="h2"
              id="pdpa-frame-heading"
              kicker="PDPA frame"
              title={regionalFrame.title}
            ...
            <div className="mt-8 max-w-3xl space-y-6 text-base leading-8 text-pretty text-muted-foreground">
              {regionalFrame.body.map((paragraph) => (
          ...
          <motion.div {...getMarketingReveal(reduceMotion, 0.06)}>
            {regionalFrame.matrix.map((item, rowIndex) => (
              <div
                key={item.title}
...
function PdpaContinuitySection({ reduceMotion }: SectionProps) {
  const { continuity } = pdpaEditorial
  const chainLabelId = "pdpa-continuity-chain"

  return (
    <MarketingPageSection
      className="relative overflow-hidden border-b border-border/70"
      aria-labelledby="pdpa-continuity-heading"
    >
      ...
      <div className="relative z-[1]">
        <motion.div {...getMarketingReveal(reduceMotion)}>
          <MarketingSectionHeading
            as="h2"
            id="pdpa-continuity-heading"
            kicker="Continuity"
            title={continuity.title}
            description={continuity.intro}
        ...
        >
          ...
          >
            <ol className="flex min-w-[min(100%,68rem)] flex-col divide-y divide-border/70 md:min-w-0 md:flex-row md:divide-x md:divide-y-0">
              {continuity.chain.map((step, index) => (
                <li
                  key={step.label}
...
function PdpaClosingSection({ reduceMotion }: SectionProps) {
  const { cta } = pdpaEditorial

  return (
    <MarketingPageSection aria-labelledby="pdpa-closing-heading">
      <motion.div
...
export default function PdpaPage() {
  ...
  const { shell } = pdpaEditorial

  return (
    <MarketingPageShell title={shell.title} tagline={shell.tagline}>
      <PdpaIdentitySection reduceMotion={reduceMotion} />
      <PdpaRegionalFrameSection reduceMotion={reduceMotion} />
      <PdpaContinuitySection reduceMotion={reduceMotion} />
      <PdpaClosingSection reduceMotion={reduceMotion} />
    </MarketingPageShell>
  )
}
