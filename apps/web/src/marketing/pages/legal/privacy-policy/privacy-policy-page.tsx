...
import { privacyPolicyEditorial } from "./privacy-policy-page-editorial"

interface SectionProps {
  readonly reduceMotion: boolean
}

function PrivacyPolicyIdentitySection({ reduceMotion }: SectionProps) {
  const { opening } = privacyPolicyEditorial
          ...
          <div>
            <motion.div {...getMarketingReveal(reduceMotion)}>
              <MarketingSectionHeading
                ...
                titleClassName="max-w-[15ch] text-[clamp(3rem,8vw,5.8rem)] leading-[0.9] tracking-[-0.07em] text-balance"
              />
...
function PrivacyPolicyPostureSection({ reduceMotion }: SectionProps) {
  const { posture } = privacyPolicyEditorial

  return (
    <MarketingPageSection
      className="relative border-b border-border/70"
      aria-labelledby="privacy-policy-posture-heading"
    >
      <MarketingStructuralVerticalRulesBg
        gapPx={104}
      ...
      <div className="relative z-[1]">
        <div className="grid gap-14 lg:grid-cols-[minmax(0,0.94fr)_minmax(0,1.06fr)] lg:items-start lg:gap-16">
          <motion.div {...getMarketingReveal(reduceMotion)}>
            <MarketingSectionHeading
              as="h2"
              id="privacy-policy-posture-heading"
          ...
          <motion.div {...getMarketingReveal(reduceMotion, 0.06)}>
            {posture.matrix.map((item, rowIndex) => (
              <div
                key={item.title}
...
function PrivacyPolicyControlChainSection({ reduceMotion }: SectionProps) {
  const { controlChain } = privacyPolicyEditorial
  const chainLabelId = "privacy-policy-control-chain"

  return (
    <MarketingPageSection
      className="relative overflow-hidden border-b border-border/70"
      aria-labelledby="privacy-policy-control-heading"
    >
      ...
      <div className="relative z-[1]">
        <motion.div {...getMarketingReveal(reduceMotion)}>
          <MarketingSectionHeading
            as="h2"
            id="privacy-policy-control-heading"
            kicker="Control chain"
            title={controlChain.title}
            description={controlChain.intro}
        ...
        >
          ...
          >
            <ol className="flex min-w-[min(100%,68rem)] flex-col divide-y divide-border/70 md:min-w-0 md:flex-row md:divide-x md:divide-y-0">
              {controlChain.chain.map((step, index) => (
                <li
                  key={step.label}
...
function PrivacyPolicyClosingSection({ reduceMotion }: SectionProps) {
  const { cta } = privacyPolicyEditorial

  return (
    <MarketingPageSection aria-labelledby="privacy-policy-closing-heading">
      <motion.div
      ...
      >
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,20rem)] lg:items-start lg:gap-14">
          <div>
            <MarketingSectionHeading
              as="h2"
              id="privacy-policy-closing-heading"
              kicker="Next"
              title={cta.title}
...
export default function PrivacyPolicyPage() {
  ...
  const { shell } = privacyPolicyEditorial

  return (
    <MarketingPageShell title={shell.title} tagline={shell.tagline}>
      <PrivacyPolicyIdentitySection reduceMotion={reduceMotion} />
      <PrivacyPolicyPostureSection reduceMotion={reduceMotion} />
      <PrivacyPolicyControlChainSection reduceMotion={reduceMotion} />
      <PrivacyPolicyClosingSection reduceMotion={reduceMotion} />
    </MarketingPageShell>
  )
}
