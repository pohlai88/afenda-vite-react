...
import { trustCenterEditorial } from "./trust-center-page-editorial"

interface SectionProps {
  readonly reduceMotion: boolean
}

function TrustCenterIdentitySection({ reduceMotion }: SectionProps) {
  const { opening } = trustCenterEditorial

  return (
    <MarketingPageSection
      className="relative overflow-hidden border-b border-border/70"
      containerClassName="pb-16 pt-28 md:pb-20 lg:pb-24 lg:pt-32"
      aria-labelledby="trust-center-hero-heading"
    >
      <MarketingStructuralGridBg sizePx={42} className="opacity-[0.05]" />
      <MarketingStructuralBaseline />
...
function TrustCenterModelSection({ reduceMotion }: SectionProps) {
  const { trustModel } = trustCenterEditorial
  const chainLabelId = "trust-center-chain"

  return (
    <MarketingPageSection
      className="relative overflow-hidden border-b border-border/70"
      aria-labelledby="trust-center-model-heading"
    >
      ...
      <div className="relative z-[1]">
        <motion.div {...getMarketingReveal(reduceMotion)}>
          <MarketingSectionHeading
            as="h2"
            id="trust-center-model-heading"
            kicker="Trust model"
            title={trustModel.title}
        ...
        >
          ...
          >
            <ol className="flex min-w-[min(100%,72rem)] flex-col divide-y divide-border/70 md:min-w-0 md:flex-row md:divide-x md:divide-y-0">
              {trustModel.chain.map((step, index) => (
                <li
                  key={step.label}
...
function TrustCenterSignalsSection({ reduceMotion }: SectionProps) {
  const { signals } = trustCenterEditorial

  return (
    <MarketingPageSection
      className="relative border-b border-border/70"
      aria-labelledby="trust-center-signals-heading"
    >
      <MarketingStructuralVerticalRulesBg
        gapPx={104}
      ...
      <div className="relative z-[1]">
        <div className="grid gap-14 lg:grid-cols-[minmax(0,0.94fr)_minmax(0,1.06fr)] lg:items-start lg:gap-16">
          <motion.div {...getMarketingReveal(reduceMotion)}>
            <MarketingSectionHeading
              as="h2"
              id="trust-center-signals-heading"
              kicker="Review conditions"
              title={signals.title}
              titleClassName="max-w-[34ch] tracking-[-0.045em] text-balance"
            />
            <div className="mt-8 max-w-3xl space-y-6 text-base leading-8 text-pretty text-muted-foreground">
              {signals.body.map((paragraph) => (
                <p key={paragraph}>{paragraph}</p>
              ))}
            </div>
          </motion.div>
          ...
          <motion.div {...getMarketingReveal(reduceMotion, 0.06)}>
            {signals.items.map((item, rowIndex) => (
              <div
                key={item.title}
        ...
        <motion.blockquote
          className="mt-16 max-w-4xl border-l-2 border-primary/50 pl-6 md:mt-20 md:pl-8"
...
function TrustCenterClosingSection({ reduceMotion }: SectionProps) {
  const { cta } = trustCenterEditorial

  return (
    <MarketingPageSection aria-labelledby="trust-center-closing-heading">
      <motion.div
      ...
      >
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,20rem)] lg:items-start lg:gap-14">
          <div>
            <MarketingSectionHeading
              as="h2"
              id="trust-center-closing-heading"
              kicker="Next"
              title={cta.title}
...
export default function TrustCenterPage() {
  ...
  const { shell } = trustCenterEditorial

  return (
    <MarketingPageShell title={shell.title} tagline={shell.tagline}>
      <TrustCenterIdentitySection reduceMotion={reduceMotion} />
      <TrustCenterModelSection reduceMotion={reduceMotion} />
      <TrustCenterSignalsSection reduceMotion={reduceMotion} />
      <TrustCenterClosingSection reduceMotion={reduceMotion} />
    </MarketingPageShell>
  )
}
