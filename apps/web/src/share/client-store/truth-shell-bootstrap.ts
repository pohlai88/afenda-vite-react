...
import { useTruthHealthStore } from './truth-health-store'
...
export function useTruthShellBootstrap() {
  ...
  useEffect(() => {
    ...
    const shellStore = useAppShellStore.getState()
    if (shellStore.currentUser === null) {
      shellStore.setUser(TRUTH_DEMO_USER)
    }
