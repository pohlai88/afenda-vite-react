...
/**
 * UserMenu provides a 6-section dropdown:
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent className="w-64" align="end" forceMount>
              ...
              <div className="px-2 py-1.5 text-sm">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={cn(
                      'font-medium',
                      healthSummary.integrityScore >= 90
                        ? 'text-(--color-truth-valid)'
                        : healthSummary.integrityScore >= 70
                          ? 'text-(--color-truth-warning)'
                          : 'text-(--color-truth-broken)',
                    )}
                  >
                ...
                {(healthSummary.invariantFailures.length > 0 ||
                  ...
                  <div className="mt-2 flex items-center gap-2 text-xs">
                    {healthSummary.invariantFailures.length > 0 && (
                      <span className="rounded bg-truth-broken/10 px-1.5 py-0.5 text-(--color-truth-broken)">
                        {healthSummary.invariantFailures.length}{' '}
                        {t('user_menu.failures', 'failures')}
                      </span>
                    )}
                    {healthSummary.warnings.length > 0 && (
                      <span className="rounded bg-truth-warning/10 px-1.5 py-0.5 text-(--color-truth-warning)">
        ...
        <DropdownMenuGroup>
          <DropdownMenuSub>
            ...
            <DropdownMenuPortal>
              <DropdownMenuSubContent>
                <DropdownMenuItem>
                  <CircleIcon className="mr-2 size-3 fill-green-500 text-green-500" />
                  {t('user_menu.status_available', 'Available')}
                </DropdownMenuItem>
                <DropdownMenuItem>
                  <CircleIcon className="mr-2 size-3 fill-red-500 text-red-500" />
                  {t('user_menu.status_busy', 'Busy')}
                </DropdownMenuItem>
                <DropdownMenuItem>
                  <CircleIcon className="mr-2 size-3 fill-yellow-500 text-yellow-500" />
                  {t('user_menu.status_away', 'Away')}
                </DropdownMenuItem>
                <DropdownMenuItem>
                  <CircleIcon className="mr-2 size-3 fill-gray-400 text-gray-400" />
        ...
        <DropdownMenuGroup>
          ...
          <DropdownMenuItem asChild>
            <Link to="/app/audit-log" className="cursor-pointer">
