...
type IdentityIntelligencePanelProps = {
  readonly resource: AuthIntelligenceResource
}
...
type IntelligenceMetricsProps = {
  ...
  /** When false, omits the primary trust header row (used under loading/unavailable chrome). */
  readonly showPrimaryHeader: boolean
}

function IntelligenceMetrics(props: IntelligenceMetricsProps) {
  const { snapshot, loading, showPrimaryHeader } = props
  ...
  return (
    <>
      ...
      <dl className="grid gap-[0.75rem] text-sm">
...
/**
 * Left-rail “identity intelligence” surface: maps {@link AuthIntelligenceResource}
 * to loading, unavailable, or full trust posture (no route-level HUD imports).
 */
      ...
      <div className="ui-stack-tight">
        ...
        <div className="rounded-xl border border-warning/30 bg-warning/5 px-4 py-3">
          <p className="text-sm font-medium text-foreground">
            Security context is currently unavailable.
          </p>
          <p className="mt-1 text-sm text-muted-foreground">
            Standard sign-in options remain available, but trust posture could
            not be evaluated for this session.
          </p>
        </div>
