import { i18n } from "../adapters/i18next-adapter"
...
function stableSerializeOptions(options: object | undefined): string {
  if (!options) {
    return ""
  }
...
function getCachedNumberFormatter(
  ...
  const key = `${locale}::${stableSerializeOptions(options)}`
...
export function formatCurrency(
  ...
  return getCachedNumberFormatter(currencyFormatterCache, getFormatLocale(), {
    style: "currency",
    currency: normalizedCurrency,
    ...options,
  }).format(amount)
...
export function formatPercent(
  ...
  return getCachedNumberFormatter(percentFormatterCache, getFormatLocale(), {
    style: "percent",
    ...options,
  }).format(value)
}
...
export function formatDate(
  ...
  return getCachedDateFormatter(getFormatLocale(), options).format(resolvedDate)
}
