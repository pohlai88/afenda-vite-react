...
import { initI18n } from "../adapters/i18next-adapter"
import {
  formatCurrency,
...
describe("i18n format helpers", () => {
  ...
  test("getFormatLocale maps active language to BCP 47", async () => {
    const { i18n } = await import("../adapters/i18next-adapter")
  ...
  test("cached formatters remain locale-aware across language changes", async () => {
    const { i18n } = await import("../adapters/i18next-adapter")
    await i18n.changeLanguage("en")
    const enNumber = formatNumber(1000.2)

    await i18n.changeLanguage("id")
    const idNumber = formatNumber(1000.2)

    expect(enNumber).not.toBe(idNumber)
    expect(formatCurrencyByBusinessRule(1234.56, "VND")).toMatch(/1[.,]235/)
    await i18n.changeLanguage("en")
  })
