...
describe("initI18n", () => {
  ...
  test("prioritizes QA querystring override before localStorage in development", () => {
    expect(getI18nDetectionOrder(true)).toEqual([
      "querystring",
      "localStorage",
      "navigator",
      "htmlTag",
  ...
  test("does not use querystring locale detection in production", () => {
    expect(getI18nDetectionOrder(false)).toEqual([
      "localStorage",
      "navigator",
      "htmlTag",
    ])
