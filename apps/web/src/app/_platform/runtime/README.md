...
Matches `_template`: `adapters/` (fetch + timeout), `services/` (`api-client-service.ts`), `policy/` (`api-client-policy.ts`), `types/` (`api-client-types.ts`), `hooks/`, `components/`, root runtime config (`api-client-config.ts`), `scripts/` (`generate-api-client-capability-report.ts`), `__tests__/` (`api-client-policy.test.ts`).
