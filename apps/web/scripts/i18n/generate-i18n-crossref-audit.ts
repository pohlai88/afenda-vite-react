 ...
 * Writes:
 *   - `apps/web/src/app/_platform/i18n/audit/conflicts.json` — deterministic conflict statuses
 *   - `apps/web/src/app/_platform/i18n/audit/provenance.json`  — import provenance trail
 *
 * Run from apps/web: `pnpm run i18n:generate-crossref-audit`
 */
...
const i18nRoot = join(appRoot, "src", "app", "_platform", "i18n")
...
const frappeFallbackPath = join(
  ...
  "i18n",
  "data",
  "i18n-corpus-frappe.sample.json"
)
const odooGeneratedPath = join(
  appRoot,
  "scripts",
  "i18n",
  "data",
  "i18n-corpus-odoo.json"
)
...
function main(): void {
  ...
  if (!tolgee) {
    throw new Error(
      `Missing primary Tolgee corpus at ${tolgeeGeneratedPath}. Run "pnpm run script:generate-i18n-corpus" first.`
    )
  }
