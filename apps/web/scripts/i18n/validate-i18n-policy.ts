 ...
 * unresolved audit conflicts, and hardcoded JSX string literals. Run from apps/web:
 * `pnpm run i18n:validate-policy`
 */
...
import {
  ...
  RELEASE_NAMESPACES,
  SUPPORTED_LOCALES,
} from "../../src/app/_platform/i18n/policy/i18n-policy.js"

const __dirname = dirname(fileURLToPath(import.meta.url))
const appRoot = join(__dirname, "..", "..")
const localesRoot = join(appRoot, "src", "app", "_platform", "i18n", "locales")
...
function isExcludedFromNonEnRatio(fk: string): boolean {
  ...
  if (isMarketingAuditOnlyI18nKey(fk)) return true
  return IDENTICAL_RATIO_EXCLUDED_PREFIXES.some((prefix: string) =>
    fk.startsWith(prefix)
  )
}

function isMarketingAuditOnlyI18nKey(fk: string): boolean {
  return fk.startsWith("shell.marketing.")
}
...
function main(): void {
  const errors: string[] = []
  const marketingAuditWarnings: string[] = []
  ...
  const runtimeJsxWarnings = scanForHardcodedJsxStrings(
    join(appRoot, "src", "app")
  )
  for (const w of runtimeJsxWarnings) {
    errors.push(w)
  }

  const marketingJsxWarnings = scanForHardcodedJsxStrings(
    join(appRoot, "src", "marketing")
  )
  for (const warning of marketingJsxWarnings) {
    marketingAuditWarnings.push(warning)
  }

  if (marketingAuditWarnings.length > 0) {
    console.warn("marketing/editorial i18n audit warnings:\n")
    for (const warning of marketingAuditWarnings) {
      console.warn(`- ${warning}`)
    }
    console.warn(
      "\nmarketing/editorial copy is audit-only during green-first closure and does not block runtime release gates."
    )
  }
...
function scanForHardcodedJsxStrings(scanRoot: string): string[] {
  const warnings: string[] = []
  const normalizedScanRoot = scanRoot.replace(/\\/g, "/")
  const tsxFiles = collectTsxFiles(scanRoot)
  ...
  for (const file of tsxFiles) {
    ...
    const relPath = file.replace(appRoot, "").replace(/\\/g, "/")
    if (!file.replace(/\\/g, "/").startsWith(normalizedScanRoot)) {
      continue
    }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Skip common type signatures that look like JSX text to the regex.
      if (/(=>|:)\s*Promise</.test(line)) continue
