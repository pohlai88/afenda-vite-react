...
`sync-pack` is the active Feature Factory runtime inside `@afenda/features-sdk`.

It turns curated feature candidates into governed internal decision artifacts, planning packs, scaffold manifests, and app/operator handoff guidance while keeping product implementation out of the SDK package.
...
- decision-oriented ranking artifacts with confidence and likely implementation surfaces
- governed 11-file planning-pack generation
- scaffold manifests with implementation hints and handoff boundaries
...
| `pnpm run feature-sync:report`           | operator utility   | Print grouped candidate portfolio and decision output. |
| `pnpm run feature-sync:generate`         | operator utility   | Generate governed planning packs.                      |
| `pnpm run feature-sync:scaffold`         | operator utility   | Generate a scaffold manifest and handoff contract.     |
...
Sync-Pack is the Feature Factory, not the feature home.

Implementation belongs in `apps/web` and `apps/api`.
