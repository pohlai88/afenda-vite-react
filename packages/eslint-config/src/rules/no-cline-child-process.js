...
const clineRuntimePathPattern =
  /[\\/]packages[\\/]cline[\\/]src[\\/].+\.[cm]?[jt]sx?$/u
 ...
 * @returns {boolean}
 */
function isClineRuntimeFile(filename) {
  return typeof filename === "string" && clineRuntimePathPattern.test(filename)
}
...
const rule = {
  meta: {
    ...
    docs: {
      description:
        "Disallow child_process imports and subprocess execution inside the governed Cline runtime layer.",
    },
    schema: [],
    messages: {
      noChildProcessImport:
        "RG-EXEC-001: packages/cline/src may not import child_process. Execute Sync-Pack workflows through the public SDK instead of spawning subprocesses.",
      noSubprocessCall:
        'RG-EXEC-001: packages/cline/src may not execute subprocess APIs such as "{{ api }}". Route execution through the typed SDK workflow catalog instead.',
    },
  },
  create(context) {
    const filename = context.filename
    if (!isClineRuntimeFile(filename)) {
      return {}
    }
