...
import rule from "../enforce-operator-kernel-boundaries.js"

const tempDirs = []

async function lintOperatorKernelFile(options) {
  const dir = await mkdtemp(
    path.join(os.tmpdir(), "afenda-operator-kernel-boundaries-")
  )
  tempDirs.push(dir)
    ...
    overrideConfig: [
      {
        ...
        plugins: {
          "afenda-ui": {
            rules: {
              "enforce-operator-kernel-boundaries": rule,
            },
          },
        },
        rules: {
...
describe("enforce-operator-kernel-boundaries", () => {
  it("blocks relative imports into packages/features-sdk", async () => {
    const messages = await lintOperatorKernelFile({
      fileSegments: [
        "packages",
        "operator-kernel",
        "src",
    ...
    expect(messages[0]?.message).toContain(
      "packages/operator-kernel may not reach into packages/features-sdk"
    )
  })
