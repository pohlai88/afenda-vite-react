...
const uiDriftRulePolicyDefinition = {
  ...
  "UIX-AST-CONTROL-001": { level: "warning", implemented: false },
...
export function validateUiDriftRulePolicyLinks(): void {
  for (const [ruleCode, entry] of Object.entries(uiDriftRulePolicy)) {
    ...
    if (entry.implemented && !eslintRuleName) {
      throw new Error(
        `Implemented UI drift rule "${ruleCode}" is missing an ESLint rule mapping.`
      )
    }

    if (!entry.implemented && eslintRuleName) {
      throw new Error(
        `UI drift rule "${ruleCode}" has an ESLint rule mapping but is marked unimplemented.`
      )
    }
  }
}
