...
import { createOperatorMcpAdapterRuntime } from "../src/mcp-adapter/index.js"
...
describe("governed Operator Kernel runtime", () => {
  ...
  it("allows architect commander to run generate with an isolated output directory", async () => {
    ...
    const outputDirectory = await mkdtemp(
      path.join(os.tmpdir(), "operator-kernel-generate-")
    )
