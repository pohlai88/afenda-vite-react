...
**Import order (canonical):** `theme-tokens-light.css` → `theme-dark.css` → `theme-keyframes.css` → `theme-tokens-layout.css` → `theme-density.css` → `theme-shadcn-compat.css` (see `src/theme/theme.css` and app `index.css` / `local.css`). Density is loaded **after** layout so `--density-scale` overrides apply on top of registered layout tokens.
