 ...
 * Zod v4 helpers for stringly-typed numeric configuration (forms, query strings); complements `readOptionalInteger` in `./environment-integer-parsing.ts`. Not Drizzle DDL; `.schema.ts` may satisfy `guard-schema-modules.ts` when present. Keep wording generic for `schema-support-boundary.test.ts`. Migrations N/A for this file.
 ...
 *
 * This module: `src/schema/environment-support/environment-integer.schema.ts` — `optionalCoercedIntegerSchema` and `optionalIntegerWithDefault`.
 */
import { z } from "zod"
