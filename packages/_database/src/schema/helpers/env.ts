 ...
 * Shared utilities under `src/schema/helpers/` — framework-agnostic integer parsing from env strings (no DDL; used by `src/client.ts`). Migrations N/A for this file.
 ...
 *
 * This module: `src/schema/helpers/env.ts` — `readOptionalInteger` for safe `process.env` → number coercion (keep wording generic; see `helper-boundary.test.ts`).
 */
