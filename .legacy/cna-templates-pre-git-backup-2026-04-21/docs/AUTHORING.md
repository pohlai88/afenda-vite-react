...
Define them in `afenda.config.json` at the root of the template directory:
...
> `afenda.config.json` is co-located with the template so it works with both slug resolution and `file://` local URLs.
> Do **not** put `customOptions` in `templates.json` — it is no longer read from there.

> **Note:** The published `create-awesome-node-app` CLI reads `cna.config.json`. This repository standardizes on `afenda.config.json`; when running stock CNA against this tree, symlink or copy `afenda.config.json` to `cna.config.json`.
