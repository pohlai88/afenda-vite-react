...
- **`afenda.config.json`** - lives in the template directory, defines `customOptions` (interactive CLI prompts; answers become EJS variables). Replaces the old `customOptions` field in `templates.json`. (Upstream CNA reads `cna.config.json`; this bank uses `afenda.config.json`.)
