...
test.describe("Orders Management", () => {
  test("should create a new order", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", {
        name: /tạo đơn hàng|create order|thêm đơn hàng/i,
      })
    ...
    const customerField = page
      .getByPlaceholder(/tên khách hàng|customer name/i)
      .first()
    ...
    const addProductBtn = page
      .getByRole("button", { name: /thêm sản phẩm|add product|add item/i })
      .first()
  ...
  test("should update order status", async ({ page }) => {
    ...
    if (await statusButton.isVisible({ timeout: 5_000 })) {
      ...
      // Select new status
      const processingStatus = page
        .getByRole("option", { name: /đang xử lý|processing/i })
        .first()
  ...
  test("should cancel an order", async ({ page }) => {
    ...
    if (await cancelBtn.isVisible({ timeout: 5_000 })) {
      ...
      const confirmBtn = page
        .getByRole("button", { name: /xác nhận|confirm|yes/i })
        .first()
    ...
    if (await statusFilter.isVisible({ timeout: 5_000 })) {
      ...
      if (await processingOption.isVisible()) {
        ...
        const filteredItems = page.locator(
          '[data-testid="order-item"], .order-item, tr'
        )
