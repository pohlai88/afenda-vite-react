...
export class VNPayGateway {
  ...
  verifyCallback(params: Record<string, string>): PaymentCallback {
    ...
    const sortedQuery = Object.keys(verifyParams)
      .sort()
...
export class MoMoGateway {
  ...
  verifyCallback(data: Record<string, any>): PaymentCallback {
    ...
    const status: PaymentCallback["status"] =
      data.resultCode === 0 ? "SUCCESS" : "FAILED"
...
export class BankTransferGateway {
  constructor(
    private config: NonNullable<PaymentGatewayConfig["bankTransfer"]>
  ) {}
...
export class PaymentGatewayManager {
  ...
  constructor(config: PaymentGatewayConfig) {
    ...
    if (config.bankTransfer)
      this.bankTransfer = new BankTransferGateway(config.bankTransfer)
    ...
    switch (request.method) {
      ...
      case "BANK_TRANSFER":
        if (!this.bankTransfer)
          throw new Error("Thông tin ngân hàng chưa được cấu hình")
      ...
      default:
        throw new Error(
          `Phương thức thanh toán không được hỗ trợ: ${request.method}`
        )
    }
  }
  ...
  getAvailableMethods(): Array<{
    ...
    return [
      ...
      },
      {
        ...
        icon: "🏦",
        available: !!this.bankTransfer,
      },
      {
        method: "VNPAY",
        name: "VNPay (ATM/Visa/QR)",
        icon: "💳",
        available: !!this.vnpay,
      },
