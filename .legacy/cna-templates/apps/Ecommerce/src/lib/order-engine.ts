 ...
 */
export function generateOrderJournalEntry(order: {
  orderNumber: string
  total: Decimal
  taxAmount: Decimal
  discount: Decimal
  ...
  const debitAccount =
    order.paymentMethod === "COD"
      ? "131"
      : ["BANK_TRANSFER", "VNPAY"].includes(order.paymentMethod)
        ? "112"
        : "131"
  ...
  const linesWithCost = order.lines.filter(
    (l) => l.costPrice && l.costPrice.gt(0)
  )
...
export function calculateOrderAnalytics(
  ...
  for (const order of orders) {
    ...
    revenueByMethod[method] = (revenueByMethod[method] || new Decimal(0)).add(
