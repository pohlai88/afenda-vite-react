...
export function recalculateCart(
  ...
  CartState,
  "subtotal" | "taxAmount" | "discount" | "shippingFee" | "total" | "itemCount"
> {
...
export function calculateShippingRates(
  ...
  const isSameProvince =
    normalizeProvince(address.province) === normalizeProvince(origin.province)
  const isUrban = ["Hồ Chí Minh", "Hà Nội", "Đà Nẵng"].some((city) =>
    normalizeProvince(address.province).includes(normalizeProvince(city))
  )
...
function calculateProviderFee(
  ...
  const rates: Record<
    string,
    { base: number; perKg: number; sameCity: number }
  > = {
...
export function validateCheckout(
  ...
  if (!request.customerPhone?.trim()) {
    ...
    !/^(0[3-9]\d{8}|84[3-9]\d{8})$/.test(
      request.customerPhone.replace(/\s/g, "")
    )
  ) {
    errors.push({
      field: "customerPhone",
