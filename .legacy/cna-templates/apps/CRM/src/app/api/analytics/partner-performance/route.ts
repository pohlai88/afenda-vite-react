  ...
  try {
    ...
    const partnerStats = partners.map((p: any) => {
      ...
      const lostDeals = deals.filter((d: any) =>
        lostStageIds.includes(d.stageId)
      )
      ...
      const revenue = wonDeals.reduce(
        (sum: number, d: any) =>
          sum + convertToBase(Number(d.value), d.currency),
        0
      )
      ...
      const commissionEarned = comms
        ...
        .reduce(
          (sum: number, c: any) =>
            sum + convertToBase(Number(c.amount), c.currency),
          0
        )
      ...
      return {
        ...
        winRate:
          closedCount > 0
            ? Math.round((wonCount / closedCount) * 100) / 100
            : 0,
