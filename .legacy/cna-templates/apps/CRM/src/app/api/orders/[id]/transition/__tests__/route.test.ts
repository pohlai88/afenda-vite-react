...
vi.mock("@/lib/auth/rbac", async () => {
  ...
  return {
    ...actual,
    requireOwnerOrRole: (...args: any[]) => mockRequireOwnerOrRole(...args),
  }
})
...
describe("POST /api/orders/[id]/transition", () => {
  ...
  it("CANCELLED succeeds with reason", async () => {
    ...
    const req = mockRequest({
      method: "POST",
      body: { toStatus: "CANCELLED", cancellationReason: "Customer request" },
    })
    const res = await POST(req, { params: { id: "order-1" } })
    expect(res.status).toBe(200)
  })
  ...
  it("rejects refundAmount > total", async () => {
    ...
    const req = mockRequest({
      method: "POST",
      body: { toStatus: "REFUNDED", refundAmount: 9999999 },
    })
    const res = await POST(req, { params: { id: "order-1" } })
    expect(res.status).toBe(400)
  })
