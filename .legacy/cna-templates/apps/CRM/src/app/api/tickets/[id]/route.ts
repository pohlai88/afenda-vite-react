...
export async function GET(
  ...
  try {
    ...
    let sla = null
    if (
      ["OPEN", "IN_PROGRESS", "WAITING_CUSTOMER", "RESOLVED"].includes(
        ticket.status
      )
    ) {
    ...
    if (error instanceof AuthError)
      return handleApiError(Unauthorized(error.message), "/api/tickets/[id]")
...
export async function PATCH(
  ...
  try {
    ...
    if (!existing)
      return handleApiError(NotFound("Ticket"), "/api/tickets/[id]")
    ...
    if (data.status && data.status !== existing.status) {
      if (!isValidTransition(existing.status, data.status)) {
        return handleApiError(
          BadRequest(
            `Không thể chuyển từ ${existing.status} sang ${data.status}`
          ),
          "/api/tickets/[id]"
        )
      }
    ...
    if (data.status === "RESOLVED" && existing.status !== "RESOLVED") {
      eventBus
        .emit(CRM_EVENTS.TICKET_RESOLVED, {
          timestamp: new Date().toISOString(),
          userId: user.id,
          ticketId: ticket.id,
          ticket: {
