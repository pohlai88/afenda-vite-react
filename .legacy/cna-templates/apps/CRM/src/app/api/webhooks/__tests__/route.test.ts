    ...
    mockWebhook.findMany.mockResolvedValue([
      {
        ...
        createdAt: new Date(),
        _count: { logs: 5 },
        logs: [{ success: true, createdAt: new Date() }],
      },
    ])
    const res = await GET()
...
describe("POST /api/webhooks", () => {
  ...
  it("rejects invalid URL", async () => {
    ...
    const req = mockRequest({
      method: "POST",
      body: { name: "Bad", url: "not-a-url", events: ["contact.created"] },
    })
    const res = await POST(req)
  ...
  it("rejects empty events array", async () => {
    ...
    const req = mockRequest({
      method: "POST",
      body: { name: "Empty", url: "https://example.com", events: [] },
    })
    const res = await POST(req)
    expect(res.status).toBe(400)
  })
  ...
  it("non-ADMIN gets 403", async () => {
    ...
    const req = mockRequest({
      method: "POST",
      body: { name: "X", url: "https://x.com", events: ["a"] },
    })
    const res = await POST(req)
    expect(res.status).toBe(403)
  })
