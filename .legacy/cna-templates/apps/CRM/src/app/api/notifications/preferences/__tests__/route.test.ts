...
vi.mock("@/lib/prisma", () => ({
  prisma: { notificationPreference: mockPref },
}))
...
describe("GET /api/notifications/preferences", () => {
  ...
  it("reflects saved preferences", async () => {
    mockGetCurrentUser.mockResolvedValue(mockCurrentUser())
    mockPref.findMany.mockResolvedValue([
      { eventType: "quote.accepted", inApp: false, email: true },
    ])
    const res = await GET()
    const body = await res.json()
...
describe("PUT /api/notifications/preferences", () => {
  ...
  it("upserts preferences correctly", async () => {
    ...
    mockPref.findMany.mockResolvedValue([
      { eventType: "quote.accepted", inApp: true, email: true },
    ])
  ...
  it("filters out invalid event types", async () => {
    ...
    const req = mockRequest({
      ...
      body: {
        preferences: [
          { eventType: "invalid.event", inApp: true, email: true },
          { eventType: "quote.accepted", inApp: true, email: false },
        ],
      },
    })
    const res = await PUT(req)
