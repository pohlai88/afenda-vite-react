...
export default function PortalTicketDetailPage() {
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-xl border border-gray-200 p-8 animate-pulse h-64" />
    )
  }
    ...
    <div className="space-y-3">
        ...
        <div className="flex items-center gap-2 mb-1">
          ...
          {ticket.assignee && (
            <span className="text-xs text-gray-400">
              · Phụ trách: {ticket.assignee.name}
            </span>
          )}
        </div>
      ...
      {ticket.status === "RESOLVED" && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 text-sm text-emerald-700">
          Yêu cầu đã được giải quyết. Nếu bạn cần hỗ trợ thêm, hãy gửi tin nhắn
          bên dưới để mở lại.
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-gray-200 divide-y divide-gray-100">
        {ticket.messages?.map((msg: any) => {
          ...
          const name = isStaff
            ...
            : `${msg.portalUser?.firstName || ""} ${msg.portalUser?.lastName || ""}`.trim() ||
              "Bạn"
          return (
            <div key={msg.id} className="p-4">
              <div className="flex items-center gap-2 mb-1">
                <span
                  className={`text-sm font-medium ${isStaff ? "text-blue-600" : "text-gray-900"}`}
                >
                  {name}
                </span>
                <span className="text-xs text-gray-400">
