    ...
    actionReg.mutate(
      {
        partnerId: id,
        regId,
        data: { status: "REJECTED", rejectionNote: "Declined by manager" },
      },
      {
    ...
    >
        ...
        <Card className="glass-card-static">
          <CardContent className="p-4 text-center">
            ...
            <div className="text-xs text-[var(--crm-text-muted)]">
              {t("partner.commission.paidTotal")}
            </div>
          </CardContent>
        </Card>
        <Card className="glass-card-static">
          <CardContent className="p-4 text-center">
            ...
            <div className="text-xs text-[var(--crm-text-muted)]">
              {t("partner.commission.pendingTotal")}
            </div>
          </CardContent>
        </Card>
        <Card className="glass-card-static">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        ...
        {partner.territory && (
          <Badge
            variant="outline"
            className="text-xs border-none bg-[var(--crm-bg-subtle)] text-[var(--crm-text-secondary)]"
          >
        ...
        <Badge
          variant="outline"
          className="text-xs border-none bg-[var(--crm-bg-subtle)] text-[var(--crm-accent-text)]"
        >
            ...
            <Table>
              ...
              <TableBody>
                {(!partner.registrations ||
                  partner.registrations.length === 0) && (
                  <TableRow>
                    <TableCell
                      colSpan={5}
                ...
                {partner.registrations?.map((reg) => {
                  const statusConst = REGISTRATION_STATUSES.find(
                    (s) => s.value === reg.status
                  )
                  return (
                    <TableRow
                      key={reg.id}
                    ...
                    >
                      <TableCell>
                        ...
                        {reg.deal?.value && (
                          <div className="text-xs text-[var(--crm-text-muted)]">
                            {formatCurrency(
                              Number(reg.deal.value),
                              reg.deal.currency || "VND"
                            )}
                          </div>
                        )}
                      </TableCell>
                      <TableCell>
                        {statusConst && (
                          <Badge
                            ...
                            className="text-xs border-none"
                            style={{
                              backgroundColor: `${statusConst.color}20`,
                              color: statusConst.color,
                            }}
                          >
            ...
            <Table>
              ...
              <TableBody>
                ...
                {partner.commissions?.map((comm) => {
                  const statusConst = COMMISSION_STATUSES.find(
                    (s) => s.value === comm.status
                  )
                  return (
                    <TableRow
                      key={comm.id}
        ...
        <Card className="glass-card-static">
          <CardContent className="p-6 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <div className="text-xs text-[var(--crm-text-muted)] uppercase tracking-wide mb-1">
                  {t("partner.company")}
                </div>
                <div className="text-sm text-[var(--crm-text-primary)]">
                  <Link
                    href={`/companies/${partner.companyId}`}
                    className="text-[var(--crm-accent-text)] hover:underline"
                  >
