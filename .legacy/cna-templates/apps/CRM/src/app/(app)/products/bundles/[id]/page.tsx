...
export default function BundleDetailPage() {
  ...
  const bundleTypeDef = BUNDLE_TYPES.find(
    (bt) => bt.value === bundle.bundleType
  )
      ...
      <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            {bundleTypeDef && (
              <span
                ...
                style={{
                  backgroundColor: `${bundleTypeDef.color}20`,
                  color: bundleTypeDef.color,
                }}
              >
        ...
        <div className="glass-card-static p-4 space-y-3">
          ...
          {bundle.items.map((item) => (
            <div
              key={item.id}
              className="flex items-center justify-between p-2 rounded-md bg-[var(--crm-bg-subtle)]"
            >
        ...
        {bundle.compatibilityWarnings &&
          bundle.compatibilityWarnings.length > 0 && (
            <div className="space-y-2">
              <h3 className="text-sm font-semibold text-[var(--crm-text-primary)]">
                {t("bundles.compatibility")}
              </h3>
              <CompatibilityWarning
                rules={bundle.compatibilityWarnings}
        ...
        {bundle.pricingTiers && bundle.pricingTiers.length > 0 && (
          <div className="glass-card-static p-4 space-y-3">
            <h3 className="text-sm font-semibold text-[var(--crm-text-primary)]">
              {t("bundles.tierPricing")}
            </h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {bundle.pricingTiers.map((pt) => {
                const tierDef = CUSTOMER_TIERS.find(
                  (ct) => ct.value === pt.tier
                )
                ...
                return (
                  <div
                    key={pt.id}
                    className="p-3 rounded-md bg-[var(--crm-bg-subtle)] text-center"
                  >
                    <p
