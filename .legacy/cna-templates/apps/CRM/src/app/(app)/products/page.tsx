      ...
      <Tabs defaultValue={defaultTab} className="space-y-4">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <TabsList className="bg-[var(--crm-bg-subtle)] border border-[var(--crm-border)]">
            <TabsTrigger
              value="products"
              className="data-[state=active]:bg-[var(--crm-bg-hover)] data-[state=active]:text-[var(--crm-text-primary)] text-[var(--crm-text-muted)]"
            >
              {t("nav.products")}
            </TabsTrigger>
            <TabsTrigger
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-[var(--crm-border)] hover:bg-transparent">
                  ...
                  <TableHead className="text-[var(--crm-text-secondary)] text-[11px] font-semibold uppercase tracking-wide">
                    {t("products.category.OTHER")}
                  </TableHead>
                  <TableHead className="text-[var(--crm-text-secondary)] text-[11px] font-semibold uppercase tracking-wide text-right">
                    {t("quotes.unitPrice")}
                  </TableHead>
                  <TableHead className="text-[var(--crm-text-secondary)] text-[11px] font-semibold uppercase tracking-wide text-center">
                    ITAR
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {productsLoading ? (
                  ...
                  filteredProducts.map((p) => (
                    ...
                    >
                      <TableCell className="text-sm text-[var(--crm-text-primary)] font-medium">
                        {p.name}
                      </TableCell>
                      <TableCell className="text-xs text-[var(--crm-text-muted)]">
                        {p.sku || "-"}
                      </TableCell>
                      <TableCell className="text-xs text-[var(--crm-text-secondary)]">
                        {getCategoryLabel(p.category)}
                      </TableCell>
                      <TableCell className="text-sm text-[var(--crm-text-primary)] text-right">
        ...
        <TabsContent value="bundles" className="space-y-4">
          ...
          {bundlesLoading ? (
            <div className="text-center text-[var(--crm-text-muted)] py-8">
              {t("common.loading")}
            </div>
          ) : bundles.length === 0 ? (
            <div className="text-center text-[var(--crm-text-muted)] py-8">
