...
export default function ReportsPage() {
  ...
  const totalDealsPages = Math.max(
    1,
    Math.ceil(dealsOverTime.length / DEALS_PER_PAGE)
  )
  const pagedDeals = dealsOverTime.slice(
    salesPage * DEALS_PER_PAGE,
    (salesPage + 1) * DEALS_PER_PAGE
  )
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                <div className="kpi-card">
                  <p className="text-xs text-[var(--crm-text-muted)]">
                    {t("analytics.totalRevenue" as any)}
                  </p>
                  <p className="text-2xl font-bold text-[#10B981] mt-1">
                    {formatShortCurrency(analytics.kpis.totalRevenue)}
                  </p>
                </div>
                <div className="kpi-card">
                  <p className="text-xs text-[var(--crm-text-muted)]">
                    {t("analytics.totalQuotes" as any)}
                  </p>
                  <p className="text-2xl font-bold text-[var(--crm-text-primary)] mt-1">
                    {analytics.kpis.totalQuotes}
                  </p>
                </div>
                <div className="kpi-card">
                  <p className="text-xs text-[var(--crm-text-muted)]">
                    {t("analytics.totalOrders" as any)}
                  </p>
                  <p className="text-2xl font-bold text-[var(--crm-text-primary)] mt-1">
                ...
                <div className="kpi-card">
                  <p className="text-xs text-[var(--crm-text-muted)]">
                    {t("analytics.avgDealValue" as any)}
                  </p>
                  <p className="text-2xl font-bold text-[#3B82F6] mt-1">
                    {formatShortCurrency(analytics.kpis.avgDealValue)}
                  </p>
                </div>
              </div>
                ...
                <div className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <Bar
                        dataKey="won"
                        name={t("analytics.won" as any)}
                        fill="#10B981"
                        radius={[4, 4, 0, 0]}
                      />
                      <Bar
                        dataKey="lost"
                        name={t("analytics.lost" as any)}
                        fill="#EF4444"
                        radius={[4, 4, 0, 0]}
                      />
                    </BarChart>
                  ...
                  <TableHeader>
                    <TableRow className="border-[var(--crm-border)] hover:bg-transparent">
                      <TableHead className="text-[var(--crm-text-secondary)]">
                        {t("reports.month" as any)}
                      </TableHead>
                      <TableHead className="text-[var(--crm-text-secondary)] text-right">
                        {t("analytics.won" as any)}
                      </TableHead>
                      <TableHead className="text-[var(--crm-text-secondary)] text-right">
                        {t("analytics.lost" as any)}
                      </TableHead>
                      <TableHead className="text-[var(--crm-text-secondary)] text-right">
                        {t("analytics.totalRevenue" as any)}
                      </TableHead>
                  ...
                  <TableBody>
                    {pagedDeals.length === 0 ? (
                      <TableRow>
                        <TableCell
                          colSpan={4}
                          className="text-center text-[var(--crm-text-muted)] py-8"
                        >
                ...
                {totalDealsPages > 1 && (
                  <div className="flex items-center justify-between p-4 border-t border-[var(--crm-border)]">
                    <span className="text-xs text-[var(--crm-text-muted)]">
                      {t("common.showing")} {salesPage * DEALS_PER_PAGE + 1}-
                      {Math.min(
                        (salesPage + 1) * DEALS_PER_PAGE,
                        dealsOverTime.length
                      )}{" "}
                      {t("common.of")} {dealsOverTime.length}
                    </span>
                    <div className="flex gap-1">
                      ...
                      <button
                        onClick={() =>
                          setSalesPage((p) =>
                            Math.min(totalDealsPages - 1, p + 1)
                          )
                        }
          ...
          {loadingFunnel ? (
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div className="kpi-card">
                  <p className="text-xs text-[var(--crm-text-muted)]">
                    {t("reports.pipelineValue")}
                  </p>
                  <p className="text-2xl font-bold text-[#10B981] mt-1">
                    {formatShortCurrency(stats?.pipelineValue || 0)}
                  </p>
                </div>
                <div className="kpi-card">
                  <p className="text-xs text-[var(--crm-text-muted)]">
                    {t("analytics.conversionRate" as any)}
                  </p>
                  <p className="text-2xl font-bold text-[#3B82F6] mt-1">
              ...
              <div className="glass-card-static overflow-hidden">
                <div className="p-4 pb-0">
                  <h3 className="text-sm font-medium text-[var(--crm-text-primary)]">
                    {t("reports.conversionByStage" as any)}
                  </h3>
                </div>
                <Table>
                  <TableHeader>
          ...
          {loadingActivities ? (
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div className="h-72">
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        ...
                        <XAxis
                          dataKey="type"
                          tick={{ fill: "var(--crm-text-muted)", fontSize: 10 }}
                          angle={-30}
                          textAnchor="end"
                          height={60}
                        />
                            ...
                            return (
                              <div className="bg-[var(--crm-bg-hover)] border border-[var(--crm-border)] rounded-md px-3 py-2 shadow-lg">
                                ...
                                <p className="text-xs text-[var(--crm-text-primary)]">
                                  {t("reports.quantity")}: {payload[0].value}
                                </p>
                              </div>
                            )
                          }}
                ...
                <div className="chart-container">
                  <h3 className="text-sm font-medium text-[var(--crm-text-primary)] mb-4">
                    {t("reports.activityDistribution" as any)}
                  </h3>
                  <div className="h-72">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie
                          data={activityBreakdown.filter((d) => d.value > 0)}
                          dataKey="value"
                          nameKey="type"
                          cx="50%"
                          cy="50%"
              ...
              <div className="glass-card-static overflow-hidden">
                <div className="p-4 pb-0">
                  <h3 className="text-sm font-medium text-[var(--crm-text-primary)]">
                    {t("reports.activitySummary" as any)}
                  </h3>
                </div>
                <Table>
                  <TableHeader>
                    <TableRow className="border-[var(--crm-border)] hover:bg-transparent">
                      ...
                      <TableHead className="text-[var(--crm-text-secondary)] text-right">
                        {t("reports.percentage" as any)}
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {activityBreakdown.map((item) => {
                      const total = activityBreakdown.reduce(
                        (sum, a) => sum + a.value,
                        0
                      )
                      const pct =
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell className="text-right text-[var(--crm-text-muted)]">
                            {pct}%
                          </TableCell>
                        </TableRow>
                      )
