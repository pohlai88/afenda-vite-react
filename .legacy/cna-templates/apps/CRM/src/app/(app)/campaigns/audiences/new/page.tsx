...
export default function NewAudiencePage() {
  ...
  const hasValidRules = rules.groups.some((g) =>
    g.rules.some((r) => r.field && r.operator)
  )
  const canSubmit =
    name &&
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-2xl">
          ...
          >
            <div className="flex items-center gap-2 mb-1">
              <Users
                className={`w-4 h-4 ${type === "STATIC" ? "text-blue-400" : "text-[var(--crm-text-muted)]"}`}
              />
              <span className="text-sm font-medium text-[var(--crm-text-primary)]">
                {t("audiences.typeStatic")}
              </span>
            </div>
            <p className="text-xs text-[var(--crm-text-muted)]">
              {t("audiences.typeStaticDesc")}
            </p>
          </button>
          <button
          ...
          >
            <div className="flex items-center gap-2 mb-1">
              <Filter
                className={`w-4 h-4 ${type === "DYNAMIC" ? "text-purple-400" : "text-[var(--crm-text-muted)]"}`}
              />
              <span className="text-sm font-medium text-[var(--crm-text-primary)]">
                {t("audiences.typeDynamic")}
              </span>
            </div>
            <p className="text-xs text-[var(--crm-text-muted)]">
              {t("audiences.typeDynamicDesc")}
            </p>
          </button>
        </div>
      </div>
        ...
        <div className="glass-card-static p-3 mt-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-medium text-[var(--crm-text-primary)]">
              {t("audiences.selectContacts", { n: selectedIds.size })}
            </h3>
            <Input
              value={search}
