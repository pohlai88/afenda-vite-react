      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-36 animate-pulse rounded-lg bg-[var(--crm-bg-subtle)]"
            />
          ))}
        </div>
          ...
          {templates.map((tpl: any) => (
            ...
            >
              ...
              <div className="flex items-center justify-between mt-auto">
                <div className="flex items-center gap-2">
                  <span
                    className={`text-[10px] px-1.5 py-0.5 rounded ${CATEGORY_COLORS[tpl.category || "campaign"] || CATEGORY_COLORS.campaign}`}
                  >
                    {t(getCategoryKey(tpl.category))}
                  </span>
                  <span className="text-[10px] text-[var(--crm-text-muted)]">
                    {t("templates.lastUpdated", {
                      time: formatDistanceToNow(new Date(tpl.updatedAt), {
                        locale: vi,
                        addSuffix: false,
                      }),
                    })}
