...
describe("CSV Importer — Pure Logic", () => {
  ...
  describe("autoMapColumns() — contacts", () => {
    it("maps standard English headers", () => {
      ...
      expect(mappings.find((m) => m.csvColumn === "FirstName")?.crmField).toBe(
        "firstName"
      )
      expect(mappings.find((m) => m.csvColumn === "LastName")?.crmField).toBe(
        "lastName"
      )
      expect(mappings.find((m) => m.csvColumn === "Email")?.crmField).toBe(
        "email"
      )
      expect(mappings.find((m) => m.csvColumn === "Phone")?.crmField).toBe(
        "phone"
      )
    })
    ...
    it("maps Vietnamese headers", () => {
      ...
      const mappings = autoMapColumns(headers, "contacts")
      expect(mappings.find((m) => m.csvColumn === "Họ")?.crmField).toBe(
        "lastName"
      )
      expect(mappings.find((m) => m.csvColumn === "Tên")?.crmField).toBe(
        "firstName"
      )
      expect(mappings.find((m) => m.csvColumn === "Điện thoại")?.crmField).toBe(
        "phone"
      )
      expect(mappings.find((m) => m.csvColumn === "Công ty")?.crmField).toBe(
        "companyName"
      )
    })
    ...
    it("maps Vietnamese without diacritics", () => {
      ...
      const mappings = autoMapColumns(headers, "contacts")
      expect(mappings.find((m) => m.csvColumn === "ho")?.crmField).toBe(
        "lastName"
      )
      expect(mappings.find((m) => m.csvColumn === "ten")?.crmField).toBe(
        "firstName"
      )
      expect(mappings.find((m) => m.csvColumn === "dien thoai")?.crmField).toBe(
        "phone"
      )
      expect(mappings.find((m) => m.csvColumn === "cong ty")?.crmField).toBe(
        "companyName"
      )
    })
    ...
    it("maps underscore and space variants", () => {
      ...
      const mappings = autoMapColumns(headers, "contacts")
      expect(mappings.find((m) => m.csvColumn === "first_name")?.crmField).toBe(
        "firstName"
      )
      expect(mappings.find((m) => m.csvColumn === "last_name")?.crmField).toBe(
        "lastName"
      )
      expect(mappings.find((m) => m.csvColumn === "job_title")?.crmField).toBe(
        "jobTitle"
      )
      expect(mappings.find((m) => m.csvColumn === "lead_score")?.crmField).toBe(
        "score"
      )
      expect(
        mappings.find((m) => m.csvColumn === "phone_number")?.crmField
      ).toBe("phone")
    })
  ...
  describe("autoMapColumns() — companies", () => {
    it("maps standard company headers", () => {
      ...
      expect(mappings.find((m) => m.csvColumn === "Name")?.crmField).toBe(
        "name"
      )
      expect(mappings.find((m) => m.csvColumn === "Domain")?.crmField).toBe(
        "domain"
      )
      expect(mappings.find((m) => m.csvColumn === "Industry")?.crmField).toBe(
        "industry"
      )
      expect(mappings.find((m) => m.csvColumn === "Phone")?.crmField).toBe(
        "phone"
    ...
    it("maps Vietnamese company headers", () => {
      ...
      const mappings = autoMapColumns(headers, "companies")
      expect(
        mappings.find((m) => m.csvColumn === "Tên công ty")?.crmField
      ).toBe("name")
      expect(mappings.find((m) => m.csvColumn === "Ngành nghề")?.crmField).toBe(
        "industry"
      )
      expect(mappings.find((m) => m.csvColumn === "Địa chỉ")?.crmField).toBe(
        "address"
      )
      expect(mappings.find((m) => m.csvColumn === "Mã số thuế")?.crmField).toBe(
        "taxCode"
      )
    })
    ...
    it("maps company_name and tax_code variants", () => {
      ...
      expect(
        mappings.find((m) => m.csvColumn === "company_name")?.crmField
      ).toBe("name")
      expect(mappings.find((m) => m.csvColumn === "tax_code")?.crmField).toBe(
        "taxCode"
      )
    })
    ...
    it("maps Vietnamese location fields", () => {
      ...
      const mappings = autoMapColumns(headers, "companies")
      expect(mappings.find((m) => m.csvColumn === "Thành phố")?.crmField).toBe(
        "city"
      )
      expect(mappings.find((m) => m.csvColumn === "Tỉnh")?.crmField).toBe(
        "province"
      )
      expect(mappings.find((m) => m.csvColumn === "Quốc gia")?.crmField).toBe(
        "country"
      )
    })
  })
