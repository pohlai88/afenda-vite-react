  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
      {cards.map((card) => {
        ...
        const isGood =
          "invertColor" in card && card.invertColor ? !isPositive : isPositive
        return (
            ...
            <div className="flex items-start justify-between">
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-1">
                  {isPositive ? (
                    <TrendingUp
                      className={`w-3.5 h-3.5 ${isGood ? "text-[var(--crm-accent-text)]" : "text-red-400"}`}
                    />
                  ) : (
                    <TrendingDown
                      className={`w-3.5 h-3.5 ${isGood ? "text-[var(--crm-accent-text)]" : "text-red-400"}`}
              ...
              <div
                className={`p-2.5 rounded-lg ${card.iconBg} ring-1 ring-[var(--crm-border)]`}
              >
