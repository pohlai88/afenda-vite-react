...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="rounded-lg border border-[var(--crm-border)] bg-[var(--crm-bg-card)]/95 backdrop-blur-xl px-3 py-2 shadow-xl">
      <p className="text-sm font-medium text-[var(--crm-text-primary)] mb-1">
        {label}
      </p>
      {payload.map((p: any) => (
        ...
        {!mounted ? null : (
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Bar
                dataKey="opened"
                name={t("analytics.opened" as any)}
                fill="#10B981"
                fillOpacity={0.85}
                radius={[2, 2, 0, 0]}
                maxBarSize={20}
              ...
              <Bar
                dataKey="clicked"
                name={t("analytics.clicked" as any)}
                fill="#F59E0B"
                fillOpacity={0.85}
                radius={[2, 2, 0, 0]}
                maxBarSize={20}
