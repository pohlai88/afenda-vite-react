...
test.describe("Companies CRUD", () => {
  test("list companies shows seed data", async ({ adminPage }) => {
    ...
    await expect(
      adminPage.getByRole("heading", { name: "Công ty" })
    ).toBeVisible({ timeout: 10_000 })

    // Seed data has [TEST] companies — displayed as cards
    await expect(adminPage.locator(".glass-card-static").first()).toBeVisible({
      timeout: 10_000,
    })
  })
  ...
  test("create company", async ({ adminPage }) => {
    ...
    await expect(
      adminPage.getByRole("heading", { name: "Thêm công ty mới" })
    ).toBeVisible({ timeout: 10_000 })

    // Fill company name (required field, placeholder "Công ty ABC")
    await adminPage
      .getByPlaceholder("Công ty ABC")
      .fill(`[E2E] Công ty Test ${Date.now()}`)
    await adminPage
      .getByPlaceholder("contact@abc.com")
      .fill(`e2e-company-${Date.now()}@test.rtr.com`)
  ...
  test("company detail shows contacts tab", async ({ adminPage }) => {
    ...
    await expect(adminPage).toHaveURL(/.*companies\/[a-z0-9-]+$/, {
      timeout: 10_000,
    })
