...
test.describe("Deals", () => {
  test("pipeline view shows stages", async ({ adminPage }) => {
    ...
    await expect(
      adminPage.getByRole("heading", { name: "Pipeline" })
    ).toBeVisible({ timeout: 10_000 })

    // Wait for kanban board to render — stages appear as columns with kanban-column class
    await expect(adminPage.locator(".kanban-column").first()).toBeVisible({
      timeout: 15_000,
    })
  })
  ...
  test("create deal", async ({ adminPage }) => {
    ...
    await expect(adminPage.getByText("Tạo deal mới")).toBeVisible({
      timeout: 10_000,
    })

    // Wait for pipeline data to load — stages populate the combobox
    const stageCombobox = adminPage
      .locator("text=Giai đoạn")
      .locator("..")
    ...
    if (response) {
      ...
      const toastText = await adminPage
        ...
        .catch(() => "")
      throw new Error(
        `Deal creation failed — no API call made. Toast: ${toastText}`
      )
    }
  })
  ...
  test("deal detail page shows info", async ({ adminPage }) => {
    ...
    await expect(adminPage).toHaveURL(/.*pipeline\/[a-z0-9-]+$/, {
      timeout: 10_000,
    })

    // Verify detail page has back button
    await expect(adminPage.getByText("Quay lại")).toBeVisible({
      timeout: 5_000,
    })
  })
})
