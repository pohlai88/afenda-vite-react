...
test.describe("Contacts CRUD", () => {
  test("list contacts shows seed data", async ({ adminPage }) => {
    ...
    await expect(
      adminPage.getByRole("heading", { name: "Liên hệ" })
  ...
  test("create new contact", async ({ adminPage }) => {
    ...
    await expect(
      adminPage.getByRole("heading", { name: "Thêm liên hệ mới" })
    ).toBeVisible({ timeout: 10_000 })
    ...
    await adminPage
      ...
      .fill(`e2e-create-${Date.now()}@test.rtr.com`)
  ...
  test("create contact with validation errors", async ({ adminPage }) => {
    ...
    await expect(adminPage.locator(".border-red-500").first()).toBeVisible({
      timeout: 5_000,
    })
  })
  ...
  test("contact detail page shows info", async ({ adminPage }) => {
    ...
    await expect(adminPage).toHaveURL(/.*contacts\/[a-z0-9-]+$/, {
      timeout: 10_000,
    })
  ...
  test("delete contact removes it from list", async ({ adminPage }) => {
    ...
    await adminPage
      .getByPlaceholder("email@example.com")
      .fill(`e2e-delete-${Date.now()}@test.rtr.com`)
    ...
    await expect(adminPage.getByText(uniqueName)).toBeHidden({
      timeout: 10_000,
    })
  })
})
