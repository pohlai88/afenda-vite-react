...
test.describe("Customer Portal", () => {
  ...
  test("portal dashboard shows stats cards", async ({ page }) => {
    ...
    await expect(page.getByText("Xem trạng thái đơn hàng")).toBeVisible({
      timeout: 5_000,
    })
  ...
  test("portal tickets page and create ticket", async ({ page }) => {
    ...
    await page
      ...
      .fill("[E2E] Hỏi về báo giá")
    await page
      .getByPlaceholder("Mô tả chi tiết")
      .fill("[E2E] Tôi cần thêm thông tin về báo giá")

    // Submit
    const responsePromise = page
      .waitForResponse(
        (resp) =>
          resp.url().includes("/api/portal/tickets") &&
    ...
    await expect(page.getByText("[E2E] Hỏi về báo giá")).toBeVisible({
      timeout: 10_000,
    })
  })
  ...
  test("portal ticket detail shows messages and reply", async ({ page }) => {
    ...
    const hasTicket = await ticketLink
    ...
    await expect(page.getByText(/muốn hỏi về đơn hàng/)).toBeVisible({
      timeout: 5_000,
    })
  ...
  test("portal profile shows user info", async ({ page }) => {
    ...
    await expect(page.getByText("portal-test@test.rtr.com")).toBeVisible({
      timeout: 5_000,
    })
