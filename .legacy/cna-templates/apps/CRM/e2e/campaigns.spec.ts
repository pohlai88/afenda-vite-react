...
test.describe("Campaign Flow", () => {
  test("campaign list page shows seed data", async ({ adminPage }) => {
    ...
    await expect(
      adminPage.getByRole("heading", { name: "Chiến dịch" })
  ...
  test("create campaign via wizard", async ({ adminPage }) => {
    ...
    await expect(
      adminPage.getByRole("heading", { name: "Tạo chiến dịch" })
    ).toBeVisible({ timeout: 10_000 })

    // Step 0 — Thông tin: fill name and subject
    await adminPage
      .getByPlaceholder(/Khuyến mãi tháng/)
      .fill("[E2E] Chiến dịch Test")
    await adminPage
      .getByPlaceholder(/ưu đãi đặc biệt/)
      .fill("[E2E] Ưu đãi dành cho bạn")
    ...
    const hasCombobox = await audienceCombobox
    ...
    await expect(
      adminPage.getByRole("heading", { name: "Tổng quan" })
    ).toBeVisible({ timeout: 5_000 })

    // Click "Tạo chiến dịch"
    const createResponsePromise = adminPage
      .waitForResponse(
        (resp) =>
    ...
    if (response) {
      ...
      if (status >= 400) {
        ...
        throw new Error(
          `Campaign API returned ${status}: ${JSON.stringify(body)}`
        )
      }
      // Should redirect to campaign detail
      await expect(adminPage).toHaveURL(/.*campaigns\/[a-z0-9]+/, {
        timeout: 10_000,
      })
    } else {
  ...
  test("campaign detail shows info", async ({ adminPage }) => {
    ...
    const campaignCard = adminPage
      .locator(
        '.glass-card-static.cursor-pointer, .glass-card-static[class*="cursor-pointer"]'
      )
      .first()
    await expect(campaignCard).toBeVisible({ timeout: 10_000 })
