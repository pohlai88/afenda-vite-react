...
test.describe("Settings", () => {
  test("settings page loads with tabs for admin", async ({ adminPage }) => {
    ...
    await expect(
      adminPage.getByRole("heading", { name: "Cài đặt" })
    ...
    await expect(
      adminPage.getByRole("tab", { name: /Thông báo/ })
    ).toBeVisible()
  ...
  test("save company settings persists values", async ({ adminPage }) => {
    ...
    const nameInput = adminPage
      .getByRole("tabpanel", { name: "Công ty" })
      .locator("input")
      .first()
    ...
    // Click save
    const saveResponsePromise = adminPage
      .waitForResponse(
        (resp) =>
    ...
    await expect(
      adminPage.getByText("Đã lưu thông tin công ty", { exact: true }).first()
