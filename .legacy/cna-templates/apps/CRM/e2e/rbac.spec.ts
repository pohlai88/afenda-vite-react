...
test.describe("RBAC Enforcement", () => {
  ...
  test("MEMBER does not see settings in sidebar navigation", async ({
    memberPage,
  }) => {
  ...
  test("VIEWER cannot see create buttons on contacts page", async ({
    viewerPage,
  }) => {
    await viewerPage.goto("/contacts")
    // Wait for page to load — viewer may see empty list or some contacts
    await expect(
      viewerPage.getByRole("heading", { name: "Liên hệ" })
    ).toBeVisible({ timeout: 10_000 })

    // Viewer should NOT see "Thêm liên hệ" button
    await expect(
      viewerPage.getByRole("link", { name: "Thêm liên hệ" })
    ).toBeHidden()
  })

  test("VIEWER cannot see create button on pipeline page", async ({
    viewerPage,
  }) => {
    await viewerPage.goto("/pipeline")
    await expect(
      viewerPage.getByRole("heading", { name: "Pipeline" })
    ).toBeVisible({ timeout: 10_000 })

    // Viewer should NOT see "Thêm deal" link/button
    await expect(
      viewerPage.getByRole("link", { name: "Thêm deal" })
    ).toBeHidden()
  })

  test("ADMIN can see save button on settings page", async ({ adminPage }) => {
    await adminPage.goto("/settings")
    await expect(adminPage.getByText("Thông tin công ty")).toBeVisible({
      timeout: 10_000,
    })
    await expect(
      adminPage.getByRole("button", { name: "Lưu thông tin" })
    ).toBeVisible()
  })

  test("MEMBER cannot see save button on settings page", async ({
    memberPage,
  }) => {
    await memberPage.goto("/settings")
    await expect(memberPage.getByText("Thông tin công ty")).toBeVisible({
      timeout: 10_000,
    })
    await expect(
      memberPage.getByRole("button", { name: "Lưu thông tin" })
    ).toBeHidden()
  })
})
