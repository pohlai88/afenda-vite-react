...
test.describe("Quote Flow", () => {
  test("list quotes shows seed data", async ({ adminPage }) => {
    ...
    await expect(
      adminPage.getByRole("heading", { name: "Báo giá" })
    ...
    await expect(adminPage.locator("table tbody tr").first()).toContainText(
      "QUO-"
    )
  })
  ...
  test("create quote with items", async ({ adminPage }) => {
    ...
    await expect(
      adminPage.getByRole("heading", { name: "Tạo báo giá" })
    ).toBeVisible({ timeout: 10_000 })
  ...
  test("quote detail shows items and totals", async ({ adminPage }) => {
    ...
    await expect(adminPage).toHaveURL(/.*quotes\/[a-z0-9]+/, {
      timeout: 10_000,
    })

    // Verify customer info section
    await expect(adminPage.getByText("Thông tin khách hàng")).toBeVisible({
      timeout: 5_000,
    })
  ...
  test("quote detail has PDF export button", async ({ adminPage }) => {
    ...
    const pdfButton = adminPage.getByRole("button", {
      name: /Xuất PDF|Tải PDF/,
    })
    await expect(pdfButton).toBeVisible({ timeout: 5_000 })
    await expect(pdfButton).toBeEnabled()

    // Click PDF button and verify the API is called
    const pdfResponsePromise = adminPage
      .waitForResponse(
        (resp) =>
          resp.url().includes("/pdf") && resp.request().method() === "GET",
        { timeout: 15_000 }
      )
      .catch(() => null)
  ...
  test("send quote dialog opens for draft quote", async ({ adminPage }) => {
    ...
    await expect(
      adminPage.getByText("Email sẽ được gửi kèm file PDF")
