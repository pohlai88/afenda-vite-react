    ...
    if (funcMatch) {
      ...
      if (funcName in aggregateFunctions) {
        ...
        if (range) {
          ...
          const result =
            aggregateFunctions[funcName as AggregateOperation](numbers)
        ...
        const result =
          aggregateFunctions[funcName as AggregateOperation](argValues)
        return { cellKey, value: result, displayValue: formatNumber(result) }
      }
      ...
      switch (funcName) {
        ...
        case "CONCATENATE": {
          const parts = args
            .split(",")
            .map((a) => resolveValue(a.trim(), context))
        ...
        case "POW": {
          const [base, exp] = args
            .split(",")
            .map((a) => Number(resolveValue(a.trim(), context)))
        ...
        case "NOW": {
          ...
          return {
            cellKey,
            value: now.getTime(),
            displayValue: now.toLocaleString(),
          }
        }
        case "TODAY": {
          ...
          return {
            cellKey,
            value: today.getTime(),
            displayValue: today.toLocaleDateString(),
          }
        }
        default:
          return {
...
function evaluateArithmetic(expr: string, context: FormulaContext): number {
  ...
  const processed = expr.replace(/[A-Z]+\d+/gi, (match) => {
    ...
    return typeof val === "number"
      ? String(val)
      : String(parseFloat(String(val)) || 0)
  })
