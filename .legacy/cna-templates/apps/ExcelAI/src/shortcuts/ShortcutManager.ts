...
export class ShortcutManager {
  ...
  customize(shortcutId: string, newKeys: string): boolean {
    ...
    if (conflict) {
      loggers.shortcuts.warn(
        `Shortcut conflict: ${newKeys} is already used by ${conflict.id}`
      )
      return false
    }
  ...
  private handleKeyDown(event: KeyboardEvent) {
    ...
    for (const binding of bindings) {
      ...
      if (binding.shortcut.when && !this.checkCondition(binding.shortcut.when))
        continue
  ...
  private checkCondition(condition: string): boolean {
    switch (condition) {
      case "editing":
        return (
          document.activeElement?.classList.contains("cell-editor") || false
        )
      case "selecting":
  ...
  getAllShortcuts(): Array<
    Shortcut & { currentKeys: string; isCustomized: boolean }
  > {
