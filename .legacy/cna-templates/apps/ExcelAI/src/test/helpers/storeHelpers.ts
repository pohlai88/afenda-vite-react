...
export function createCellsWithFormulas(
  ...
  formulas.forEach(({ row, col, formula, displayValue }) => {
    const key = `${row}:${col}`
    cells[key] = createTestCell(null, {
      formula,
      displayValue: displayValue ?? formula,
    })
  })
...
export function getStoreSnapshot(): ReturnType<
  typeof useWorkbookStore.getState
> {
  return useWorkbookStore.getState()
}

// Type guard for checking if value is a number
export function isNumericCell(
  sheetId: string,
  row: number,
  col: number
): boolean {
