...
const HEADER_PATTERNS: Array<{
  ...
  {
    pattern:
      /\b(price|cost|amount|total|revenue|salary|budget|income|expense|payment)\b/i,
    type: "currency",
    confidence: 0.85,
  ...
  // Measurement
  {
    pattern: /\b(weight|mass)\b/i,
    type: "mass",
    confidence: 0.85,
    reason: "Header suggests mass",
  },
  {
  ...
  infer(header: string, samples: unknown[]): TypeInferenceResult {
    ...
    if (headerResult.confidence >= 0.85) {
      return {
        ...
        confidence: Math.min(
          1,
          headerResult.confidence + samplesResult.confidence * 0.1
        ),
    ...
    if (headerResult.type.id === samplesResult.type.id) {
      return {
        ...
        confidence: Math.min(
          1,
          (headerResult.confidence + samplesResult.confidence) / 1.5
        ),
