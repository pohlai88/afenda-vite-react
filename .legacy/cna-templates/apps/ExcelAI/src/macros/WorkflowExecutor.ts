...
function parseRangeString(rangeStr: string): CellRange | null {
  ...
  return {
    start: {
      ...
      col: colToNum(match[1].toUpperCase()),
    },
...
export class WorkflowExecutor {
  ...
  private async executeLoop(
    ...
    switch (loop.type) {
      ...
      case "for_each":
        const collection =
          (this.variables.get(loop.collection!) as unknown[]) || []
  ...
  > = {
    ...
    paste_range: async (params) => {
      ...
      const mode =
        (params.mode as "all" | "values" | "formulas" | "formatting") || "all"
      store.paste(mode)
    ...
    format_cells: async (params) => {
      ...
      if (params.underline !== undefined)
        format.underline = params.underline as boolean
      ...
      if (params.backgroundColor)
        format.backgroundColor = params.backgroundColor as string
      if (params.textAlign || params.align)
        format.align = (params.textAlign || params.align) as
          | "left"
          | "center"
          | "right"
      if (params.numberFormat)
        format.numberFormat = params.numberFormat as string
    ...
    export_pdf: async (params) => {
      ...
      const filename =
        ...
        `${store.workbookName || "spreadsheet"}.pdf`
    ...
    export_excel: async (params) => {
      ...
      const filename =
        (params.filename as string) || `${workbookName || "spreadsheet"}.xlsx`
    ...
    export_csv: async (params) => {
      ...
      const filename =
        (params.filename as string) || `${workbookName || "spreadsheet"}.csv`
    ...
    send_slack: async (params) => {
      ...
      if (!webhookUrl) {
        ...
        useUIStore
          .getState()
          .showToast(`Slack message: ${message}`, "info", 5000)
    ...
    show_notification: async (params) => {
      const message = params.message as string
      const type =
        (params.type as "success" | "error" | "warning" | "info") || "info"
    ...
    ai_create_chart: async (params) => {
      const workbookStore = useWorkbookStore.getState()
      const chartStore = useChartStore.getState()
      const { activeSheetId, workbookId, sheets, selectionRange } =
        workbookStore
    ...
    ai_analyze: async (_params) => {
      ...
      if (emptyCells > totalCells * 0.1) {
        insights.push(
          `Data completeness: ${completeness}% (${emptyCells} empty cells)`
        )
      }
