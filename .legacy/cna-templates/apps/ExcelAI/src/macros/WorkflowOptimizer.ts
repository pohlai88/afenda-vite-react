...
export class WorkflowOptimizer {
  ...
  private findReorderingSuggestions(
    workflow: Workflow
  ): OptimizationSuggestion[] {
    const suggestions: OptimizationSuggestion[] = []
    ...
    for (const step of workflow.steps) {
      if (
        ["export_pdf", "export_excel", "export_csv"].includes(step.action.type)
      ) {
        hasExport = true
      }
      ...
      if (
        hasExport &&
        ["filter_data", "sort_data", "apply_formula"].includes(step.action.type)
      ) {
        suggestions.push({
          id: crypto.randomUUID(),
          type: "reorder",
