...
export class TriggerManager {
  ...
  private dataWatchers: Map<
    string,
    { macroId: string; range?: string; sheet?: string }
  > = new Map()
  ...
  private getNextWeeklyRun(daysOfWeek: number[], timeOfDay: string): Date {
    ...
    for (const day of sortedDays) {
      if (
        day > currentDay ||
        (day === currentDay && this.isTimeInFuture(hours, minutes))
      ) {
        const next = new Date(now)
  ...
  private rangeOverlaps(range1: string, range2: string): boolean {
    ...
    return (
      range1 === range2 || range1.includes(range2) || range2.includes(range1)
    )
  }
}
