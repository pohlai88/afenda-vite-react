...
export function useOfflineCache(
  ...
  const getCachedCells = useCallback(
    async (workbookId: string, sheetId: string) => {
      if (!cacheRef.current) return []
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (cache.current && options.cacheKey) {
        try {
          const cached = await cache.current.getMetadata<{
            data: T
            timestamp: number
          }>(options.cacheKey)
          if (cached) {
