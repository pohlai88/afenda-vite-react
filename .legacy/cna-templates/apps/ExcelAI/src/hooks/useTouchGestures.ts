...
export function useTouchGestures(
  ...
  const gestureTypeRef = useRef<"none" | "scroll" | "pinch" | "longpress">(
    "none"
  )
  ...
  const setZoom = useCallback(
    (newZoom: number) => {
      const clamped = Math.min(
        MAX_ZOOM,
        Math.max(MIN_ZOOM, Math.round(newZoom))
      )
      if (hasStoreZoom) {
  ...
  const handleTouchMove = useCallback(
    (e: TouchEvent) => {
      ...
      if (touches.length === 1 && gestureTypeRef.current !== "pinch") {
        ...
        if (
          initialDistance > 0 &&
          Math.abs(distance - initialDistance) > pinchThreshold
        ) {
