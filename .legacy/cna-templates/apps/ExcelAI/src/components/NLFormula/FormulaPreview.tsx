    ...
    return (
      <div className="formula-preview formula-preview--error">
        <div className="formula-preview__header">
          <span className="formula-preview__icon formula-preview__icon--error">
            !
          </span>
...
const ConfidenceLabel: React.FC<ConfidenceLabelProps> = ({
  confidence,
  mini = false,
}) => {
  const percent = Math.round(confidence * 100)
  ...
  if (mini) {
    return (
      <span
        className={`confidence-label confidence-label--mini confidence-label--${level}`}
      >
        {percent}%
      </span>
    )
...
const ConfidenceExplanation: React.FC<ConfidenceExplanationProps> = ({
  confidence,
}) => {
  const percent = Math.round(confidence * 100)
  ...
  const getFactors = () => {
    ...
    if (percent >= 85) {
      ...
      factors.push({
        factor: "Some entities may need verification",
        impact: "neutral",
      })
    } else {
      factors.push({ factor: "Intent partially recognized", impact: "neutral" })
      factors.push({
        factor: "Consider rephrasing for better results",
        impact: "negative",
      })
    }
...
const FormulaHighlight: React.FC<FormulaHighlightProps> = ({ formula }) => {
  ...
  const highlightFormula = (f: string) => {
    ...
    const patterns = [
      ...
      {
        regex: /^[A-Z]+\d+(?::[A-Z]+\d+)?/i,
        className: "formula-hl--reference",
      },
  ...
  return (
    <div className="formula-history-panel">
      <div className="formula-history-panel__header">
        ...
        <button onClick={onClose} className="formula-history-panel__close">
          ×
        </button>
      </div>
      <ul className="formula-history-panel__list">
        {history.map((item, i) => (
          ...
          >
            <div className="formula-history-panel__item-input">
              {item.input}
            </div>
            <code className="formula-history-panel__item-formula">
              {item.formula}
            </code>
