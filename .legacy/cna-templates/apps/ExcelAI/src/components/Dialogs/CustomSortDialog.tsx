...
export const CustomSortDialog: React.FC<CustomSortDialogProps> = ({
  onClose,
}) => {
  ...
  const addLevel = () => {
    if (sortLevels.length < 5) {
      setSortLevels([
        ...sortLevels,
        { column: sortLevels.length, direction: "asc" },
      ])
    }
  }
            ...
            {sortLevels.map((level, index) => (
              <div key={index} className="sort-level">
                ...
                <select
                  value={level.column}
                  onChange={(e) =>
                    updateLevel(index, "column", parseInt(e.target.value))
                  }
