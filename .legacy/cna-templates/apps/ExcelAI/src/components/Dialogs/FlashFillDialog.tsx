...
export const FlashFillDialog: React.FC<FlashFillDialogProps> = ({
  onClose,
}) => {
  const {
    activeSheetId,
  ...
  const [flashFillResult, setFlashFillResult] =
    useState<FlashFillResult | null>(null)
  ...
  if (!range || !sheet) {
    return (
      <div className="dialog-overlay" onClick={onClose}>
        <div
          className="dialog flash-fill-dialog"
      ...
      >
        ...
        <div className="dialog-content">
          <div className="flash-fill-intro">
            ...
            <p>
              Flash Fill detects patterns from your examples and applies them
              automatically. Enter a few examples in the target column, then
              click Preview.
            </p>
          </div>
          ...
          <div className="examples-info">
            ...
            {exampleCount === 0 && (
              <span className="hint">
                Enter at least one example in column {colToLetter(targetCol)} to
                get started
              </span>
            )}
          </div>
                ...
                <>
                  ...
                  <div className="pattern-info">
                    <span className="pattern-type">
                      {flashFillResult.pattern?.type}
                    </span>
                    <span className="pattern-desc">
                      {flashFillResult.pattern?.description}
                    </span>
                    <span className="pattern-confidence">
                      {Math.round(
                        (flashFillResult.pattern?.confidence ?? 0) * 100
                      )}
                      % confidence
                    </span>
                  </div>
                        ...
                        {flashFillResult.values.length > 10 && (
                          <tr>
                            <td colSpan={3} className="more-rows">
                              ...and {flashFillResult.values.length - 10} more
                              rows
                            </td>
                          </tr>
                        )}
