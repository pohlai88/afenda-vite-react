...
export const SolverDialog: React.FC<SolverDialogProps> = ({
  isOpen,
  onClose,
}) => {
  const [objectiveCell, setObjectiveCell] = useState("")
  const [goal, setGoal] = useState<"maximize" | "minimize" | "value">(
    "maximize"
  )
  ...
  const updateConstraint = useCallback(
    (id: string, field: keyof ConstraintRow, val: string) => {
      setConstraints((prev) =>
              ...
              {constraints.map((c) => (
                <div key={c.id} className="flex items-center gap-1.5">
                  <input
                    value={c.cellRef}
                    onChange={(e) =>
                      updateConstraint(c.id, "cellRef", e.target.value)
                    }
                    placeholder="Cell"
                  ...
                  <select
                    value={c.operator}
                    onChange={(e) =>
                      updateConstraint(c.id, "operator", e.target.value)
                    }
                  ...
                  <input
                    value={c.value}
                    onChange={(e) =>
                      updateConstraint(c.id, "value", e.target.value)
                    }
                    placeholder="Value"
                    className="w-20 px-2 py-1 text-xs border border-neutral-300 dark:border-neutral-600 rounded bg-white dark:bg-neutral-700"
                  />
