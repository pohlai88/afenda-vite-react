...
const CHART_ICONS: Record<string, React.ReactNode> = {
  ...
  radar: (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <polygon
        points="12,6 17,10 15,17 9,17 7,10"
        fill="currentColor"
        opacity="0.3"
      />
    </svg>
...
export const RecommendationCard: React.FC<RecommendationCardProps> = ({
  ...
  const chartName =
    CHART_NAMES[recommendation.chartType]?.[language] ||
    recommendation.chartType
  const reason =
    language === "vi" ? recommendation.reasonVi : recommendation.reason
  const insight =
    language === "vi" ? recommendation.insightVi : recommendation.insight
