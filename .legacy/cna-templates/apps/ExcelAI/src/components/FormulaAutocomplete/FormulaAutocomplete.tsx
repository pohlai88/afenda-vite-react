...
const FUNCTION_METADATA: Record<
  string,
  { description: string; category: string; syntax: string }
> = {
  // Math
  SUM: {
  ...
  YEAR: {
    description: "Extracts year",
    category: "Date",
    syntax: "YEAR(date)",
  },
  MONTH: {
    description: "Extracts month",
    category: "Date",
    syntax: "MONTH(date)",
  },
...
export function saveRecentFunction(name: string): void {
  try {
    ...
    const updated = [name.toUpperCase(), ...filtered].slice(
      0,
        ...
        {suggestions.map((suggestion, index) => (
          ...
          >
            <div className="autocomplete-icon">
              ...
              {CATEGORY_ICONS[suggestion.category || "Other"] || (
                <Calculator size={14} />
              )}
            </div>
