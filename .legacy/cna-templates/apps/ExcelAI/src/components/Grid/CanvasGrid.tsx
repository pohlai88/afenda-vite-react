...
const formatNumberDisplay = (
  ...
  if (
    numberFormat &&
    (numberFormat.includes("₫") || numberFormat.toLowerCase().includes("vnd"))
  ) {
...
export const CanvasGrid: React.FC<CanvasGridProps> = ({ sheetId }) => {
  ...
  const getCellDisplayValue = useWorkbookStore(
    (state) => state.getCellDisplayValue
  )
  ...
  const colWidths = useMemo(() => {
    ...
    for (let c = 0; c < activeColCount; c++) {
      ...
      widths.push(
        customWidth ? Math.round(customWidth * zoomFactor) : DEFAULT_CELL_WIDTH
      )
    }
    return widths
  ...
  const getColWidth = useCallback(
    (col: number): number => {
      ...
      return customWidth
        ? Math.round(customWidth * zoomFactor)
        : DEFAULT_CELL_WIDTH
    },
  ...
  const setSelectionRange = useSelectionStore(
    (state) => state.setSelectionRange
  )
  ...
  const renderGrid = useCallback(() => {
    ...
    const endRow = Math.min(
      MAX_ROWS,
      startRow + Math.ceil(height / CELL_HEIGHT) + 1
    )
    ...
    const cfRangeStats = new Map<
      string,
      ReturnType<typeof calculateRangeStats>
    >()
    if (allCFRules.length > 0 && sheet?.cells) {
      for (const rule of allCFRules) {
        ...
        const colToNum = (c: string) =>
          c.split("").reduce((acc, ch) => acc * 26 + ch.charCodeAt(0) - 64, 0) -
          1
    ...
    for (let row = startRow; row < endRow; row++) {
      for (let col = startCol; col < endCol; col++) {
        ...
        if (!cellData) {
          ...
          if (
            valRule?.validationType.type === "list" &&
            valRule.validationType.dropdown
          ) {
        ...
        let cfItalic = false
        let cfDataBar:
          | { width: number; isNegative: boolean; color: string }
          | undefined
          ...
          for (const rule of rules) {
            ...
            if (rule.style) {
              if (rule.style.backgroundColor)
                cfBgColor = rule.style.backgroundColor
            ...
            if (rule.type === "dataBar" && rule.dataBar) {
              ...
              if (!isNaN(numVal)) {
                ...
                cfDataBar = {
                  ...bar,
                  color: bar.isNegative
                    ? rule.dataBar.negativeColor
            ...
            if (rule.type === "colorScale" && rule.colorScale) {
              const numVal = parseFloat(String(cellData.value))
              if (!isNaN(numVal)) {
                cfColorScaleColor = calculateColorScaleColor(
                  numVal,
                  rule.colorScale,
                  stats
                )
            ...
            if (rule.type === "iconSet" && rule.iconSet) {
              const numVal = parseFloat(String(cellData.value))
              if (!isNaN(numVal)) {
                const iconDef = ICON_SET_DEFINITIONS.find(
                  (d) => d.id === rule.iconSet!.iconStyle
                )
                if (iconDef) {
                  cfIcon = calculateIconSetIcon(
                    numVal,
                    rule.iconSet,
                    stats,
                    iconDef
        ...
        if (displayValue) {
          ctx.fillStyle =
            cfTextColor || cellData.format?.textColor || colors.text
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      const dropdownOptions = valStoreState.getDropdownOptions(
        sheetId,
        cell.row,
        cell.col
      )
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (
        cell &&
        dragEndRef.current &&
        (cell.row !== dragEndRef.current.row ||
          cell.col !== dragEndRef.current.col)
      ) {
        dragEndRef.current = cell
    ...
    (e: React.MouseEvent) => {
      ...
      if (cell) {
        ...
        if (!canPerformAction(sheetId, "formatCells")) {
          showToast(
            "This sheet is protected. Unprotect it to make changes.",
            "warning"
          )
          return
        }
  ...
  const handleFileDrop = useCallback(
    async (e: React.DragEvent) => {
      ...
      if (
        ...
        showToast(
          "Unsupported file format. Please drop .xlsx, .xls, or .csv files.",
          "warning"
        )
        return
      }
      ...
      try {
        ...
        if (name.endsWith(".csv")) {
          ...
          const updates = Object.entries(result.sheets[0].cells).map(
            ([key, cell]) => {
          ...
          if (result.charts && result.charts.length > 0) {
            ...
            const { populateChartDataFromCells } =
              await import("../../utils/excelIO")
  ...
  const goToCell = useCallback(
    (ref: string) => {
      ...
      if (!match) {
        showToast(
          "Invalid cell reference. Use format like A1, B25, AA100.",
          "warning"
        )
        return
      }
      ...
      if (containerRef.current) {
        ...
        containerRef.current.scrollTop = Math.max(
          0,
          targetY - containerSize.height / 3
        )
        containerRef.current.scrollLeft = Math.max(
          0,
          targetX - containerSize.width / 3
        )
      }
  ...
  const handleColumnHeaderClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      for (const key of Object.keys(sheet.cells)) {
        ...
        if (parseInt(cStr) === col || true) {
          // need all cols in range
    ...
    (value: string) => {
      ...
      if (!result.isValid) {
        ...
        showToast(
          ...
          errorStyle === "warning" ? "warning" : "info"
        )
      }
