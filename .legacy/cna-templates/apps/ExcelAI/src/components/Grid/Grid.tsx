...
export const Grid: React.FC<GridProps> = ({ sheetId }) => {
  ...
  const visibleRows =
    Math.ceil(containerSize.height / CELL_HEIGHT) + BUFFER_ROWS * 2
  const visibleCols =
    Math.ceil(containerSize.width / CELL_WIDTH) + BUFFER_COLS * 2

  const startRow = Math.max(
    0,
    Math.floor(scrollTop / CELL_HEIGHT) - BUFFER_ROWS
  )
  const startCol = Math.max(
    0,
    Math.floor(scrollLeft / CELL_WIDTH) - BUFFER_COLS
  )
