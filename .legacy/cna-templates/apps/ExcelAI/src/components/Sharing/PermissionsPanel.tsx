      ...
      try {
        ...
        if (response.ok) {
          onPermissionsChange(
            permissions.filter(
              (p) =>
                p.granteeType !== perm.granteeType ||
                p.granteeId !== perm.granteeId
            )
          )
        }
      } catch (e) {
...
const PermissionItem: React.FC<PermissionItemProps> = ({
  ...
  const getGranteeDisplay = () => {
    ...
    if (permission.granteeType === "Team")
      return `Team: ${permission.granteeId}`
...
const GeneralAccessControl: React.FC<GeneralAccessControlProps> = ({
  ...
  const [anyoneRole, setAnyoneRole] = useState<WorkbookRole>(
    anyonePermission?.role || "Viewer"
  )
  ...
  const handleAccessChange = async (restricted: boolean) => {
    ...
    if (restricted && anyonePermission) {
      ...
      try {
        ...
        onPermissionsChange(
          permissions.filter((p) => p.granteeType !== "Anyone")
        )
      } catch (e) {
    ...
    <div style={generalAccessStyle}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <div
          style={{
            ...iconContainerStyle,
            backgroundColor: isRestricted ? "#f3f4f6" : "#dbeafe",
          }}
        >
        ...
        <div style={{ flex: 1 }}>
          {canEdit ? (
            <select
              ...
              onChange={(e) =>
                handleAccessChange(e.target.value === "restricted")
              }
              style={{
                ...selectStyle,
                padding: "6px 10px",
