...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const colWidth =
    (containerWidth - padding[0] * 2 - margin[0] * (columns - 1)) / columns
  ...
  const getGridPosition = useCallback(
    (
      ...
      const x = Math.max(
        ...
        Math.min(
          columns - 1,
          Math.round((pixelX - padding[0]) / (colWidth + margin[0]))
        )
      )
      const y = Math.max(
        0,
        Math.round((pixelY - padding[1]) / (rowHeight + margin[1]))
      )
      const w = Math.max(
        1,
        Math.min(
  ...
  const handleDrag = useCallback(
    (e: React.MouseEvent) => {
      ...
      const newGridPos = getGridPosition(
        newPixelX,
        newPixelY,
        pixelPos.width,
        pixelPos.height
      ...
      updateWidgetPositions(dashboard.id, [
        {
          id: draggedWidget,
          position: { ...widget.position, x: newGridPos.x, y: newGridPos.y },
        },
      ])
    },
    [
  ...
  const totalHeight = useMemo(() => {
    ...
    const maxBottom = Math.max(
      ...dashboard.widgets.map(
        (w) => (w.position.y + w.position.h) * rowHeight + padding[1] * 2
      )
    )
