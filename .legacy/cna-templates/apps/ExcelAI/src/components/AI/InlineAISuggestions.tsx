...
const ERROR_PATTERNS: Array<{
  ...
  },
  {
    ...
    fixPrompt:
      "This formula has a #VALUE! error. Analyze the formula and suggest a fix.",
  },
  {
    pattern: /^#NAME\?/,
    errorType: "#NAME?",
  ...
  },
  {
    ...
    fixPrompt:
      "This formula divides by zero. Suggest how to handle this with IFERROR or IF.",
  },
  {
    pattern: /^#N\/A/,
    errorType: "#N/A",
    ...
    fixPrompt:
      "This lookup formula returned #N/A. Suggest how to handle missing values.",
  },
  {
    pattern: /^#NUM!/,
    errorType: "#NUM!",
    ...
    fixPrompt:
      "This formula has a #NUM! error - invalid number. Analyze and suggest a fix.",
  },
]
...
export const InlineAISuggestions: React.FC<InlineAISuggestionsProps> = ({
  ...
  useEffect(() => {
    ...
    if (formula.startsWith("=")) {
      const completionMatch = COMPLETION_PATTERNS.find((p) =>
        p.pattern.test(formula)
      )
      if (completionMatch) {
        newSuggestions.push({
      ...
      if (
        ...
        newSuggestions.push({
          ...
          type: "optimization",
          message:
            "Consider wrapping VLOOKUP with IFERROR to handle missing values",
          action: "Show me how",
      ...
      {suggestions.map((suggestion) => (
        ...
        >
          <div className="inline-suggestion-icon">
            ...
            {suggestion.type === "formula_completion" && (
              <Lightbulb size={12} />
            )}
            {suggestion.type === "validation_warning" && (
              <AlertTriangle size={12} />
            )}
            {suggestion.type === "optimization" && <Sparkles size={12} />}
          </div>
          <span className="inline-suggestion-message">
            {suggestion.message}
          </span>
          {suggestion.action && (
            <button
...
export const FormulaBarAIHint: React.FC<{ formula: string }> = ({
  formula,
}) => {
