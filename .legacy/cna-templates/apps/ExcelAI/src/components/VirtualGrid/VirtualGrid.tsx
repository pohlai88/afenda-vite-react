...
export const VirtualGrid: React.FC<VirtualGridProps> = memo(
  ({
    ...
    // Stores
    const { sheets, getCellDisplayValue, getCellFormula, updateCell } =
      useWorkbookStore()
    const {
      selectedCell,
      selectionRange,
      ...
      async (value: string) => {
        ...
        try {
          ...
          const response = isFormula
            ? await apiClient.setCellFormula(
                workbookId,
                sheetId,
                row,
                col,
          ...
          if (response.success) {
            const cellData = await apiClient.getCell(
              workbookId,
              sheetId,
              row,
              col
          ...
          case "PageUp":
            e.preventDefault()
            {
              const visibleRows = Math.floor(
                containerSize.height / DEFAULT_CELL_HEIGHT
              )
    ...
    const rangeSelectionStyle = useMemo<React.CSSProperties | null>(() => {
      ...
      return {
        ...
        height:
          endVirtualRow.start + endVirtualRow.size - startVirtualRow.start,
