...
export const Timeline: React.FC<TimelineProps> = ({
  ...
  const handlePeriodClick = (period: TimelinePeriod, e: React.MouseEvent) => {
    if (e.shiftKey && timeline.startDate) {
      // Extend selection
      const newStart =
        period.start < timeline.startDate ? period.start : timeline.startDate
      const newEnd =
  ...
  const updatePivotFilter = (start: Date, end: Date) => {
    ...
    if (
      ...
      selectedDates.length === dateRange.dates.length
    ) {
      removeFilter(pivot.id, timeline.fieldId)
    } else {
      setFilter(pivot.id, {
    ...
    >
        ...
        >
          ...
          <div className="timeline-level">
            <select
              ...
              onChange={(e) =>
                handleLevelChange(e.target.value as TimelineLevel)
              }
      ...
      <div className="timeline-body">
        ...
        <div className="timeline-periods">
          {periods
            .slice(scrollOffset, scrollOffset + 10)
            .map((period, index) => (
              <button
                key={index}
