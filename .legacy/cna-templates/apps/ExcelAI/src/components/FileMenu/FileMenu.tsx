...
const useRecentFiles = () => {
  ...
  const addRecentFile = useCallback(
    (file: Omit<RecentFile, "id" | "lastOpened">) => {
      setRecentFiles((prev) => {
        ...
        try {
          ...
          localStorage.setItem(
            "excelai-recent-files",
            JSON.stringify(reducedFiles)
          )
        }
        return updated
      })
...
const NewSubmenu: React.FC<{ onSelect: (type: string) => void }> = ({
  onSelect,
}) => (
  <div className="file-menu__submenu">
    <button
      className="file-menu__submenu-item"
      onClick={() => onSelect("blank")}
    >
      <FileSpreadsheet size={18} />
      <div>
        <span className="file-menu__submenu-title">Blank Workbook</span>
        <span className="file-menu__submenu-desc">
    ...
    >
      ...
      <div>
        ...
        <span className="file-menu__submenu-desc">
          Import from CSV, JSON, or other sources
        </span>
      </div>
    </button>
  </div>
...
const SaveAsSubmenu: React.FC<{ onSelect: (format: string) => void }> = ({
  onSelect,
}) => (
  <div className="file-menu__submenu">
    <button
...
const PrintSubmenu: React.FC<{ onSelect: (action: string) => void }> = ({
  onSelect,
}) => (
  <div className="file-menu__submenu">
    <button
...
const EncryptSubmenu: React.FC<{ onSelect: (action: string) => void }> = ({
  onSelect,
}) => (
  <div className="file-menu__submenu">
    <button
...
const BackupSubmenu: React.FC<{ onSelect: (action: string) => void }> = ({
  onSelect,
}) => (
  <div className="file-menu__submenu">
    <button
...
const HelpSubmenu: React.FC<{ onSelect: (action: string) => void }> = ({
  onSelect,
}) => (
  <div className="file-menu__submenu">
    <button
...
export const FileMenu: React.FC<FileMenuProps> = ({ isOpen, onClose }) => {
  ...
  const handleSendEmail = useCallback(() => {
    const subject = encodeURIComponent(
      `${workbookName || "Spreadsheet"} - ExcelAI`
    )
  ...
  const handleFileOpen = useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      loggers.ui.debug(
        "[FileMenu] handleFileOpen called:",
        fileName,
        file.size,
        "bytes"
      )
        ...
        if (fileName.endsWith(".xlsx") || fileName.endsWith(".xls")) {
          ...
          if (result.charts && result.charts.length > 0) {
            ...
            for (const ic of result.charts) {
              const chartSheetId =
                importedSheetIds[ic.sheetIndex] || importedSheetIds[0]
  ...
  const renderSubmenu = () => {
    switch (activeMenu) {
                    ...
                    onClick={() => {
                      ...
                      if (file.workbookData) {
                        try {
                          ...
                          alert(
                            "Could not load file. Please open it again from disk."
                          )
                        }
                      } else {
      ...
      case "print":
        return (
          <PrintSubmenu
            onSelect={(action) => handleMenuAction("print", action)}
          />
        )
      case "encrypt":
        return (
          <EncryptSubmenu
            onSelect={(action) => handleMenuAction("encrypt", action)}
          />
        )
      case "backup":
        return (
          <BackupSubmenu
            onSelect={(action) => handleMenuAction("backup", action)}
          />
        )
      case "help":
        return (
          <HelpSubmenu
            onSelect={(action) => handleMenuAction("help", action)}
          />
        )
      default:
        return null
