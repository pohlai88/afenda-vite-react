...
export const DataBarsPreview: React.FC<DataBarsPreviewProps> = ({
  onSelect,
}) => {
  const { addRule } = useConditionalFormattingStore()
  const { selectionRange } = useSelectionStore()

  const gradientPresets = DATA_BAR_PRESETS.filter(
    (p) => p.config.dataBar?.fillType === "gradient"
  )
  const solidPresets = DATA_BAR_PRESETS.filter(
    (p) => p.config.dataBar?.fillType === "solid"
  )
  ...
  return (
    <div className="data-bars-preview">
      ...
      <div className="preview-footer">
        <button className="more-options-btn">More Data Bars...</button>
      </div>
    </div>
  )
}
