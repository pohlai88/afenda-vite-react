...
describe("Financial Functions", () => {
  ...
  const calc = (formula: string) =>
    engine.calculate(formula, "sheet1", 0, 0, mockDataProvider)
  ...
  describe("IPMT", () => {
    ...
    it("should decrease interest over time", () => {
      ...
      expect(Math.abs(first.value as number)).toBeGreaterThan(
        Math.abs(last.value as number)
      )
    })
  })
  ...
  describe("PPMT", () => {
    ...
    it("should increase principal over time", () => {
      ...
      expect(Math.abs(last.value as number)).toBeGreaterThan(
        Math.abs(first.value as number)
      )
    })
  ...
  describe("XNPV", () => {
    it("should calculate NPV with dates", () => {
      const result = calc(
        "=XNPV(0.1,{-100,50,60},{DATE(2024,1,1),DATE(2024,6,1),DATE(2025,1,1)})"
      )
      expect(result.error).toBeUndefined()
    })
  })
  ...
  describe("XIRR", () => {
    it("should calculate IRR with dates", () => {
      const result = calc(
        "=XIRR({-100,50,60},{DATE(2024,1,1),DATE(2024,6,1),DATE(2025,1,1)})"
      )
      expect(result.error).toBeUndefined()
    })
  })
  ...
  describe("PRICE", () => {
    it("should calculate bond price", () => {
      ...
      const result = calc(
        "=PRICE(DATE(2024,1,1),DATE(2034,1,1),0.05,0.06,100,2)"
      )
      expect(result.error).toBeUndefined()
      expect(typeof result.value).toBe("number")
    })
  ...
  describe("ACCRINT", () => {
    it("should calculate accrued interest", () => {
      const result = calc(
        "=ACCRINT(DATE(2024,1,1),DATE(2024,7,1),DATE(2024,3,1),0.05,1000,2)"
      )
      expect(result.error).toBeUndefined()
    })
  })
