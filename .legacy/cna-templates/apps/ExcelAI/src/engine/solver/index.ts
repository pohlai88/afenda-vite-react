...
export function solve(params: SolverParams): SolverResult {
  ...
  const eval_ = (v: number[]) =>
    goal === "maximize" ? -objective(v) : objective(v)
  ...
  const clamp = (v: number[]): number[] => {
    if (!bounds) return v
    return v.map((val, i) =>
      Math.max(bounds[i][0], Math.min(bounds[i][1], val))
    )
  }
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    if (
      (goal === "minimize" && realObj < bestObj) ||
      (goal === "maximize" && realObj > bestObj)
    ) {
      bestObj = realObj
      bestVars = [...vars]
    }
