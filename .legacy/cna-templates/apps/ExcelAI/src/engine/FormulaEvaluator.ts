...
export class FormulaEvaluator {
  ...
  private evalFunctionCall(
    ...
    if (node.args.length > fnDef.maxArgs) {
      return new FormulaError(
        "#VALUE!",
        `${node.name} accepts at most ${fnDef.maxArgs} arguments`
      )
    }
  ...
  private evalRefFunction(
    ...
    switch (node.name) {
      ...
      case "ADDRESS": {
        ...
        const sheetText = node.args[4]
          ? this.evalNode(node.args[4], context)
          : ""
