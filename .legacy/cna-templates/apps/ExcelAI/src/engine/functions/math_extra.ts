...
export const mathExtraFunctions: FunctionDef[] = [
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      for (let i = 0; i < roman.length; i++) {
        ...
        if (current === undefined) {
          return new FormulaError(
            "#VALUE!",
            `Invalid Roman numeral character: ${roman[i]}`
          )
        }
  ...
  // RANDARRAY - Returns an array of random numbers
  {
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      if (r < 1 || c < 1)
        return new FormulaError("#VALUE!", "Rows and cols must be >= 1")
  ...
  // ACOTH - Inverse hyperbolic cotangent
  {
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      const n = num as number
      if (Math.abs(n) <= 1)
        return new FormulaError("#NUM!", "Absolute value must be > 1")
