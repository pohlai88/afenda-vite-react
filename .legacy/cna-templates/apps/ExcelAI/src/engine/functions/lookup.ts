...
export const lookupFunctions: FunctionDef[] = [
  // VLOOKUP - vertical lookup
  {
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      const rangeLookup =
        args[3] !== undefined ? args[3] !== false && args[3] !== 0 : true
      ...
      if (
        (colIndex as number) < 1 ||
        (colIndex as number) > tableArray[0].length
      ) {
  ...
  // HLOOKUP - horizontal lookup
  {
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      if (
        (rowIndex as number) < 1 ||
        (rowIndex as number) > tableArray.length
      ) {
  ...
  // OFFSET - reference offset
  {
    ...
    fn: (_args: FormulaValue[]): FormulaValue => {
      ...
      return new FormulaError(
        "#VALUE!",
        "OFFSET requires cell reference context"
      )
    },
  },
      ...
      if (matchMode === 0) {
        ...
        for (let i = 0; i < lookupArray.length; i++) {
          ...
          if ((matchMode as number) === -1 && cmp < 0) {
            if (
              bestMatch === -1 ||
              compareValues(lookupArray[i], lookupArray[bestMatch]) > 0
            ) {
              bestMatch = i
            }
          }
