...
export const statisticalFunctions: FunctionDef[] = [
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      if (!Array.isArray(avgRange)) {
        return new FormulaError(
          "#VALUE!",
          "AVERAGEIFS requires an average range"
        )
      }
  ...
  // AVEDEV - average absolute deviation
  {
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      const mean = numbers.reduce((a, b) => a + b, 0) / numbers.length
      const sumDeviations = numbers.reduce(
        (sum, n) => sum + Math.abs(n - mean),
        0
      )
  ...
  // T.DIST - Student's t-distribution
  {
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      if (cumulative === true || cumulative === "TRUE") {
        ...
        const coef =
          gamma((dfVal + 1) / 2) /
          (Math.sqrt(dfVal * Math.PI) * gamma(dfVal / 2))
  ...
  // GAMMA.DIST - Gamma distribution
  {
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      if (xVal === 0)
        return cumulative === true || cumulative === "TRUE" ? 0 : 0
      ...
      // PDF
      return (
        (Math.pow(xVal, a - 1) * Math.exp(-xVal / b)) /
        (Math.pow(b, a) * gamma(a))
      )
    },
  },
  ...
  // WEIBULL.DIST - Weibull distribution
  {
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      return (
        (a / b) * Math.pow(xVal / b, a - 1) * Math.exp(-Math.pow(xVal / b, a))
      )
    },
  },
  ...
  // BETA.DIST - Beta distribution
  {
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      if (a <= 0 || b <= 0 || xVal < lo || xVal > hi || lo >= hi)
        return new FormulaError("#NUM!")
      ...
      return (
        (Math.pow(xNorm, a - 1) * Math.pow(1 - xNorm, b - 1)) /
        (betaFunc * (hi - lo))
      )
    },
  },
  ...
  // CHISQ.DIST - Chi-squared distribution
  {
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      const k2 = dfVal / 2
      return (
        (Math.pow(xVal, k2 - 1) * Math.exp(-xVal / 2)) /
        (Math.pow(2, k2) * gamma(k2))
      )
    },
  },
  ...
  // PROB - Returns the probability that values are between limits
  {
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      if (probRange.some((p) => p < 0 || p > 1))
        return new FormulaError("#NUM!")

      let result = 0
      for (let i = 0; i < xRange.length; i++) {
        if (
          xRange[i] >= (lowerLimit as number) &&
          xRange[i] <= (upperLimit as number)
        ) {
          result += probRange[i]
        }
      }
  ...
  // SKEW - Returns the skewness of a distribution
  {
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      const s = Math.sqrt(
        numbers.reduce((sum, v) => sum + Math.pow(v - mean, 2), 0) / (n - 1)
      )
      if (s === 0) return new FormulaError("#DIV/0!")
      const m3 = numbers.reduce(
        (sum, v) => sum + Math.pow((v - mean) / s, 3),
        0
      )
      return (n / ((n - 1) * (n - 2))) * m3
    },
  ...
  // KURT - Returns the kurtosis of a data set
  {
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      if (s === 0) return new FormulaError("#DIV/0!")
      const m4 = numbers.reduce(
        (sum, v) => sum + Math.pow((v - mean) / s, 4),
        0
      )
      return (
