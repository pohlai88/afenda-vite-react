...
function getFieldIndex(
  ...
  if (typeof field === "string") {
    ...
    return new FormulaError(
      "#VALUE!",
      `Field "${field}" not found in database headers`
    )
  }
...
function matchesCriteriaValue(
  ...
  if (typeof criteriaValue === "string") {
    ...
    return (
      String(cellValue).toLowerCase() === String(criteriaValue).toLowerCase()
    )
  }
...
function parseDatabaseArgs(
  ...
  | { database: FormulaValue[][]; fieldIdx: number; criteria: FormulaValue[][] }
  | FormulaError {
  ...
  if (!Array.isArray(criteria) || criteria.length < 1) {
    return new FormulaError(
      "#VALUE!",
      "Criteria must be a 2D array with at least a header row"
    )
  }
  ...
  // DGET - Get single value from database field matching criteria
  {
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      if (rows.length > 1) {
        return new FormulaError(
          "#NUM!",
          "More than one record matches criteria"
        )
      }
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      switch (normalizedId) {
        ...
        case 7: {
          ...
          const var7 =
            numbers.reduce((s, v) => s + Math.pow(v - mean7, 2), 0) /
            (numbers.length - 1)
        ...
        case 8: {
          ...
          const var8 =
            numbers.reduce((s, v) => s + Math.pow(v - mean8, 2), 0) /
            numbers.length
        ...
        case 10: {
          ...
          return (
            numbers.reduce((s, v) => s + Math.pow(v - mean10, 2), 0) /
            (numbers.length - 1)
          )
        }
        case 11: {
          ...
          return (
            numbers.reduce((s, v) => s + Math.pow(v - mean11, 2), 0) /
            numbers.length
          )
        }
        default:
          return new FormulaError(
            "#VALUE!",
            `Invalid SUBTOTAL function_num: ${funcId}`
          )
      }
    },
  },
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      switch (funcId) {
        ...
        default:
          return new FormulaError(
            "#VALUE!",
            `Invalid AGGREGATE function_num: ${funcId}`
          )
      }
    },
  },
