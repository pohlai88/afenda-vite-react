...
export const engineeringFunctions: FunctionDef[] = [
...

  {
    ...
    fn: (args) => {
      ...
      if (!/^[01]{1,10}$/.test(bin))
        throw new FormulaError("#NUM!", "Invalid binary number")
  ...
  },
  {
    ...
    fn: (args) => {
      ...
      if (dec < -512 || dec > 511)
        throw new FormulaError("#NUM!", "Out of range")
      const places = args[1] !== undefined ? ensureNumber(args[1]) : 0
  ...
  },
  {
    ...
    fn: (args) => {
      ...
      if (!/^[0-9A-Fa-f]{1,10}$/.test(hex))
        throw new FormulaError("#NUM!", "Invalid hex")
  ...
  },
  {
    ...
    fn: (args) => {
      ...
      if (!/^[0-7]{1,10}$/.test(oct))
        throw new FormulaError("#NUM!", "Invalid octal")
...

  {
    ...
    fn: (args) => {
      ...
      if (stddev <= 0)
        throw new FormulaError("#NUM!", "Standard deviation must be positive")
  ...
  },
  {
    ...
    fn: (args) => {
      ...
      const stddev = ensureNumber(args[2])
      if (p <= 0 || p >= 1)
        throw new FormulaError("#NUM!", "Probability must be between 0 and 1")
      if (stddev <= 0)
        throw new FormulaError("#NUM!", "Standard deviation must be positive")
  ...
  },
  {
    ...
    fn: (args) => {
      ...
      const k = df / 2
      return (
        (Math.pow(x, k - 1) * Math.exp(-x / 2)) / (Math.pow(2, k) * gamma(k))
      )
    },
  },
  {
    ...
    fn: (args) => {
      ...
      if (!cumulative) {
        ...
        const num =
          Math.pow((d1 * x) / (d1 * x + d2), d1 / 2) *
          Math.pow(d2 / (d1 * x + d2), d2 / 2)
  ...
  },
  {
    ...
    fn: (args) => {
      ...
      // PDF
      return (
        (Math.pow(x, alpha - 1) * Math.exp(-x / b)) /
        (Math.pow(b, alpha) * gamma(alpha))
      )
    },
  },
  {
    ...
    fn: (args) => {
      ...
      return (
        (alpha / b) *
        Math.pow(x / b, alpha - 1) *
        Math.exp(-Math.pow(x / b, alpha))
      )
    },
  },
  {
    ...
    fn: (args) => {
      ...
      if (!cumulative) {
        return (
          (Math.pow(z, alpha - 1) * Math.pow(1 - z, b - 1)) /
          beta(alpha, b) /
          (b_bound - a_bound)
        )
      }
  ...
  },
  {
    ...
    fn: (args) => {
      ...
      return (
        0.5 +
        0.5 * (1 - Math.pow(1 + t2 / df, -(df - 1) / 2)) * (x > 0 ? 1 : -1)
      )
    },
  },
  {
