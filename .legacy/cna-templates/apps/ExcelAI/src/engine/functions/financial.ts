...
export const financialFunctions: FunctionDef[] = [
  ...
  // FV - future value
  {
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      let fv =
        -presentValue * pvif -
        (payment * (1 + r * paymentType) * (pvif - 1)) / r
  ...
  // PV - present value
  {
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      const pvif = Math.pow(1 + r, n)
      const pv =
        (-futureValue - (payment * (1 + r * paymentType) * (pvif - 1)) / r) /
        pvif
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      for (let i = 0; i < maxIterations; i++) {
        ...
        const dy =
          ...
          payment *
            (1 + rate * paymentType) *
            ((n * Math.pow(1 + rate, n - 1) * rate - pvif + 1) /
              (rate * rate)) +
  ...
  // DB - declining balance depreciation
  {
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      // Calculate rate
      const rate =
        Math.round((1 - Math.pow(salvageVal / costVal, 1 / lifeVal)) * 1000) /
        1000
  ...
  // PRICE - bond price
  {
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      const periods = Math.ceil(
        ((maturity as number) - (settlement as number)) / (365 / freq)
      )
