...
export const webFunctions: FunctionDef[] = [
  ...
  // TEXTSPLIT - Splits text by column and/or row delimiters into a 2D array
  {
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      const text = toString(args[0])
      const colDelimiter =
        args[1] !== null && args[1] !== undefined ? toString(args[1]) : null
      const rowDelimiter =
        args.length > 2 && args[2] !== null && args[2] !== undefined
          ? toString(args[2])
          : null
  ...
  // LAMBDA wrapper for reusable formulas: MAKEARRAY
  {
    ...
    fn: (args: FormulaValue[]): FormulaValue => {
      ...
      if (
        lambda &&
        typeof lambda === "object" &&
        "__isLambda" in lambda &&
        lambda.__isLambda
      ) {
        const result: FormulaValue[][] = []
        for (let i = 0; i < r; i++) {
