...
export class FormulaInterpreter {
  ...
  private findBestHeaderMatch(
    ...
    for (const header of headers) {
      ...
      if (
        headerLower.includes(normalizedInput) ||
        normalizedInput.includes(headerLower)
      ) {
  ...
  private buildLookup(
    ...
    const lookupValue = this.resolveValue(
      entities.lookupValue as string,
      context
    )
    ...
    if (entities.lookupColumn && entities.returnColumn) {
      const lookupCol = this.resolveRange(
        entities.lookupColumn as string,
        context
      )
      const returnCol = this.resolveRange(
    ...
    const tableRange = this.resolveRange(
      (entities.tableRange as string) || "A:B",
      context
    )
  ...
  private buildIndexMatch(
    ...
    const returnRange = this.resolveRange(
      entities.returnColumn as string,
      context
    )
    const lookupRange = this.resolveRange(
      entities.lookupColumn as string,
      context
    )
