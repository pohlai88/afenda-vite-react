...
function createMultiColumnData(
  rows: unknown[][],
  types: string[]
): CleanerSheetData {
  return {
...
describe("OutlierDetector", () => {
  ...
  describe("detect", () => {
    ...
    describe("with MAD method", () => {
      it("detects outliers using MAD", () => {
        const madDetector = new OutlierDetector({
          method: "mad",
          zScoreThreshold: 2.5,
        })
  ...
  describe("getBounds", () => {
    it("calculates bounds for z-score method", () => {
      const zscoreDetector = new OutlierDetector({
        method: "zscore",
        zScoreThreshold: 3,
      })
      const stats = {
    ...
    it("replaces outliers with median value", () => {
      ...
      if (results.length > 0) {
        ...
        expect(changes[0].newValue).toBe(
          String(Number(results[0].stats.median.toFixed(2)))
        )
      }
    })
  })
