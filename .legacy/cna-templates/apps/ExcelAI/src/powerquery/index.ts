...
const transforms: Record<TransformType, TransformFn> = {
  ...
  sort: (data, params) => {
    const { column, direction } = params as {
      column: string
      direction: "asc" | "desc"
    }
  ...
  replaceValues: (data, params) => {
    const { column, find, replace } = params as {
      column: string
      find: string
      replace: string
    }
  ...
  pivot: (data, params) => {
    ...
    const allPivotKeys = [
      ...new Set(data.map((r) => String(r[pivotColumn] ?? ""))),
    ]
  ...
  removeDuplicates: (data, params) => {
    ...
    return data.filter((row) => {
      const key = (columns || Object.keys(row))
        .map((c) => String(row[c] ?? ""))
        .join("|")
  ...
  trimText: (data, params) => {
    ...
    return data.map((row) => ({
      ...row,
      [column]:
        typeof row[column] === "string" ? row[column].trim() : row[column],
    }))
  },
...
export function sheetToDataTable(
  cells: Record<
    string,
    { value: string | number | boolean | null; displayValue: string }
  >,
  maxRow: number,
  maxCol: number
): DataTable {
