...
export class FeedbackLoop {
  ...
  getStats(): FeedbackStats {
    ...
    return {
      ...
      negative: this.records.filter(
        (r) =>
          r.feedback.type === "thumbs_down" ||
          (r.feedback.rating && r.feedback.rating <= 2)
      ).length,
      corrections: this.records.filter((r) => r.feedback.type === "correction")
        .length,
      averageRating:
        ratings.length > 0
  ...
  getImprovementSuggestions(): string[] {
    ...
    if (stats.averageRating < 3 && stats.totalFeedback >= 10) {
      suggestions.push(
        "Overall satisfaction is low - review recent conversations"
      )
    }
