...
export class AIRuntime {
  ...
  async *streamMessage(content: string): AsyncGenerator<{
    type: "text" | "tool" | "done"
    content: string | AIToolCall
  }> {
    if (!this.conversation) {
      this.startConversation()
    }
    ...
    for await (const chunk of this.client.streamMessage(
      ...
      if (chunk.type === "text") {
        ...
        const processedTool = await this.processToolCall(
          chunk.content as AIToolCall
        )
...
- [🤔] for inferred conclusions with reasoning`
  ...
  private shouldAutoApprove(
    ...
    if (
      !this.config.autoApprove.riskLevels.includes(
        tool.riskLevel as "low" | "medium"
      )
    ) {
      return false
    }
  ...
  createDirectReadClaim(
    ...
    return this.groundingManager.createDirectReadClaim(
      statement,
      cellRef,
      value,
      sheetName
  ...
  createComputedClaim(
    ...
    return this.groundingManager.createComputedClaim(
      statement,
      formula,
      result,
      sourceCells
  ...
  createInferredClaim(
    ...
    return this.groundingManager.createInferredClaim(
      statement,
      reasoning,
      supportingEvidence
    )
