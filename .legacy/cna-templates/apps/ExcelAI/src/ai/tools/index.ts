...
export const AI_TOOLS: AITool[] = [
  ...
  },
  {
    name: "write_range",
    description:
      "Ghi giá trị hoặc formulas vào cells. Cần approval nếu >10 cells.",
    parameters: [
      {
        name: "range",
        type: "string",
  ...
  },
  {
    ...
    parameters: [
      ...
      },
      {
        ...
        type: "string",
        description:
          '"upstream" (precedents), "downstream" (dependents), or "both"',
        required: false,
        default: "both",
      },
    ],
...
export class AIToolExecutor {
    ...
    switch (tool) {
      case "read_range":
        return this.readRange(
          ...
          (args.includeFormulas as boolean) ?? true
        )
  ...
  private proposeAction(
    ...
    const proposedAction: AIProposedAction = {
      ...
      riskLevel:
        changes.length > 100 ? "high" : changes.length > 10 ? "medium" : "low",
  ...
  private getSheetInfo(): {
    ...
    const formulaCount = Object.values(sheet.cells).filter(
      (c) => c.formula
    ).length
    ...
    const usedRange =
      cellCount > 0 ? `A1:${colToLetter(maxCol)}${maxRow + 1}` : "Empty"
  ...
  private getSelection(): {
    ...
    if (selectionRange) {
      ...
      rangeInfo = {
        ...
        colCount:
          Math.abs(selectionRange.end.col - selectionRange.start.col) + 1,
      }
    }
