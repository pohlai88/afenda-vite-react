  ...
  describe("sandbox creation", () => {
    describe("createSandbox", () => {
      ...
      it("supports intent and reasoning metadata", () => {
        ...
        expect(result.sandbox.metadata.reasoning).toBe(
          "User requested a header row"
        )
      })
    })
    ...
    describe("createSandboxForRangeWrite", () => {
      it("creates sandbox for range write", () => {
        ...
        const result = manager.createSandboxForRangeWrite(
          "sheet1",
          "A1",
          values
        )
    ...
    describe("createSandboxForCellChange", () => {
      it("creates sandbox for single cell change", () => {
        const result = manager.createSandboxForCellChange(
          "sheet1",
          "A1",
          "New Value"
        )
  ...
  describe("sandbox actions", () => {
    describe("approve", () => {
      ...
      it("throws for non-existent sandbox", () => {
        expect(() => manager.approve("non-existent")).toThrow(
          "Sandbox not found"
        )
      })
      ...
      it("updates timestamp", () => {
        ...
        manager.approve(sandbox.id)
        expect(sandbox.updatedAt.getTime()).toBeGreaterThanOrEqual(
          before.getTime()
        )
      })
    })
    ...
    describe("reject", () => {
      ...
      it("throws for non-existent sandbox", () => {
        expect(() => manager.reject("non-existent")).toThrow(
          "Sandbox not found"
        )
      })
    ...
    describe("merge", () => {
      ...
      it("auto-approves low-risk pending sandboxes", () => {
        const changes = new Map<string, CellState>()
        changes.set("sheet1:A1", { ref: "A1", value: "Test", formula: null })
        const { sandbox, riskAssessment } = manager.createSandbox(
          "Test",
          "Test",
          changes
        )
    ...
    describe("discard", () => {
      ...
      it("throws for non-existent sandbox", () => {
        expect(() => manager.discard("non-existent")).toThrow(
          "Sandbox not found"
        )
      })
      ...
      it("throws for merged sandbox", () => {
        ...
        expect(() => manager.discard(sandbox.id)).toThrow(
          "Cannot discard a merged sandbox"
        )
      })
    })
