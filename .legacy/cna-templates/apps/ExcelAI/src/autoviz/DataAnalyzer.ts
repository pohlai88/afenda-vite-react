...
export class DataAnalyzer {
  ...
  private suggestRole(
    ...
    if (
      timeKeywords.some((k) => headerLower.includes(k)) ||
      dataType === "date"
    ) {
      return "time"
    }
  ...
  getSummary(characteristics: DataCharacteristics): string {
    ...
    parts.push(
      `${characteristics.rowCount} rows × ${characteristics.columnCount} columns`
    )
    ...
    const trendPatterns = characteristics.patterns.filter(
      (p) => p.type === "trend"
    )
