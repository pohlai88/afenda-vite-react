...
const CHART_METADATA: ChartMetadata[] = [
  ...
  },
  {
    ...
    pros: [
      "More modern look",
      "Center can show total",
      "Less visual distortion",
    ],
    prosVi: [
      "Giao diện hiện đại",
      "Trung tâm hiển thị tổng",
      "Ít méo thị giác",
    ],
  ...
  },
  {
    ...
    prosVi: [
      "Hiển thị rõ sự giảm",
      "Trực quan cho quy trình",
      "Ưa thích trong marketing",
    ],
...
export class ChartRecommender {
  ...
  recommend(
    ...
    for (const metadata of CHART_METADATA) {
      const factors = this.calculateScoreFactors(
        metadata,
        data,
        characteristics
      )
    ...
    for (let i = 0; i < Math.min(scores.length, 5); i++) {
      ...
      recommendations.push({
        ...
        suggestedConfig: this.createSuggestedConfig(
          type,
          data,
          characteristics
        ),
  ...
  private calculateScoreFactors(
    ...
    factors.insightClarity = this.calculateInsightClarity(
      metadata,
      characteristics
    )
  ...
  private calculateDataFit(
    ...
    if (metadata.requiresTime) {
      ...
      if (
        ["line", "area", "stacked_area", "sparkline"].includes(metadata.type)
      ) {
        score += 5
      }
    }
    ...
    if (
      numericCols >= 2 &&
      ["scatter", "bubble", "heatmap"].includes(metadata.type)
    ) {
      score += 5
    }
  ...
  private calculateInsightClarity(
    ...
    if (characteristics.hasCategoryColumn && !characteristics.hasTimeColumn) {
      if (
        ["bar", "column", "pie", "donut", "treemap"].includes(metadata.type)
      ) {
        score += 5
      }
    }
  ...
  private generateReason(
    ...
    if (
      ...
      ["line", "area"].includes(metadata.type)
    ) {
    ...
    const trendPattern = characteristics.patterns.find(
      (p) => p.type === "trend"
    )
  ...
  private generateInsight(
    ...
    if (insights.length === 0) {
      const cols = characteristics.columns.filter(
  ...
  private generateInsightVi(
    ...
    if (
      cols.length > 0 &&
      cols[0].min !== undefined &&
      cols[0].max !== undefined
    ) {
