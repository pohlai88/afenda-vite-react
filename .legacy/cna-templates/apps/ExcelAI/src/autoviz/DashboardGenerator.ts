...
const TEMPLATES: Record<DashboardTemplate, TemplateConfig> = {
  ...
  marketing: {
    charts: [
      ...
      {
        type: "funnel",
        position: { x: 0, y: 2, w: 5, h: 5 },
        title: "Conversion Funnel",
      },
      {
        type: "line",
        position: { x: 5, y: 2, w: 7, h: 5 },
...
export class DashboardGenerator {
  ...
  suggestTemplate(characteristics: DataCharacteristics): DashboardTemplate {
    ...
    if (
      ...
      characteristics.columns.length > 3
    ) {
      return "comparison"
    }
  ...
  private generateCharts(
    ...
    const chartConfigs = templateConfig.charts.slice(
      0,
      maxCharts || templateConfig.charts.length
    )
  ...
  private generateFilters(
    ...
    const timeCol = characteristics.columns.find(
      (c) => c.suggestedRole === "time"
    )
    if (timeCol) {
      filters.push({
