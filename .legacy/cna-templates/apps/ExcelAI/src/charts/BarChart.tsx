...
export const BarChart: React.FC<BarChartProps> = ({
    ...
    if (stacked) {
      ...
      for (let i = 0; i < barCount; i++) {
        ...
        for (let dsIndex = 0; dsIndex < data.datasets.length; dsIndex++) {
          ...
          const color =
            data.datasets[dsIndex].color || colorScheme.colors[dsIndex]
  ...
  return (
    <ChartWrapper
      config={config}
      className={`bar-chart ${horizontal ? "horizontal" : "vertical"}`}
    >
      <svg
        width={chartWidth}
        height={chartHeight}
              ...
              <rect
                ...
                rx={
                  style.borderRadius > 0
                    ? Math.min(style.borderRadius / 2, 4)
                    : 0
                }
