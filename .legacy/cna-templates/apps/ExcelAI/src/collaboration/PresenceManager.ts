  ...
  private setupPresenceHandlers(): void {
    this.wsClient.on("user_joined", (message) => {
      this.handleUserJoined(
        message.payload as { user: CollaborationUser; sessionId?: string }
      )
    })
    ...
    this.wsClient.on("presence_update", (message) => {
      this.handlePresenceUpdate(
        message.payload as { userId: string; activeSheet?: string }
      )
    })

    this.wsClient.on("cursor_move", (message) => {
      this.handleCursorMove(
        message.payload as { userId: string; position: CursorPosition }
      )
    })
