...
export class IndexedDBCache {
  ...
  private createStores(db: IDBDatabase): void {
    ...
    if (!db.objectStoreNames.contains(STORES.CELLS)) {
      ...
      cellsStore.createIndex("workbook_sheet", ["workbookId", "sheetId"], {
        unique: false,
      })
    }
    ...
    if (!db.objectStoreNames.contains(STORES.FORMULAS)) {
      const formulasStore = db.createObjectStore(STORES.FORMULAS, {
        keyPath: "key",
      })
    ...
    if (!db.objectStoreNames.contains(STORES.FORMATS)) {
      const formatsStore = db.createObjectStore(STORES.FORMATS, {
        keyPath: "key",
      })
  ...
  async getCellsBySheet(
    workbookId: string,
    sheetId: string
  ): Promise<CachedCell[]> {
    return this.getByIndex<CachedCell>(STORES.CELLS, "workbook_sheet", [
      workbookId,
      sheetId,
    ])
  }
