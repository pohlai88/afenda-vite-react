...
const MobileGridOverlay = lazy(() =>
  import("./components/Mobile/MobileGridOverlay").then((m) => ({
    default: m.MobileGridOverlay,
  }))
)
...
const CompetitiveLanding = lazy(() =>
  import("./components/Landing/CompetitiveLanding").then((m) => ({
    default: m.CompetitiveLanding,
  }))
)
...
const FindReplaceDialog = lazy(() =>
  import("./components/FindReplace").then((m) => ({
...
const ProactiveAINotifications = lazy(() =>
  import("./components/AI").then((m) => ({
    default: m.ProactiveAINotifications,
  }))
)
...
function App() {
  ...
  const setBackendAvailableStore = useSyncStore(
  ...
  const { isConnected: _collabConnected, userCount: _collabUserCount } =
    useCollaboration({
  ...
  useEffect(() => {
    ...
    const unsubAudit = useWorkbookStore.subscribe((state, prev) => {
      ...
      for (const sid of newSheets) {
        auditLog.logData(
          "sheet.create",
          `Sheet created: ${state.sheets[sid]?.name}`,
          {
      ...
      if (state.workbookId !== prev.workbookId && state.workbookId) {
        auditLog.logData(
          "workbook.open",
          `Workbook opened: ${state.workbookName}`,
          {
