...
function createMockDOMStringList(items: string[]): DOMStringList {
  ...
  ;(list as unknown as { contains: (name: string) => boolean }).contains = (
    name: string
  ) => items.includes(name)
  ;(list as unknown as { item: (index: number) => string | null }).item = (
    index: number
  ) => items[index] || null
  return list
}
  ...
  const store: MockIDBObjectStore = {
    ...
    add(value: unknown, key?: IDBValidKey) {
      ...
      const actualKey =
        key ||
        (keyPath
          ? (value as Record<string, unknown>)[keyPath as string]
          : undefined)
    ...
    objectStore(name: string) {
      if (!storeNames.includes(name)) {
        throw new DOMException(
          `Object store "${name}" not in transaction scope`
        )
      }
...
// Test helpers
export function getMockDatabaseData(
  dbName: string
): Map<string, MockObjectStore> | undefined {
