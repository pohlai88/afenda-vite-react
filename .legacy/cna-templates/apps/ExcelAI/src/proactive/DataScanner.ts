      ...
      if (header.type === "text") {
        for (let row = 1; row < data.rowCount; row++) {
          ...
          if (
            cell &&
            cell.type === "text" &&
            this.looksLikeDate(String(cell.value))
          ) {
      ...
      if (header.type === "text") {
        for (let row = 1; row < data.rowCount; row++) {
          ...
          if (
            cell &&
            cell.type === "text" &&
            this.looksLikeNumber(String(cell.value))
          ) {
    ...
    for (const header of data.headers) {
      ...
      if (inconsistent.length > 0) {
        ...
        issues.push({
          ...
          confidence: 0.7,
          impact: {
            cellCount: inconsistent.reduce(
              (sum, i) => sum + i.variants.length,
              0
            ),
            severity: "low",
