...
export class ProactiveEngine {
  ...
  async runScan(data: SheetData): Promise<ScanResult> {
    ...
    const [issues, insights, optimizations, patterns] = await Promise.all([
      ...
      this.config.scanInsights
        ? this.insightDetector.detect(data)
        : Promise.resolve([]),
      this.config.scanOptimizations
        ? this.formulaOptimizer.optimize(data)
        : Promise.resolve([]),
      this.config.scanPatterns
        ? this.patternRecognizer.detect(data)
        : Promise.resolve([]),
    ])
  ...
  async executeAction(
    ...
    if (
      result.success &&
      (result.suggestionDismissed || action.type === "primary")
    ) {
