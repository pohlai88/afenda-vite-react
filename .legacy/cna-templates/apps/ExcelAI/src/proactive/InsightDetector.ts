...
export class InsightDetector {
          ...
          actions: [
            {
              ...
              params: {
                type: "scatter",
                columns: [header1.letter, header2.letter],
              },
            },
            {
              id: "analyze",
  ...
  private generateSummaries(data: SheetData): DataInsight[] {
    ...
    if (numericColumns.length > 0) {
      const summaryMetrics: InsightMetric[] = numericColumns
        .slice(0, 3)
        .map((h) => {
          const values: number[] = []
          for (let row = 1; row < data.rowCount; row++) {
  ...
  private calculateStats(values: number[]): {
    ...
    const skewness =
      stdDev === 0
        ? 0
        : values.reduce((acc, v) => acc + Math.pow((v - mean) / stdDev, 3), 0) /
          n
