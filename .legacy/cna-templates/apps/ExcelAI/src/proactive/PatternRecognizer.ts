      ...
      if (seq.frequency >= this.config.patternMinFrequency) {
        patterns.push({
          ...
          automationSuggestion: {
            ...
            steps: seq.actions.map(
              (a, i) => `${i + 1}. ${this.describeAction(a)}`
            ),
  ...
  private detectCopyPastePatterns(): UserPattern[] {
    ...
    if (pasteCount >= 5) {
      patterns.push({
        ...
        automationSuggestion: {
          description:
            "Use fill-down (Ctrl+D) or formulas instead of copy-paste",
          steps: [
    ...
    for (let row = 0; row < data.rowCount; row++) {
      for (let col = 0; col < data.colCount; col++) {
        ...
        for (let r = row - 1; r >= 0 && countAbove < 10; r--) {
          ...
          if (
            aboveCell?.type === "number" &&
            typeof aboveCell.value === "number"
          ) {
        ...
        if (
          countAbove >= 3 &&
          Math.abs(sumAbove - (cell.value as number)) < 0.01
        ) {
          patterns.push({
            ...
            impact: {
              ...
              severity: "medium",
              description:
                "Manual calculations need to be updated when data changes",
            },
