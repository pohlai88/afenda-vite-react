  ...
  private readonly optimizations: OptimizationRule[] = [
    // VLOOKUP → INDEX/MATCH
    {
      ...
      transform: (_match, lookupValue, tableArray, colIndex) => {
        const colLetter = this.getColumnFromRange(
          tableArray,
          parseInt(colIndex) - 1
        )
    ...
    {
      pattern:
        /SUM\s*\(\s*IF\s*\(\s*([^=<>]+)\s*=\s*([^,]+),\s*([^,]+),\s*0\s*\)\s*\)/gi,
    ...
    {
      pattern:
        /COUNT\s*\(\s*IF\s*\(\s*([^=<>]+)\s*=\s*([^,]+),\s*1\s*,\s*0\s*\)\s*\)/gi,
    ...
    // Volatile functions warning
    {
      ...
      improvement: {
        type: "speed",
        description:
          "Volatile functions recalculate on every change, slowing down the workbook",
      },
    },
    // Entire column references
    {
      ...
      improvement: {
        type: "speed",
        description:
          "Using entire column references (A:A) is slower than specific ranges",
      },
    },
    ...
    // Error handling suggestion
    {
      ...
      improvement: {
        type: "reliability",
        description:
          "Wrap lookup functions in IFERROR for better error handling",
      },
    },
  ]
  ...
  async optimize(data: SheetData): Promise<FormulaOptimization[]> {
    ...
    for (let row = 0; row < data.rowCount; row++) {
      for (let col = 0; col < data.colCount; col++) {
        ...
        if (cell?.formula) {
          const cellOptimizations = this.analyzeFormula(
            cell.formula,
            cell.ref,
            data
          )
    ...
    if (analysis.nestedDepth > 3) {
      results.push({
        ...
        improvement: {
          type: "readability",
          description:
            "Breaking complex formulas into steps improves maintainability",
        },
      })
    }
