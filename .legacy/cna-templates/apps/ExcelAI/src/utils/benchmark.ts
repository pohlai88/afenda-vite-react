...
export async function runBenchmark(
  ...
  return {
    ...
    memoryAfter,
    memoryDelta:
      memoryBefore && memoryAfter ? memoryAfter - memoryBefore : undefined,
  }
}
...
export function formatBenchmarkResult(result: BenchmarkResult): string {
  ...
  if (result.memoryDelta !== undefined) {
    lines.push(
      `   Memory Δ: ${result.memoryDelta > 0 ? "+" : ""}${result.memoryDelta}MB`
    )
  }
...
if (typeof window !== "undefined") {
  ;(
    window as unknown as { runBenchmark: typeof runFullBenchmarkSuite }
  ).runBenchmark = runFullBenchmarkSuite
}
