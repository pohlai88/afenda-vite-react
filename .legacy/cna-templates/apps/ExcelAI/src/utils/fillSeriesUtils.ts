...
function detectQuarterPattern(values: string[]): DetectedPattern | null {
  ...
  const indices = values.map((v) =>
    QUARTERS.findIndex((q) => q.toLowerCase() === v.toLowerCase())
  )
...
function detectTextWithNumberPattern(values: string[]): DetectedPattern | null {
  ...
  if (patterns.every((p) => p !== null)) {
    ...
    if (
      validPatterns.every(
        (p) => p.prefix === firstPrefix && p.suffix === firstSuffix
      )
    ) {
      // Calculate step
...
function detectDatePattern(values: string[]): DetectedPattern | null {
  ...
  if (validDates.length >= 2) {
    ...
    for (let i = 1; i < validDates.length; i++) {
      dayDiffs.push(
        Math.round(
          (validDates[i].getTime() - validDates[i - 1].getTime()) /
            (1000 * 60 * 60 * 24)
        )
      )
    }
...
export function generateSeriesValues(
  ...
  switch (pattern.type) {
    ...
    case "growth":
      return generateGrowthSeries(
        sourceValues,
        count,
        config?.step ?? pattern.step ?? 2
      )
 ...
 */
export function generateDayNameSeries(
  pattern: DetectedPattern,
  count: number
): CellValue[] {
  const result: CellValue[] = []
 ...
 */
export function generateMonthNameSeries(
  pattern: DetectedPattern,
  count: number
): CellValue[] {
  const result: CellValue[] = []
 ...
 */
export function generateQuarterSeries(
  pattern: DetectedPattern,
  count: number
): CellValue[] {
  const result: CellValue[] = []
 ...
 */
export function generateTextWithNumberSeries(
  pattern: DetectedPattern,
  count: number
): CellValue[] {
  const result: CellValue[] = []
 ...
 */
export function generateCopySeries(
  sourceValues: CellValue[],
  count: number
): CellValue[] {
  const result: CellValue[] = []
