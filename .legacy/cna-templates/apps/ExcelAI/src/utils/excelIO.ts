...
export function populateChartDataFromCells(
  ...
  const hasHeaders =
    firstRowCell &&
    typeof firstRowCell.value === "string" &&
    isNaN(Number(firstRowCell.value))
...
function excelJSCellToFormat(cell: ExcelJS.Cell): CellFormat | undefined {
  ...
  if (font) {
    ...
    const textColor = resolveColor(
      font.color as { argb?: string; theme?: number; tint?: number }
    )
  ...
  if (fill && fill.type === "pattern" && fill.pattern === "solid") {
    const fgColor = fill.fgColor as
      | { argb?: string; theme?: number; tint?: number }
      | undefined
...
export const importExcelFile = async (file: File): Promise<ImportResult> => {
  ...
  workbook.eachSheet((worksheet) => {
    ...
    for (const drawing of allChartSources) {
      ...
      if (dataRange) {
        chartInfo.dataRange =
          typeof dataRange === "string" ? dataRange : undefined
      }
      ...
      if (Array.isArray(seriesList) && seriesList.length > 0) {
        ...
        if (!chartInfo.dataRange && chartInfo.series.length > 0) {
          ...
          const lastVal =
            chartInfo.series[chartInfo.series.length - 1]?.valuesRef
          if (firstCat && lastVal) {
...
function cellFormatToExcelJS(format: CellFormat): {
  ...
  if (format.textColor)
    font.color = { argb: "FF" + format.textColor.replace("#", "") }
