...
export const calculateDataBarWidth = (
  ...
  if (dataBar.maxType === "percent")
    max =
      stats.min + ((stats.max - stats.min) * (dataBar.maxValue || 100)) / 100
...
export const calculateColorScaleColor = (
  ...
  const min =
    colorScale.minType === "min"
      ? stats.min
      : parseFloat(String(colorScale.minValue)) || 0
  const max =
    colorScale.maxType === "max"
      ? stats.max
      : parseFloat(String(colorScale.maxValue)) || 100
...
export const calculateIconSetIcon = (
  ...
  const percentile =
    (sortedValues.indexOf(value) / (sortedValues.length - 1)) * 100
