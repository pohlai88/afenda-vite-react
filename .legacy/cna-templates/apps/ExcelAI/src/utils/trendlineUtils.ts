...
export function calculatePolynomialTrendline(
  ...
  for (let i = 0; i <= degree; i++) {
    ...
    matrix.push(row)
    rhs.push(
      xValues.reduce((sum, x, k) => sum + Math.pow(x, i) * yValues[k], 0)
    )
  }
...
export function calculatePowerTrendline(
  ...
  const b =
    (m * sumLogXLogY - sumLogX * sumLogY) / (m * sumLogX2 - sumLogX * sumLogX)
...
export function calculateTrendline(
  ...
  switch (config.type) {
    ...
    case "logarithmic":
      return calculateLogarithmicTrendline(
        xValues,
        yValues,
        config.forward || 0
      )
...
export const DEFAULT_TRENDLINE_CONFIGS: Record<
  TrendlineType,
  Partial<TrendlineConfig>
> = {
