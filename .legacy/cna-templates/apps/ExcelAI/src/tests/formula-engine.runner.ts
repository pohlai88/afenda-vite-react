...
export const runFormulaTests = () => {
  ...
  testFormula('=CONCATENATE("A","B","C")', "ABC", "CONCATENATE")
    ? passed++
    : failed++
  testFormula('=CONCAT("X","Y","Z")', "XYZ", "CONCAT") ? passed++ : failed++
  testFormula('="Hello"&" "&"World"', "Hello World", "Concat operator")
    ? passed++
    : failed++
  testFormula('=PROPER("hello world")', "Hello World", "PROPER")
    ? passed++
    : failed++
  testFormula('=REPT("Ab",3)', "AbAbAb", "REPT") ? passed++ : failed++
  testFormula('=FIND("l","Hello")', 3, "FIND") ? passed++ : failed++
  testFormula('=SEARCH("L","Hello")', 3, "SEARCH case-insensitive")
    ? passed++
    : failed++
  testFormula('=SUBSTITUTE("Hello","l","X")', "HeXXo", "SUBSTITUTE")
    ? passed++
    : failed++
  ...
  testFormula('=IF(A1>5,"Big","Small")', "Big", "IF with cell ref")
    ? passed++
    : failed++
  ...
  testFormula('=ISNUMBER("text")', false, "ISNUMBER false")
    ? passed++
    : failed++
  ...
  console.log("\n─── Date Functions ───")
  const dataProvider = createDataProvider()
  const todayResult = formulaEngine.calculate(
    "=TODAY()",
    "sheet1",
    0,
    0,
  ...
  const todayPass =
    typeof todayResult.value === "number" && todayResult.value > 40000
  console.log(
    `${todayPass ? "✅" : "❌"} TODAY returns serial: ${todayResult.value}`
  )
  todayPass ? passed++ : failed++

  const nowResult = formulaEngine.calculate(
    "=NOW()",
    "sheet1",
    0,
    0,
  ...
  console.log(
    `${divZeroPass ? "✅" : "❌"} Division by zero: ${divZero.displayValue}`
  )
  divZeroPass ? passed++ : failed++

  const unknownFunc = formulaEngine.calculate(
    "=UNKNOWN()",
    "sheet1",
    0,
    0,
  ...
  console.log(
    `${unknownPass ? "✅" : "❌"} Unknown function: ${unknownFunc.displayValue}`
  )
  unknownPass ? passed++ : failed++

  const sqrtNeg = formulaEngine.calculate(
    "=SQRT(-1)",
    "sheet1",
    0,
    0,
  ...
  console.log(
    `${sqrtNegPass ? "✅" : "❌"} SQRT negative: ${sqrtNeg.displayValue}`
  )
