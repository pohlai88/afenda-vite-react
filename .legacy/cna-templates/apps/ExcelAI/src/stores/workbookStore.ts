  ...
  subscribeWithSelector((set, get) => ({
    ...
    updateCell: (sheetId, row, col, data) => {
      ...
      if (formula && formula.startsWith("=")) {
        ...
        // Calculate formula
        const result = formulaEngine.calculate(
          formula,
          sheetId,
          row,
          col,
        ...
        if (dependentKeys.length > 0) {
          ...
          setTimeout(() => {
            get().recalculateCells(
              dependentKeys.map((k) => formulaEngine.parseCellKey(k))
            )
          }, 0)
        }
      } else {
        ...
        newCell.displayValue =
          data.displayValue ?? String(data.value ?? existing.value ?? "")
    ...
    clearCell: (sheetId, row, col) => {
      set((state) => {
        ...
        return {
          sheets: {
            ...state.sheets,
            [sheetId]: { ...sheet, cells: remainingCells },
          },
        }
      })
    },
    ...
    applyFormat: (format) => {
      ...
      if (
        !useProtectionStore
          .getState()
          .canPerformAction(activeSheetId, "formatCells")
      )
        return
    ...
    deleteSheet: (sheetId) => {
      ...
      set((state) => {
        ...
        if (state.activeSheetId === sheetId) {
          ...
          newActiveSheetId =
            newSheetOrder[Math.max(0, index - 1)] || newSheetOrder[0]
        }
    ...
    insertRow: (index, count = 1) => {
      ...
      if (
        !useProtectionStore
          .getState()
          .canPerformAction(activeSheetId, "insertRows")
      )
        return
    ...
    deleteRow: (index, count = 1) => {
      ...
      if (
        !useProtectionStore
          .getState()
          .canPerformAction(activeSheetId, "deleteRows")
      )
        return
    ...
    insertColumn: (index, count = 1) => {
      ...
      if (
        !useProtectionStore
          .getState()
          .canPerformAction(activeSheetId, "insertColumns")
      )
        return
    ...
    deleteColumn: (index, count = 1) => {
      ...
      if (
        !useProtectionStore
          .getState()
          .canPerformAction(activeSheetId, "deleteColumns")
      )
        return
    ...
    pasteSpecial: (options) => {
      ...
      clipboardEntries.forEach(([key, cell]) => {
        ...
        if (
          skipBlanks &&
          (cell.value === null || cell.value === "" || cell.value === undefined)
        ) {
          return
        }
          ...
          case "values":
            ...
            if (
              ...
              switch (operation) {
                ...
                case "divide":
                  newValue =
                    cell.value !== 0 ? existing.value / cell.value : "#DIV/0!"
                  break
              }
            }
    ...
    hideRow: (index) => {
      ...
      if (index !== undefined) {
        ...
        for (
          let row = selectionRange.start.row;
    ...
    hideColumn: (index) => {
      ...
      if (index !== undefined) {
        ...
        for (
          let col = selectionRange.start.col;
    ...
    hideSheet: (sheetId) => {
      set((state) => {
        ...
        const visibleSheets = Object.values(state.sheets).filter(
          (s) => !s.hidden
        )
        ...
        return {
          ...
          activeSheetId:
            state.activeSheetId === sheetId
              ? state.sheetOrder.find(
                  (id) => id !== sheetId && !state.sheets[id]?.hidden
                ) || state.activeSheetId
              : state.activeSheetId,
        }
      })
    ...
    unhideSheet: (sheetId) => {
      set((state) => {
        ...
        const { hidden: _, ...restSheet } = sheet as typeof sheet & {
          hidden?: boolean
        }
      ...
      Object.entries(sheet.cells).forEach(([key, cell]) => {
        ...
        if (parseInt(colStr) === colIndex) {
          ...
          const estimatedWidth = Math.max(
            64,
            Math.min(400, contentLength * 8 + 16)
          )
    ...
    setRowHeight: (index, height) => {
      ...
      const rowHeights = {
        ...sheet.rowHeights,
        [index]: Math.max(12, Math.min(400, height)),
      }
    ...
    setColumnWidth: (index, width) => {
      ...
      const columnWidths = {
        ...sheet.columnWidths,
        [index]: Math.max(20, Math.min(500, width)),
      }
    ...
    deleteComment: (row, col) => {
      ...
      set({
        sheets: {
          ...
          [activeSheetId]: {
            ...sheet,
            cells: { ...sheet.cells, [key]: restCell as typeof cell },
          },
        },
      })
    },
