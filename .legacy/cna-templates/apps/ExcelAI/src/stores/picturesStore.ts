...
interface PicturesStore {
  ...
  updatePictureStyle: (
    sheetId: string,
    id: string,
    style: Partial<PictureObject>
  ) => void
  setShadow: (
    sheetId: string,
    id: string,
    shadow: PictureShadow | undefined
  ) => void
  ...
  persist(
    (set, get) => ({
      ...
      addPicture: (sheetId, src, width, height, name) => {
        ...
        const maxZ =
          pictures.length > 0 ? Math.max(...pictures.map((p) => p.zIndex)) : 0
      ...
      deletePicture: (sheetId, id) => {
        set((state) => ({
          pictures: {
            ...state.pictures,
            [sheetId]: (state.pictures[sheetId] || []).filter(
              (p) => p.id !== id
            ),
          },
          selection:
