...
export const useFindStore = create<FindState>((set, get) => ({
  ...
  open: (replaceMode = false) =>
    set({ isOpen: true, isReplaceMode: replaceMode }),

  close: () => set({ isOpen: false }),

  setSearchText: (text) =>
    set({ searchText: text, matches: [], currentMatchIndex: -1 }),
  ...
  replaceCurrent: () => {
    const { matches, currentMatchIndex, replaceText, searchText, options } =
      get()
