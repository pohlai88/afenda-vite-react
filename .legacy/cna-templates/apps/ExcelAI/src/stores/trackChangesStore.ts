...
interface TrackChangesStore {
  ...
  // Recording
  recordChange: (
    change: Omit<CellChange, "id" | "timestamp" | "status">
  ) => string
  ...
  persist(
    (set, get) => ({
      ...
      getPendingChanges: (sheetId) => {
        return (get().changes[sheetId] || []).filter(
          (c) => c.status === "pending"
        )
      },
      ...
      goToPrevChange: (sheetId) => {
        ...
        const prevIndex =
          currentIndex <= 0 ? pendingChanges.length - 1 : currentIndex - 1
