...
interface SlicerStore {
  ...
  updateSlicerPosition: (
    id: string,
    position: Partial<Slicer["position"]>
  ) => void
  updateTimelinePosition: (
    id: string,
    position: Partial<Timeline["position"]>
  ) => void
  ...
  persist(
    (set, get) => ({
      ...
      deleteSlicer: (id) => {
        set((state) => {
          ...
          return {
            slicers: rest,
            selectedSlicerId:
              state.selectedSlicerId === id ? null : state.selectedSlicerId,
          }
        })
      },
      ...
      deleteTimeline: (id) => {
        set((state) => {
          ...
          return {
            timelines: rest,
            selectedTimelineId:
              state.selectedTimelineId === id ? null : state.selectedTimelineId,
          }
        })
      },
      ...
      getTimelinesForPivot: (pivotId) => {
        return Object.values(get().timelines).filter(
          (t) => t.pivotId === pivotId
        )
      },
      ...
      updateTimelinePosition: (id, position) => {
        ...
        if (timeline) {
          get().updateTimeline(id, {
            position: { ...timeline.position, ...position },
          })
        }
      },
      ...
      deleteSlicersForPivot: (pivotId) => {
        set((state) => {
          const newSlicers = Object.fromEntries(
            Object.entries(state.slicers).filter(
              ([_, s]) => s.pivotId !== pivotId
            )
          )
      ...
      deleteTimelinesForPivot: (pivotId) => {
        set((state) => {
          const newTimelines = Object.fromEntries(
            Object.entries(state.timelines).filter(
              ([_, t]) => t.pivotId !== pivotId
            )
          )
