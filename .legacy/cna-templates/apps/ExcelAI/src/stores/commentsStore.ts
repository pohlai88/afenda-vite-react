    ...
    (set, get) => ({
      ...
      deleteComment: (commentId, sheetId) => {
        set((state) => ({
          comments: {
            ...state.comments,
            [sheetId]: (state.comments[sheetId] || []).filter(
              (c) => c.id !== commentId
            ),
          },
          activeCommentId:
      ...
      goToNextComment: (sheetId) => {
        ...
        const currentIndex = sheetComments.findIndex(
      ...
      goToPrevComment: (sheetId) => {
        ...
        const prevIndex =
          currentIndex <= 0 ? sheetComments.length - 1 : currentIndex - 1
      ...
      deleteNote: (noteId, sheetId) => {
        set((state) => ({
          notes: {
            ...state.notes,
            [sheetId]: (state.notes[sheetId] || []).filter(
              (n) => n.id !== noteId
            ),
          },
        }))
      ...
      toggleShowAllComments: () =>
        set((state) => ({ showAllComments: !state.showAllComments })),
      setFilter: (filter) =>
        set((state) => ({ filter: { ...state.filter, ...filter } })),

      getCommentsForCell: (sheetId, cellRef) => {
        return (get().comments[sheetId] || []).filter(
          (c) => c.cellRef === cellRef
        )
      },
