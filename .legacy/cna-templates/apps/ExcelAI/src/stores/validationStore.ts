...
const cellKey = (sheetId: string, row: number, col: number): string =>
  `${sheetId}:${row}:${col}`
  ...
  switch (validationType.type) {
    ...
    case "wholeNumber": {
      ...
      if (
        !checkComparison(
          num,
          validationType.operator,
          validationType.value1,
          validationType.value2
        )
      ) {
        return {
          isValid: false,
    ...
    case "date": {
      ...
      if (!dateRegex.test(value)) {
        return {
          isValid: false,
          message: "Value must be a valid date (YYYY-MM-DD)",
        }
      }
      return { isValid: true }
    }
...
export const useValidationStore = create<ValidationState>()((set, get) => ({
  ...
  removeRule: (ruleId) => {
    set((state) => {
      ...
      const rangeRules = state.rangeRules.filter(
        ([, , , , , rid]) => rid !== ruleId
      )
  ...
  applyToRange: (ruleId, sheetId, startRow, startCol, endRow, endCol) => {
    set((state) => ({
      rangeRules: [
        ...state.rangeRules,
        [sheetId, startRow, startCol, endRow, endCol, ruleId],
      ],
    }))
  },
