...
test.describe("Zoom Controls", () => {
  ...
  test("should show 100% zoom by default", async ({ page }) => {
    await expect(page.locator(".status-bar-2026__zoom-value")).toContainText(
      "100%"
    )
  })
...
test.describe("Keyboard Navigation", () => {
  ...
  test("should navigate with Tab (right) and Shift+Tab (left)", async ({
    page,
  }) => {
    const canvas = page.locator("canvas").nth(0)
    await canvas.click({ position: { x: 50, y: 12 } })
