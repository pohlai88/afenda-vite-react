...
export default function ContentViewerPage() {
    ...
    async function fetchContent() {
      try {
        const res = await fetch(
          `/api/learning/enrollments/${params.id}/content`
        )
        if (res.ok) {
  ...
  const markComplete = async () => {
    ...
    try {
      await fetch(
        `/api/learning/enrollments/${params.id}/content/${currentItem.id}/complete`,
        {
          method: "POST",
        }
      )
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline">
                    {currentItem.type === "video" ? "Video" : "Tài liệu"}
                  </Badge>
                </div>
              </CardHeader>
              <CardContent>
                <div className="min-h-[400px] border rounded-lg p-6 bg-muted/30">
                  <p className="text-muted-foreground">
                    {currentItem.content ||
                      "Nội dung khóa học sẽ được hiển thị ở đây"}
                  </p>
                </div>
