    ...
    async function fetchAnalytics() {
      try {
        const res = await fetch(
          `/api/learning/admin/analytics?period=${period}`
        )
        if (res.ok) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Báo cáo tổng quan về hoạt động đào tạo
          </p>
        </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-muted-foreground">Giờ/Nhân viên</p>
                <p className="text-2xl font-bold">
                  {analytics?.overview.avgHoursPerEmployee || 0}h
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-muted-foreground">Chứng chỉ mới</p>
                <p className="text-2xl font-bold">
                  {analytics?.overview.totalCertifications || 0}
                </p>
              </div>
            </div>
          </CardContent>
                ...
                <div key={i} className="flex items-center gap-4">
                  ...
                  <div className="flex-1 flex items-center gap-2">
                    <div className="flex-1 bg-muted rounded-full h-4 relative">
                      <div
                        className="bg-blue-500 rounded-full h-4 absolute"
                        style={{
                          width: `${(month.enrollments / Math.max(...analytics!.monthlyTrend.map((m) => m.enrollments))) * 100}%`,
                        }}
                      />
                    </div>
                  ...
                  <div className="flex-1 flex items-center gap-2">
                    <div className="flex-1 bg-muted rounded-full h-4 relative">
                      <div
                        className="bg-green-500 rounded-full h-4 absolute"
                        style={{
                          width: `${(month.completions / Math.max(...analytics!.monthlyTrend.map((m) => m.completions || 1))) * 100}%`,
                        }}
                      />
                    </div>
                    <span className="text-xs text-muted-foreground w-12">
