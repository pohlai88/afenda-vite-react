      ...
      {frameworks.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-zinc-500">
          ...
          <p className="text-sm text-zinc-600 mt-1">
            Tạo framework để định nghĩa các năng lực cần đánh giá
          </p>
        </div>
      ) : (
              ...
              <CardContent>
                {framework.competencies.length === 0 ? (
                  <p className="text-sm text-zinc-600 text-center py-4">
                    Chưa có năng lực nào trong framework này
                  </p>
                ) : (
                  <div className="space-y-3">
                    {/* Group by category */}
                    {COMPETENCY_CATEGORIES.map((category) => {
                      const comps = framework.competencies.filter(
                    ...
                    {/* Uncategorized */}
                    {framework.competencies.filter(
                      (c) =>
                        !c.category ||
                        !COMPETENCY_CATEGORIES.includes(c.category as any)
                    ).length > 0 && (
                      <div>
                        <p className="text-xs text-zinc-500 font-medium mb-2 uppercase tracking-wider">
                          Khác
                        ...
                        <div className="space-y-2">
                          {framework.competencies
                            .filter(
                              (c) =>
                                !c.category ||
                                !COMPETENCY_CATEGORIES.includes(
                                  c.category as any
                                )
                            )
                            .map((comp) => (
