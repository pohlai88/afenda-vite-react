        ...
        <div key={idx} className="flex gap-3">
          <div className="flex flex-col items-center">
            <div
              className={`w-3 h-3 rounded-full ${
                event.type === "milestone"
                  ? (event.data as PIPMilestone).completedAt
                    ? "bg-green-400"
                    : "bg-amber-400"
                  : "bg-blue-400"
              }`}
            ...
            {idx < allEvents.length - 1 && (
              <div className="w-px h-full bg-zinc-700 min-h-[20px]" />
            )}
          </div>
          <div className="flex-1 pb-4">
            {event.type === "milestone" ? (
              <div className="p-3 bg-zinc-800 rounded-lg border border-zinc-700">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-zinc-200">
                    {(event.data as PIPMilestone).title}
                  </span>
                  <Badge variant="outline" className="text-xs border-zinc-600">
                    {(event.data as PIPMilestone).completedAt
                      ? "Hoàn thành"
                      : (event.data as PIPMilestone).status}
                  </Badge>
                </div>
                ...
                <p className="text-xs text-zinc-600 mt-1">
                  Hạn:{" "}
                  {new Date(
                    (event.data as PIPMilestone).dueDate
              ...
              <div className="p-3 bg-zinc-800/50 rounded-lg border border-zinc-700/50">
                ...
                <p className="text-xs text-zinc-500 mt-1">
                  {(event.data as PIPCheckIn).progressNotes}
                </p>
              </div>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="flex items-center gap-2 text-zinc-400">
                  ...
                  <span>
                    Nhân viên: {pip.employee?.fullName} (
          ...
          <Card className="bg-zinc-900 border-zinc-800">
            ...
            <CardContent>
              {(pip.milestones?.length || 0) + (pip.checkIns?.length || 0) >
              0 ? (
                <PIPTimeline
              ...
              <div className="flex justify-between">
                ...
                <span className="text-zinc-300">
                  {Math.max(
                    0,
                    Math.ceil(
                      (new Date(pip.endDate).getTime() - Date.now()) /
                        (1000 * 60 * 60 * 24)
                    )
                  )}
                </span>
