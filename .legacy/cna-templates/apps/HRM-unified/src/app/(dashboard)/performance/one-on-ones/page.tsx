...
export default function OneOnOnesPage() {
  ...
  const upcoming = meetings.filter(
    (m) => !m.completedAt && new Date(m.scheduledAt) >= new Date()
  )
      ...
      {meetings.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-zinc-500">
          ...
          <p className="text-sm text-zinc-600 mt-1">
            Lên lịch họp 1-on-1 với team của bạn
          </p>
        </div>
      ) : (
        <>
          ...
          <Card className="bg-zinc-900 border-zinc-800">
            <CardHeader>
              <CardTitle className="text-zinc-100 flex items-center gap-2">
                <Calendar className="h-4 w-4 text-amber-400" /> Sắp tới (
                {upcoming.length})
              </CardTitle>
            </CardHeader>
            <CardContent>
                    ...
                    {upcoming.map((meeting) => (
                      <TableRow key={meeting.id} className="border-zinc-800">
                        <TableCell className="text-zinc-200">
                          {meeting.employee?.fullName || "N/A"} /{" "}
                          {meeting.manager?.fullName || "N/A"}
                        </TableCell>
          ...
          <Card className="bg-zinc-900 border-zinc-800">
            <CardHeader>
              <CardTitle className="text-zinc-100 flex items-center gap-2">
                <Clock className="h-4 w-4 text-zinc-500" /> Đã qua (
                {past.length})
              </CardTitle>
            </CardHeader>
            <CardContent>
