        ...
        <Card className="bg-zinc-900 border-zinc-800">
          ...
          <CardContent>
            ...
            <p className="text-xs text-zinc-500">
              {analytics.completedReviews} hoàn thành
            </p>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <p className="text-2xl font-bold text-zinc-100">
              {analytics.totalReviews > 0
                ? Math.round(
                    (analytics.completedReviews / analytics.totalReviews) * 100
                  )
                : 0}
              %
            </p>
          ...
          <CardContent>
            <div className="space-y-3">
              {analytics.ratingDistribution.map((item) => {
                const ratingInfo = RATING_SCALE.find(
                  (r) => r.value === item.rating
                )
                return (
                  <div key={item.rating} className="flex items-center gap-3">
                    <span className="text-sm text-zinc-400 w-24">
                      {ratingInfo?.label || item.rating}
                    </span>
                ...
                {analytics.topPerformers.map((performer, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span
                        className={`text-sm font-bold ${idx < 3 ? "text-amber-400" : "text-zinc-500"}`}
                      >
                        #{idx + 1}
                      </span>
