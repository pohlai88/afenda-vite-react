...
export default function CycleDetailPage() {
  ...
  const statusInfo =
    REVIEW_CYCLE_STATUS[cycle.status as keyof typeof REVIEW_CYCLE_STATUS]
  const typeInfo =
    REVIEW_CYCLE_TYPE[cycle.cycleType as keyof typeof REVIEW_CYCLE_TYPE]

  const phases = [
    {
      label: "Thiết lập mục tiêu",
      start: cycle.goalSettingStart,
      end: cycle.goalSettingEnd,
      status: "GOAL_SETTING",
    },
    {
      label: "Tự đánh giá",
      start: cycle.selfReviewStart,
      end: cycle.selfReviewEnd,
      status: "SELF_REVIEW",
    },
    {
      label: "Quản lý đánh giá",
      start: cycle.managerReviewStart,
      end: cycle.managerReviewEnd,
      status: "MANAGER_REVIEW",
    },
    {
      label: "Calibration",
      start: cycle.calibrationStart,
      end: cycle.calibrationEnd,
      status: "CALIBRATION",
    },
  ]
        ...
        <CardContent>
          <div className="flex gap-2">
            {phases.map((phase, idx) => {
              ...
              const isPast =
                phases.findIndex((p) => p.status === cycle.status) > idx
              return (
                <div key={idx} className="flex-1">
                  <div
                    className={`h-2 rounded ${isActive ? "bg-amber-400" : isPast ? "bg-green-500" : "bg-zinc-800"}`}
                  />
                  <p
                    className={`text-xs mt-2 ${isActive ? "text-amber-400" : isPast ? "text-green-400" : "text-zinc-600"}`}
                  >
                    {phase.label}
                  </p>
                  {phase.start && (
                    <p className="text-[10px] text-zinc-600">
                      {new Date(phase.start).toLocaleDateString("vi-VN")} -{" "}
                      {phase.end
                        ? new Date(phase.end).toLocaleDateString("vi-VN")
                        : ""}
                    </p>
                  )}
                </div>
          ...
          {reviews.length === 0 ? (
            ...
            <Table>
              ...
              <TableBody>
                {reviews.map((review) => {
                  const revStatus =
                    REVIEW_STATUS[review.status as keyof typeof REVIEW_STATUS]
                  return (
