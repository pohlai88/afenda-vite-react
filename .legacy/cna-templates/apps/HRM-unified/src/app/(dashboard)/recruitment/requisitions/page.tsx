...
import { Plus, Search, Eye, SendHorizontal, MoreHorizontal } from "lucide-react"
    ...
    async function fetchRequisitions() {
      try {
        ...
        const res = await fetch(
          `/api/recruitment/requisitions?${params.toString()}`
        )
        if (!res.ok)
          throw new Error("Không thể tải danh sách yêu cầu tuyển dụng")
        const json = await res.json()
        // API returns { success: true, data: { requisitions, total, page, limit } }
        const data =
          json.data?.requisitions ||
          json.data ||
          json.requisitions ||
          json ||
          []
