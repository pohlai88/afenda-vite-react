...
test.describe("Leave Request - Employee", () => {
  ...
  test("should create leave request", async ({ page }) => {
    ...
    await page.click(
      '[data-testid="new-request-button"], button:has-text("Tạo đơn")'
    )
  ...
  test("should cancel pending request", async ({ page }) => {
    ...
    const pendingRequest = page
      .locator('[data-testid="leave-request"][data-status="PENDING"]')
      .first()
    ...
    if (await pendingRequest.isVisible()) {
      ...
      // Click cancel button
      await page.click(
        '[data-testid="cancel-request"], button:has-text("Hủy đơn")'
      )
...
test.describe("Leave Approval - Manager", () => {
  ...
  test("should approve leave request", async ({ page }) => {
    const pendingRequest = page
      .locator('[data-testid="pending-request"]')
      .first()
    ...
    if (await pendingRequest.isVisible()) {
      ...
      // Click approve
      await page.click(
        '[data-testid="approve-button"], button:has-text("Duyệt")'
      )
  ...
  test("should reject leave request with reason", async ({ page }) => {
    ...
    if (await pendingRequest.isVisible()) {
      ...
      // Click reject
      await page.click(
        '[data-testid="reject-button"], button:has-text("Từ chối")'
      )

      // Must provide reason
      await page.fill(
        'textarea[name="rejectReason"]',
        "E2E Test - Lý do từ chối"
      )
