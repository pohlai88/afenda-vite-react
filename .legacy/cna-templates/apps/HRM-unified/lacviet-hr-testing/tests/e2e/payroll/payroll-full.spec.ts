...
test.describe("Payroll Management", () => {
  ...
  test.describe("Payroll Period", () => {
    ...
    test("should show period status workflow", async () => {
      ...
      expect(statusText).toMatch(
        /Nháp|Đang tính|Chờ duyệt|Đã duyệt|Đã thanh toán/
      )
    })
  ...
  test.describe("Salary Calculation", () => {
    test("@critical should calculate payroll for period", async () => {
      ...
      await expect(
        page.locator('[data-testid="calculation-progress"]')
      ).toBeVisible()
      await expect(
        page.locator('[data-testid="calculation-complete"]')
      ).toBeVisible({ timeout: 120000 })

      // Verify results
      await expect(
        page.locator('[data-testid="payroll-summary"]')
      ).toBeVisible()
    })
    ...
    test("should calculate PIT tax correctly", async () => {
      ...
      await expect(
        page.locator('[data-testid="personal-deduction"]')
      ).toBeVisible()
      await expect(
        page.locator('[data-testid="dependent-deductions"]')
      ).toBeVisible()
    ...
    test("should calculate insurance correctly", async () => {
      ...
      await expect(
        page.locator('[data-testid="total-insurance"]')
      ).toBeVisible()
    })
    ...
    test("should handle mid-month joiners/leavers", async () => {
      ...
      const partialEmployees = page.locator(
        '[data-testid="employee-salary-row"]'
      )
      if ((await partialEmployees.count()) > 0) {
        await partialEmployees.first().click()

        // Verify pro-rated salary
        const workingDays = await page
          .locator('[data-testid="working-days"]')
          .textContent()
        const totalDays = await page
          .locator('[data-testid="total-days"]')
          .textContent()

        expect(parseFloat(workingDays || "0")).toBeLessThan(
          parseFloat(totalDays || "0")
        )
      }
    })
  })
  ...
  test.describe("Payslips", () => {
    test("@critical should generate payslips", async () => {
      ...
      // Wait for generation
      await expect(
        page.locator('[data-testid="generating-payslips"]')
      ).toBeVisible()
    ...
    test("should view payslip", async () => {
      ...
      await expect(
        page.locator('[data-testid="earnings-section"]')
      ).toBeVisible()
      await expect(
        page.locator('[data-testid="deductions-section"]')
      ).toBeVisible()
    ...
    test("should bulk send payslips", async () => {
      ...
      // Verify progress
      await expect(
        page.locator('[data-testid="sending-progress"]')
      ).toBeVisible()
      await expect(page.locator('[data-testid="send-complete"]')).toBeVisible({
        timeout: 120000,
      })
    })
    ...
    test("employee should view own payslip", async () => {
      ...
      await expect(
        page.locator('[data-testid="my-payslips-list"]')
      ).toBeVisible()
      await page.locator('[data-testid="payslip-row"]').first().click()
  ...
  test.describe("Salary Adjustments", () => {
    test("should add bonus to employee", async () => {
      ...
      // Select type
      await page
        .locator('[data-testid="adjustment-type"]')
        .selectOption("BONUS")
      ...
      // Enter description
      await page
        .locator('[data-testid="adjustment-note"]')
        .fill("Thưởng KPI Q4/2024")
    ...
    test("should add allowance to employee", async () => {
      ...
      await page
        .locator('[data-testid="adjustment-type"]')
        .selectOption("ALLOWANCE")
      await page
        .locator('[data-testid="allowance-type"]')
    ...
    test("should bulk import adjustments", async () => {
      ...
      await page.setInputFiles(
        ...
        "./tests/fixtures/salary-adjustments.xlsx"
      )
  ...
  test.describe("Bank Transfer", () => {
    test("@critical should generate bank transfer file", async () => {
      ...
      const downloadPromise = page.waitForEvent("download")
      await expect(page.locator('[data-testid="file-ready"]')).toBeVisible({
        timeout: 30000,
      })
    ...
    test("should preview bank transfer", async () => {
      ...
      await expect(
        page.locator('[data-testid="transfer-preview-table"]')
      ).toBeVisible()
    ...
    test("should mark period as paid", async () => {
      ...
      await page
        ...
        .fill("VCB-20250205-001")
      await page.locator('[data-testid="confirm-paid"]').click()

      await expect(page.locator('[data-testid="period-status"]')).toContainText(
        "Đã thanh toán"
      )
    })
  })
  ...
  test.describe("Reports", () => {
    ...
    test("should generate department cost report", async () => {
      ...
      await expect(
        page.locator('[data-testid="department-cost-chart"]')
      ).toBeVisible()
      await expect(
        page.locator('[data-testid="department-cost-table"]')
      ).toBeVisible()
    })
    ...
    test("should generate PIT report (C06-TS)", async () => {
      ...
      await expect(
        page.locator('[data-testid="pit-report-table"]')
      ).toBeVisible()
    ...
    test("should generate insurance report (D02-TS)", async () => {
      ...
      await expect(
        page.locator('[data-testid="insurance-report"]')
      ).toBeVisible()
  ...
  test.describe("Settings", () => {
    test("should configure PIT settings", async () => {
      ...
      // Personal deduction
      const personalDeduction = page.locator(
        '[data-testid="personal-deduction"]'
      )
      await expect(personalDeduction).toHaveValue("11000000")

      // Dependent deduction
      const dependentDeduction = page.locator(
        '[data-testid="dependent-deduction"]'
      )
      await expect(dependentDeduction).toHaveValue("4400000")

      // Tax brackets should be displayed
      await expect(
        page.locator('[data-testid="tax-brackets-table"]')
      ).toBeVisible()
    })
    ...
    test("should configure insurance settings", async () => {
      ...
      // Insurance rates
      await expect(
        page.locator('[data-testid="bhxh-rate-employee"]')
      ).toBeVisible()
      await expect(
        page.locator('[data-testid="bhxh-rate-employer"]')
      ).toBeVisible()
      await expect(
        page.locator('[data-testid="bhyt-rate-employee"]')
      ).toBeVisible()
      await expect(
        page.locator('[data-testid="bhtn-rate-employee"]')
      ).toBeVisible()

      // Salary caps
      await expect(
        page.locator('[data-testid="max-insurance-salary"]')
      ).toBeVisible()
    })
    ...
    test("should configure overtime rates", async () => {
      ...
      // OT rates
      await expect(page.locator('[data-testid="ot-weekday-rate"]')).toHaveValue(
        "150"
      )
      await expect(page.locator('[data-testid="ot-weekend-rate"]')).toHaveValue(
        "200"
      )
      await expect(page.locator('[data-testid="ot-holiday-rate"]')).toHaveValue(
        "300"
      )
      await expect(page.locator('[data-testid="ot-night-rate"]')).toHaveValue(
        "30"
      )
    })
  })
