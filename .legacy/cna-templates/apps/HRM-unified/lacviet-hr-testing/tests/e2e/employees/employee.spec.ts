...
test.describe("Employee Management", () => {
  ...
  test.describe("Create Employee", () => {
    test("should create new employee with valid data", async () => {
      ...
      await employeePage.expectEmployeeInList(
    ...
    test("should show validation errors for required fields", async () => {
      ...
      await employeePage.expectFormError(
        "departmentId",
  ...
  test.describe("View Employee", () => {
    ...
    test("should display employee details", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="employee-email"]')
      ).toContainText(testUsers.existingEmployee.email)
      await expect(
        page.locator('[data-testid="employee-department"]')
      ).toContainText(testUsers.existingEmployee.department)
    })
  ...
  test.describe("Update Employee", () => {
    test("should update employee information", async () => {
      ...
      await employeePage.gotoDetail(testUsers.existingEmployee.id)
      await expect(
        employeePage.page.locator('[data-testid="employee-name"]')
    ...
    test("should update employee department", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="employee-department"]')
      ).toContainText(testDepartments.engineering.name)
    })

    test("should show confirmation when changing critical fields", async ({
      page,
    }) => {
      await employeePage.gotoEdit(testUsers.existingEmployee.id)

      // Change email (critical field)
      await employeePage.fillInput(
        employeePage.emailInput,
  ...
  test.describe("Delete Employee", () => {
    test("should deactivate employee (soft delete)", async () => {
      ...
      await employeePage.goto()
      await employeePage.expectEmployeeNotInList(
        `${employeeData.firstName} ${employeeData.lastName}`
      )
    ...
    test("should show confirmation before delete", async () => {
      ...
      await employeePage.expectConfirmDialog(
  ...
  test.describe("Search & Filter", () => {
    ...
    test("should filter by department", async () => {
      ...
      for (let i = 0; i < count; i++) {
        const dept = await rows
          .nth(i)
          .locator('[data-testid="employee-dept"]')
          .textContent()
    ...
    test("should show empty state when no results", async () => {
      ...
      await expect(
        employeePage.page.locator('[data-testid="empty-state"]')
      ).toBeVisible()
      await expect(
        employeePage.page.locator('[data-testid="empty-state"]')
      ).toContainText("Không tìm thấy nhân viên")
    })
  })
  ...
  test.describe("Pagination", () => {
    ...
    test("should navigate to specific page", async () => {
      ...
      const activePage = employeePage.page.locator(
        '[data-testid="pagination-page-3"].active'
      )
  ...
  test.describe("Bulk Actions", () => {
    test("should select multiple employees", async () => {
      ...
      const selectedCount = await employeePage.page
    ...
    test("should bulk update department", async () => {
      ...
      await employeePage.expectToastMessage(
        "Cập nhật thành công 2 nhân viên",
        "success"
      )
    })
  })
  ...
  test.describe("Import/Export", () => {
    ...
    test("should import employees from Excel", async () => {
      ...
      await employeePage.importEmployees(
        "./tests/fixtures/employees-import.xlsx"
      )
