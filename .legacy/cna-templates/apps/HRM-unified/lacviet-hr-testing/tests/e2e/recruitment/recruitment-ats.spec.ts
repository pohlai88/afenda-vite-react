...
test.describe("Recruitment & ATS", () => {
  ...
  test.describe("Job Postings", () => {
    test("should display job postings list", async ({ page }) => {
      ...
      const jobCount = await page
        .locator('[data-testid="jobs-table"] tbody tr')
        .count()
    ...
    test("should create new job posting", async ({ page }) => {
      ...
      await page
        ...
        .fill("Senior Software Developer")
      await page
        .locator('[data-testid="field-department"]')
        .selectOption("Engineering")
      await page
        .locator('[data-testid="field-location"]')
        .fill("Ho Chi Minh City")
      await page
        .locator('[data-testid="field-employment-type"]')
      ...
      await page
        ...
        .fill("We are looking for a senior developer...")
      await page
        .locator('[data-testid="field-requirements"]')
        .fill("- 5+ years experience\n- Strong JavaScript skills")
    ...
    test("should edit existing job posting", async ({ page }) => {
      ...
      await page
        .locator('[data-testid="field-title"]')
        .fill("Senior Software Developer (Updated)")
  ...
  test.describe("Candidates", () => {
    test("should display candidates list", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="candidates-table"]')
      ).toBeVisible()
    })
    ...
    test("should add new candidate manually", async ({ page }) => {
      ...
      await page
        .locator('[data-testid="field-firstName"]')
        .fill(candidateData.firstName)
      await page
        .locator('[data-testid="field-lastName"]')
        .fill(candidateData.lastName)
      await page
        .locator('[data-testid="field-email"]')
        .fill(candidateData.email)
      await page
        .locator('[data-testid="field-phone"]')
        .fill(candidateData.phone)
      await page
        .locator('[data-testid="field-position"]')
        .fill(candidateData.position)
      await page
        .locator('[data-testid="field-experience"]')
        .fill(candidateData.experience.toString())
      await page
        .locator('[data-testid="field-expectedSalary"]')
        .fill(candidateData.expectedSalary.toString())
    ...
    test("should upload candidate resume", async ({ page }) => {
      ...
      // Upload resume
      await page
        .locator('[data-testid="resume-upload"]')
  ...
  test.describe("Kanban Pipeline", () => {
    test("should display kanban board", async ({ page }) => {
      ...
      for (const stage of stages) {
        await expect(
          page.locator(`[data-testid="column-${stage}"]`)
        ).toBeVisible()
      }
    })
    ...
    test("should drag candidate to next stage", async ({ page }) => {
      ...
      const candidateCard = page
        .locator('[data-testid="column-NEW"] [data-testid="candidate-card"]')
        .first()
      ...
      if (await candidateCard.isVisible()) {
        ...
        ).toContainText("Cập nhật trạng thái thành công")
      }
    })
    ...
    test("should quick actions on candidate card", async ({ page }) => {
      ...
      if (await candidateCard.isVisible()) {
        ...
        await expect(
          candidateCard.locator('[data-testid="quick-schedule"]')
        ).toBeVisible()
        await expect(
          candidateCard.locator('[data-testid="quick-email"]')
        ).toBeVisible()
        await expect(
          candidateCard.locator('[data-testid="quick-reject"]')
        ).toBeVisible()
      }
    })
  })
  ...
  test.describe("Interviews", () => {
    test("should schedule interview", async ({ page }) => {
      ...
      const firstCandidate = page
        .locator('[data-testid="candidates-table"] tbody tr')
        .first()
      ...
      await page
        .locator('[data-testid="interview-type"]')
        .selectOption("IN_PERSON")
      await page.locator('[data-testid="interviewer"]').click()
      await page.locator('[role="option"]').first().click()
      await page
        .locator('[data-testid="interview-notes"]')
        .fill("Technical interview round 1")
    ...
    test("should display interview calendar", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="interview-calendar"]')
      ).toBeVisible()
    ...
    test("should submit interview feedback", async ({ page }) => {
      ...
      const pendingFeedback = page
        .locator('[data-testid="pending-feedback"]')
        .first()
      ...
      if (await pendingFeedback.isVisible()) {
        ...
        // Fill feedback form
        await page
          .locator('[data-testid="rating-technical"]')
          .locator("button")
          .nth(3)
          .click() // 4 stars
        await page
          .locator('[data-testid="rating-communication"]')
          .locator("button")
          .nth(4)
          .click() // 5 stars
        await page
          .locator('[data-testid="rating-cultural"]')
          .locator("button")
          .nth(3)
          .click() // 4 stars
        await page
          .locator('[data-testid="feedback-notes"]')
          .fill("Strong technical skills, good communication")
        await page
          .locator('[data-testid="recommendation"]')
  ...
  test.describe("Recruitment Reports", () => {
    test("should display recruitment dashboard", async ({ page }) => {
      ...
      // Check key metrics
      await expect(
        page.locator('[data-testid="metric-open-positions"]')
      ).toBeVisible()
      await expect(
        page.locator('[data-testid="metric-total-candidates"]')
      ).toBeVisible()
      await expect(
        page.locator('[data-testid="metric-interviews-scheduled"]')
      ).toBeVisible()
      await expect(
        page.locator('[data-testid="metric-offers-pending"]')
      ).toBeVisible()
    })
