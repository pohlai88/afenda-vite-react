...
test.describe("Job Requisitions", () => {
  ...
  test("should create new job requisition", async ({ page }) => {
    ...
    await page.click(
      '[data-testid="new-requisition"], button:has-text("Tạo tin tuyển dụng")'
    )
    ...
    const descriptionEditor = page.locator(
      '[data-testid="description-editor"] .ProseMirror, textarea[name="description"]'
    )
    await descriptionEditor.fill(
      "E2E Test Job Description - Yêu cầu công việc..."
    )

    // Fill requirements
    const requirementsEditor = page.locator(
      '[data-testid="requirements-editor"] .ProseMirror, textarea[name="requirements"]'
    )
    await requirementsEditor.fill(
...
test.describe("Candidates", () => {
  ...
  test("should add new candidate", async ({ page }) => {
    await page.click(
      '[data-testid="add-candidate"], button:has-text("Thêm ứng viên")'
    )
    ...
    await page.fill(
      'input[name="email"]',
      `e2e.candidate.${Date.now()}@test.com`
    )
...
test.describe("Interviews", () => {
  ...
  test("should submit interview feedback", async ({ page }) => {
    ...
    await page.fill(
      ...
      "E2E Test - Ứng viên có kỹ năng tốt"
    )
  ...
  test("should drag and drop candidate between stages", async ({ page }) => {
    ...
    if (await candidateCard.isVisible()) {
      ...
      const targetStage = page.locator(
        '[data-testid="pipeline-stage"]:nth-child(2)'
      )
...
test.describe("Recruitment Reports", () => {
  ...
  test("should display recruitment metrics", async ({ page }) => {
    ...
    // Key metrics
    await expect(
      page.locator('[data-testid="metric-time-to-fill"]')
    ).toBeVisible()
    await expect(
      page.locator('[data-testid="metric-cost-per-hire"]')
    ).toBeVisible()
  })
