...
test.describe("Core Web Vitals", () => {
  ...
  test("should have good FID simulation (First Input Delay)", async ({
    page,
  }) => {
    await page.goto("/dashboard")
    await page.waitForLoadState("networkidle")
    ...
    const startTime = Date.now()
    await page
      .click('[data-testid="first-interactive"], button, a', { timeout: 5000 })
      .catch(() => {})
...
test.describe("Rendering Performance", () => {
  ...
  test("should handle pagination without lag", async ({ page }) => {
    ...
    const page2Button = page.locator(
      '[data-testid="page-2"], .pagination button:has-text("2")'
    )
  ...
  test("should handle search with minimal delay", async ({ page }) => {
    ...
    const searchInput = page.locator(
      'input[name="search"], input[placeholder*="Tìm"]'
    )
...
test.describe("Network Performance", () => {
  test("should minimize API calls on dashboard", async ({ page }) => {
    ...
    console.log(
      "Endpoints:",
      apiCalls.map((url) => new URL(url).pathname)
    )
