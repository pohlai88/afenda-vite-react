...
test.describe("Workflow & Approval System", () => {
  ...
  test.describe("Workflow Configuration", () => {
    ...
    test("@critical should create leave approval workflow", async () => {
      ...
      await page
        ...
        .fill("Phê duyệt nghỉ phép")
      await page
        .locator('[data-testid="workflow-type"]')
        .selectOption("LEAVE_REQUEST")
      await page
        .locator('[data-testid="workflow-description"]')
        .fill("Quy trình phê duyệt đơn nghỉ phép")

      // Add approval steps
      // Step 1: Direct manager
      await page.locator('[data-testid="add-step"]').click()
      await page
        .locator('[data-testid="step-name-0"]')
        .fill("Quản lý trực tiếp")
      await page
        .locator('[data-testid="approver-type-0"]')
        .selectOption("DIRECT_MANAGER")

      // Step 2: Department head
      await page.locator('[data-testid="add-step"]').click()
      await page.locator('[data-testid="step-name-1"]').fill("Trưởng phòng")
      await page
        .locator('[data-testid="approver-type-1"]')
        .selectOption("DEPARTMENT_HEAD")
    ...
    test("should create conditional workflow", async () => {
      ...
      await page
        .locator('[data-testid="workflow-name"]')
        .fill("Phê duyệt nghỉ phép có điều kiện")
      await page
        .locator('[data-testid="workflow-type"]')
        .selectOption("LEAVE_REQUEST")
      ...
      await page
        ...
        .selectOption("LEAVE_DAYS")
      await page
        .locator('[data-testid="condition-operator"]')
        .selectOption("GREATER_THAN")
      await page.locator('[data-testid="condition-value"]').fill("3")

      // If days > 3, require additional approval
      await page
        .locator('[data-testid="then-approver-type"]')
        .selectOption("HR_MANAGER")
      await page
        .locator('[data-testid="else-approver-type"]')
        .selectOption("DIRECT_MANAGER")
    ...
    test("should set workflow priority and SLA", async () => {
      ...
      await page
        .locator('[data-testid="escalation-to"]')
        .selectOption("HR_DIRECTOR")
    ...
    test("should clone existing workflow", async () => {
      ...
      await page
        .locator('[data-testid="workflow-name"]')
        .fill("Phê duyệt nghỉ phép - Chi nhánh HCM")
  ...
  test.describe("Approval Process", () => {
    test("@critical should submit request and track through approval", async () => {
      ...
      await expect(
        page.locator('[data-testid="request-status"]').first()
    ...
    test("should reject request with reason", async () => {
      ...
      await page
        ...
        .fill("Không đủ nhân sự trong thời gian này")
    ...
    test("should request more information", async () => {
      ...
      await page
        .locator('[data-testid="info-request-message"]')
        .fill("Vui lòng cung cấp thêm lý do chi tiết")
    ...
    test("should show approval history", async () => {
      ...
      await expect(
        page.locator('[data-testid="approval-timeline"]')
      ).toBeVisible()
  ...
  test.describe("Approval Delegation", () => {
    ...
    test("should show delegation history", async () => {
      await expect(
        page.locator('[data-testid="delegation-history"]')
      ).toBeVisible()
    })
    ...
    test("delegated user should see approval tasks", async () => {
      ...
      await page.goto("/approvals/pending")
      await expect(
        page.locator('[data-testid="delegated-badge"]')
      ).toBeVisible()
    })
  })
  ...
  test.describe("Escalation", () => {
    test("should show escalation warning", async () => {
      ...
      if ((await overdueItem.count()) > 0) {
        await expect(
          overdueItem.first().locator('[data-testid="escalation-warning"]')
        ).toBeVisible()
      }
    })
    ...
    test("should manually escalate request", async () => {
      ...
      await page
        ...
        .fill("Cần quyết định từ cấp cao hơn")
    ...
    test("admin should view escalation dashboard", async () => {
      ...
      await expect(
        page.locator('[data-testid="escalation-dashboard"]')
      ).toBeVisible()
      await expect(
        page.locator('[data-testid="escalation-stats"]')
      ).toBeVisible()
    })
  })
  ...
  test.describe("Bulk Approval", () => {
    ...
    test("should bulk reject multiple requests", async () => {
      ...
      await page
        .locator('[data-testid="bulk-reject-reason"]')
        .fill("Không đủ điều kiện")
  ...
  test.describe("Reports", () => {
    ...
    test("should view approval turnaround report", async () => {
      ...
      await expect(
        page.locator('[data-testid="turnaround-chart"]')
      ).toBeVisible()
      await expect(
        page.locator('[data-testid="avg-turnaround-time"]')
      ).toBeVisible()
    })
    ...
    test("should view approval by manager report", async () => {
      ...
      await expect(
        page.locator('[data-testid="manager-approval-table"]')
      ).toBeVisible()
    })
