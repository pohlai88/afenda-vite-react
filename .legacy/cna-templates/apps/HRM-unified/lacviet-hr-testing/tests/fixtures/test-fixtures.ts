...
export class BasePage {
  ...
  async expectToastSuccess(message?: string) {
    const toast = this.page
      .locator('.toast-success, [data-type="success"]')
      .first()
...
export class DashboardPage extends BasePage {
  ...
  readonly recentActivities = this.page.locator(
  ...
  readonly pendingApprovals = this.page.locator(
...
export class EmployeeListPage extends BasePage {
  readonly searchInput = this.page.locator(
    'input[placeholder*="Tìm kiếm"], input[name="search"]'
  )
  readonly filterButton = this.page.locator('[data-testid="filter-button"]')
  readonly addEmployeeButton = this.page.locator(
    '[data-testid="add-employee"], button:has-text("Thêm nhân viên")'
  )
  readonly employeeTable = this.page.locator(
    'table, [data-testid="employee-table"]'
  )
  readonly employeeRows = this.page.locator(
    'tbody tr, [data-testid="employee-row"]'
  )
  ...
  async filterByDepartment(department: string) {
    ...
    await this.page.selectOption('[data-testid="department-filter"]', {
      label: department,
    })
...
export class EmployeeFormPage extends BasePage {
  ...
  readonly departmentSelect = this.page.locator(
    'select[name="departmentId"], [data-testid="department-select"]'
  )
  readonly positionSelect = this.page.locator(
    'select[name="positionId"], [data-testid="position-select"]'
  )
  ...
  async fillForm(employee: Partial<TestEmployee>) {
    ...
    if (employee.departmentId)
      await this.departmentSelect.selectOption(employee.departmentId)
    if (employee.positionId)
      await this.positionSelect.selectOption(employee.positionId)
  }
  ...
  async expectValidationError(field: string) {
    const error = this.page.locator(
      `[data-testid="error-${field}"], .error-${field}`
    )
