...
export class PayrollPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.calculationProgress = page.locator(
      '[data-testid="calculation-progress"]'
    )
    ...
    this.generatePayslipsButton = page.locator(
      '[data-testid="generate-payslips"]'
    )
    this.downloadPayslipButton = page.locator(
    ...
    this.generateBankFileButton = page.locator(
      '[data-testid="generate-bank-file"]'
    )
    this.downloadBankFileButton = page.locator(
      '[data-testid="download-bank-file"]'
    )
  ...
  async calculatePayroll(): Promise<void> {
    ...
    await expect(
      this.page.locator('[data-testid="calculation-complete"]')
    ).toBeVisible({
      timeout: 120000,
    })
  }
  ...
  async generatePayslips(): Promise<void> {
    ...
    await expect(
      this.page.locator('[data-testid="generating-payslips"]')
    ).toBeVisible()
    await expect(
      this.page.locator('[data-testid="payslips-ready"]')
    ).toBeVisible({
      timeout: 60000,
    })
  }
  ...
  async bulkSendPayslips(): Promise<void> {
    ...
    await expect(
      this.page.locator('[data-testid="sending-progress"]')
    ).toBeVisible()
    await expect(
      this.page.locator('[data-testid="send-complete"]')
    ).toBeVisible({
      timeout: 120000,
    })
  }
  ...
  async addAdjustment(data: PayrollAdjustment): Promise<void> {
    ...
    if (data.employeeId) {
      ...
      await this.page
        .locator(`[data-testid="employee-${data.employeeId}"]`)
        .click()
    } else {
      await this.page.locator('[data-testid="employee-select"]').click()
    ...
    if (data.type === "ALLOWANCE" && data.allowanceType) {
      await this.page
        ...
        .selectOption(data.allowanceType)
    }
  ...
  async markPeriodAsPaid(
    ...
    await this.fillInput(
      this.page.locator('[data-testid="payment-date"]'),
      paymentDate
    )
    await this.fillInput(
      this.page.locator('[data-testid="payment-reference"]'),
      reference
    )
  ...
  async expectPayslipVisible(): Promise<void> {
    await expect(
      this.page.locator('[data-testid="payslip-header"]')
    ).toBeVisible()
    await expect(
      this.page.locator('[data-testid="earnings-section"]')
    ).toBeVisible()
    await expect(
      this.page.locator('[data-testid="deductions-section"]')
    ).toBeVisible()
