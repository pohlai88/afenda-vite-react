...
export const NineBoxGrid: React.FC<NineBoxGridProps> = ({
  ...
  const getBoxData = (position: NineBoxPosition) => {
    return (
      data.find((d) => d.position === position) || {
        position,
        count: 0,
        employees: [],
      }
          ...
          <div className="mt-2 space-y-1 max-h-20 overflow-y-auto">
            ...
            {boxData.employees.length > 3 && (
              <div className="text-xs text-gray-400">
                +{boxData.employees.length - 3} more
              </div>
            )}
          </div>
        )}
    ...
    return (
      <div className="bg-white rounded-lg border p-4 hover:shadow-md transition-shadow">
        <div className="flex items-center space-x-3">
          ...
          <div
            className={`px-2 py-1 rounded text-xs ${flightRiskColors[profile.flightRisk]}`}
          >
  ...
  return (
    <div className="bg-white rounded-lg shadow overflow-hidden">
      ...
      <div className="grid grid-cols-4 gap-px bg-gray-200">
        ...
        <div className="bg-white p-4 text-center">
          <div className="text-2xl font-bold text-gray-900">
            {profile.yearsInCompany.toFixed(1)}
          </div>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <span
            className={`px-2 py-1 rounded text-sm ${flightRiskColors[profile.flightRisk]}`}
          >
            {profile.flightRisk.replace("_", " ")}
          </span>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between text-sm mb-1">
            ...
            <span className="text-gray-900">
              ...
              {profile.activeDevelopmentActivities +
                profile.completedDevelopmentActivities}
            </span>
          </div>
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              {plan.criticalPosition.jobTitle} •{" "}
              {plan.criticalPosition.departmentName}
            </p>
          </div>
          <button
...
const SuccessorCard: React.FC<SuccessorCardProps> = ({
  successor,
  rank,
  onEdit,
  onRemove,
  ...
  const readinessColors: Record<SuccessorReadiness, string> = {
    ...
    [SuccessorReadiness.READY_1_YEAR]:
      "bg-blue-100 text-blue-800 border-blue-300",
    [SuccessorReadiness.READY_2_YEARS]:
      "bg-yellow-100 text-yellow-800 border-yellow-300",
    [SuccessorReadiness.READY_3_PLUS]:
  ...
  return (
    <div className="border rounded-lg p-4 hover:shadow-md transition-shadow">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center space-x-2">
            <span className="font-medium text-gray-900">
              {successor.talentProfile.fullName}
            </span>
          ...
          <div className="text-sm text-gray-500">
            {successor.talentProfile.currentPositionTitle} •{" "}
            {successor.talentProfile.departmentName}
          </div>
        </div>

        {/* Readiness */}
        <div
          className={`px-3 py-1 rounded-full border text-sm ${readinessColors[successor.readiness]}`}
        >
      ...
      <div className="mt-4 pt-4 border-t grid grid-cols-4 gap-4 text-sm">
        ...
        <div>
          ...
          <div
            className={`font-medium ${
              successor.flightRisk === FlightRisk.VERY_HIGH ||
              successor.flightRisk === FlightRisk.HIGH
                ? "text-red-600"
                : "text-gray-900"
            }`}
          >
        ...
        <div>
          ...
          <div className="font-medium">
            {successor.developmentProgress}% complete
          </div>
        </div>
      </div>
      ...
      {(successor.competencyGaps.length > 0 ||
        successor.experienceGaps.length > 0) && (
        <div className="mt-3 p-3 bg-yellow-50 rounded text-sm">
          ...
          <div className="flex flex-wrap gap-1">
            {successor.competencyGaps.slice(0, 3).map((gap, i) => (
              <span
                key={i}
                className="px-2 py-0.5 bg-yellow-100 text-yellow-700 rounded"
              >
                {gap.competencyName}
              </span>
            ))}
          ...
          {plan.activities.map((activity) => (
            <ActivityCard
              ...
              onUpdateProgress={(progress) =>
                onUpdateActivity(activity.id, progress)
              }
            />
          ))}
        </div>
      ...
      <div className="flex items-start space-x-3">
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between">
            ...
            <span
              className={`px-2 py-0.5 rounded text-xs ${statusColors[activity.status]}`}
            >
