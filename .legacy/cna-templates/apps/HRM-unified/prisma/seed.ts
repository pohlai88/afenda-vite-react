...
async function main() {
  ...
  const employeeDistribution = [
    {
      deptCode: "BGD",
      count: 5,
      positions: ["CEO", "CFO", "CTO", "COO", "CHRO"],
    },
    { deptCode: "HR", count: 8, positions: ["TP", "PP", "HR_BP"] },
    {
      deptCode: "HR-REC",
      count: 15,
      positions: ["TN", "REC_LEAD", "RECRUITER"],
    },
    ...
    { deptCode: "FIN", count: 5, positions: ["TP", "PP"] },
    {
      deptCode: "FIN-ACC",
      count: 25,
      positions: ["ACC_MANAGER", "SENIOR_ACC", "ACC", "NV"],
    },
    ...
    { deptCode: "IT", count: 5, positions: ["TP", "PP", "ARCH"] },
    {
      deptCode: "IT-DEV",
      count: 80,
      positions: ["TECH_LEAD", "SENIOR_DEV", "DEV", "JUNIOR_DEV", "TTS"],
    },
    {
      deptCode: "IT-INF",
      count: 25,
      positions: ["TN", "DEVOPS", "SYS_ADMIN", "NV"],
    },
    ...
    { deptCode: "SALES", count: 8, positions: ["TP", "PP", "CSO"] },
    {
      deptCode: "SALES-B2B",
      count: 45,
      positions: ["SALES_MGR", "KEY_ACC_MGR", "SALES_EXEC", "SALES_REP"],
    },
    {
      deptCode: "SALES-B2C",
      count: 50,
      positions: ["SALES_MGR", "SALES_EXEC", "SALES_REP"],
    },
    {
      deptCode: "SALES-KEY",
      count: 20,
      positions: ["KEY_ACC_MGR", "SALES_EXEC"],
    },
    { deptCode: "MKT", count: 5, positions: ["CMO", "TP", "PP"] },
    {
      deptCode: "MKT-DIG",
      count: 30,
      positions: ["MKT_MGR", "DIGITAL_MKT", "CONTENT_WRITER", "DESIGNER"],
    },
    {
      deptCode: "MKT-BRD",
      count: 20,
      positions: ["CONTENT_LEAD", "CONTENT_WRITER", "DESIGNER"],
    },
    ...
    { deptCode: "OPS-LOG", count: 40, positions: ["TN", "NV", "FRESHER"] },
    {
      deptCode: "OPS-QA",
      count: 35,
      positions: ["QA_LEAD", "SENIOR_QA", "QA", "NV"],
    },
    {
      deptCode: "ADM",
      count: 25,
      positions: ["ADM_MGR", "NV", "RECEPTIONIST", "DRIVER"],
    },
  ]
  ...
  for (const dist of employeeDistribution) {
    for (let i = 0; i < dist.count; i++) {
      ...
      const baseSalary = faker.number.int({
        min: salaryRange.min,
        max: salaryRange.max,
      })
      ...
      await prisma.contract.upsert({
        where: {
          tenantId_contractNumber: { tenantId: tenant.id, contractNumber },
        },
        update: {},
        create: {
          tenantId: tenant.id,
  ...
  for (const [deptCode, deptId] of Object.entries(departmentMap)) {
    const head = allEmployees.find(
      (e) =>
        e.departmentId === deptId &&
        (e.employeeCode === "NV00001" || // CEO
          allEmployees
            .filter((emp) => emp.departmentId === deptId)
            .sort((a, b) => a.employeeCode.localeCompare(b.employeeCode))[0]
            ?.id === e.id)
    )
    if (head) {
  ...
  for (const emp of allEmployees) {
    const deptCode = Object.entries(departmentMap).find(
      ([, id]) => id === emp.departmentId
    )?.[0]
  ...
  // HR Manager
  const hrHead = allEmployees.find(
    (e) =>
      e.departmentId === departmentMap["HR"] && e.employeeCode !== "NV00001"
  )
  await prisma.user.upsert({
  ...
  const itHead = allEmployees.find(
    (e) =>
      e.departmentId === departmentMap["IT"] && e.employeeCode !== "NV00001"
  )
  await prisma.user.upsert({
    where: {
      tenantId_email: { tenantId: tenant.id, email: "manager@demo.com" },
    },
    update: {},
    create: {
      tenantId: tenant.id,
  ...
  // Regular Employee
  const regularEmp = allEmployees.find(
    (e) => e.status === "ACTIVE" && e.departmentId === departmentMap["IT-DEV"]
  )
  await prisma.user.upsert({
    where: {
      tenantId_email: { tenantId: tenant.id, email: "employee@demo.com" },
    },
    update: {},
    create: {
      tenantId: tenant.id,
  ...
  console.log(
    "✅ Created 4 demo users (admin@demo.com, hr@demo.com, manager@demo.com, employee@demo.com)"
  )
  ...
  for (let dayOffset = 30; dayOffset >= 0; dayOffset--) {
    ...
    for (const emp of activeEmployees.slice(0, 500)) {
      // First 500 active employees
      ...
      if (rand > 0.98) {
        ...
        checkIn.setHours(
          7,
          45 + faker.number.int({ min: 0, max: 15 }),
      ...
      attendanceRecords.push({
        ...
        checkIn,
        checkOut,
        checkInSource: faker.helpers.arrayElement([
          "WEB_CLOCK",
          "FINGERPRINT",
          "FACE_ID",
        ]),
        checkOutSource: faker.helpers.arrayElement([
          "WEB_CLOCK",
          "FINGERPRINT",
          "FACE_ID",
        ]),
  ...
  const salaryComponentsData = [
    ...
    },
    {
      ...
      category: "ALLOWANCE_TAXABLE" as const,
      itemType: "EARNING" as const,
      isTaxable: true,
    ...
    {
      code: "ALLOWANCE_LUNCH",
      name: "Phụ cấp ăn trưa",
      category: "ALLOWANCE_NON_TAXABLE" as const,
      itemType: "EARNING" as const,
      isTaxable: false,
      isInsuranceable: false,
      sortOrder: 4,
    ...
    },
    {
      code: "BHXH_EE",
      name: "BHXH (8%)",
      category: "INSURANCE_EMPLOYEE" as const,
    ...
    },
    {
      ...
      category: "INSURANCE_EMPLOYER" as const,
      itemType: "EMPLOYER_COST" as const,
      isTaxable: false,
      isInsuranceable: false,
      isSystem: true,
      sortOrder: 60,
  ...
  console.log(
    "✅ Created payroll config and",
    salaryComponentsData.length,
    "salary components"
  )
  ...
  const existingSteps = await prisma.workflowStep.findMany({
    where: { definitionId: leaveWorkflow.id },
  })
  if (existingSteps.length === 0) {
    await prisma.workflowStep.createMany({
      data: [
        {
          definitionId: leaveWorkflow.id,
          stepOrder: 1,
        ...
        },
        {
          ...
          conditions: { totalDays: { operator: ">=", value: 3 } },
          canSkip: true,
        },
        {
          definitionId: leaveWorkflow.id,
          stepOrder: 3,
          name: "HR Manager duyệt",
          approverType: "HR_MANAGER",
          conditions: { totalDays: { operator: ">=", value: 5 } },
          canSkip: true,
        },
      ],
    })
  ...
  const knowledgeArticlesData = [
    {
      ...
      category: "Nghỉ phép",
      keywords: ["phép năm", "annual leave"],
      content: "Nhân viên được nghỉ 12 ngày phép năm...",
    },
    {
      title: "Quy trình nghỉ thai sản",
      category: "Nghỉ phép",
      keywords: ["thai sản", "maternity"],
      content: "Chế độ nghỉ thai sản 6 tháng...",
    },
    {
      title: "Quy định chấm công",
      category: "Chấm công",
      keywords: ["chấm công", "attendance"],
      content: "Giờ làm việc 8:00-17:00...",
    },
    {
      title: "Chế độ tăng ca",
      category: "Tăng ca",
      keywords: ["tăng ca", "overtime"],
      content: "Hệ số OT: 150%, 200%, 300%...",
    },
    {
      title: "Chế độ BHXH, BHYT, BHTN",
      category: "Bảo hiểm",
      keywords: ["BHXH", "BHYT", "BHTN"],
      content: "Tỷ lệ đóng bảo hiểm...",
    },
    {
      title: "Cách xem phiếu lương",
      category: "Lương thưởng",
      keywords: ["phiếu lương", "payslip"],
      content: "Đăng nhập ESS > Phiếu lương...",
    },
    {
      title: "Quy định thuế TNCN",
      category: "Lương thưởng",
      keywords: ["thuế", "TNCN", "PIT"],
      content: "Biểu thuế lũy tiến từ 5% đến 35%...",
    },
    {
      title: "Quy trình xin nghỉ phép",
      category: "Quy định chung",
      keywords: ["xin nghỉ", "đơn phép"],
      content: "Bước 1: Đăng nhập hệ thống...",
    },
  ]
  ...
  const emailTemplatesData = [
    {
      ...
      subject: "Chào mừng {{employeeName}} đến với {{companyName}}",
      bodyHtml:
        "<h2>Xin chào {{employeeName}}!</h2><p>Chào mừng bạn đến với công ty.</p>",
    },
    {
      code: "LEAVE_APPROVED",
      name: "Đơn nghỉ phép đã duyệt",
      subject: "Đơn nghỉ phép đã được duyệt",
      bodyHtml: "<h2>Thông báo</h2><p>Đơn nghỉ phép của bạn đã được duyệt.</p>",
    },
    {
      code: "LEAVE_REJECTED",
      name: "Đơn nghỉ phép bị từ chối",
      subject: "Đơn nghỉ phép bị từ chối",
      bodyHtml: "<h2>Thông báo</h2><p>Đơn nghỉ phép của bạn bị từ chối.</p>",
    },
    {
      code: "PAYSLIP_READY",
      name: "Phiếu lương đã sẵn sàng",
      subject: "Phiếu lương tháng {{month}}/{{year}}",
      bodyHtml: "<h2>Thông báo</h2><p>Phiếu lương đã được tạo.</p>",
    },
  ]
  ...
  console.log(
    "✅ Created competency framework with",
    competencies.length,
    "competencies"
  )
  ...
  const skillCategories = [
    ...
    },
    {
      ...
      skills: ["Ra quyết định", "Coaching", "Delegation"],
    },
  ]
