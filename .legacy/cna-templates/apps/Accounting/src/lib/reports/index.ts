...
export function buildBalanceSheet(
  ...
  const getBalance = (prefix: string): Decimal => {
    return trialBalance
      ...
      .reduce((sum, row) => {
        const closingNet = new Decimal(row.closingDebit).minus(
  ...
  const getCreditBalance = (prefix: string): Decimal => {
    return trialBalance
      ...
      .reduce((sum, row) => {
        return sum.plus(
          new Decimal(row.closingCredit).minus(new Decimal(row.closingDebit))
        )
      }, new Decimal(0))
  }

  // Current Assets
  const cashAndEquivalents = getBalance("111")
    .plus(getBalance("112"))
    .plus(getBalance("113"))
  const shortTermInvestments = getBalance("121").plus(getBalance("128"))
  const shortTermReceivables = getBalance("131")
    .plus(getBalance("133"))
    .plus(getBalance("136"))
    .plus(getBalance("138"))
    .plus(getBalance("141"))
  const inventories = getBalance("151")
    .plus(getBalance("152"))
    .plus(getBalance("153"))
    .plus(getBalance("154"))
    .plus(getBalance("155"))
  ...
  const totalCurrentAssets = cashAndEquivalents
    .plus(shortTermInvestments)
    .plus(shortTermReceivables)
    .plus(inventories)
    .plus(otherCurrentAssets)

  // Non-current Assets
  const fixedAssets = getBalance("211")
    .plus(getBalance("212"))
    .plus(getBalance("213"))
    .minus(getCreditBalance("214"))
  const investmentProperty = getBalance("217")
  const longTermInvestments = getBalance("221")
    .plus(getBalance("222"))
    .plus(getBalance("228"))
    .minus(getCreditBalance("229"))
  const longTermReceivables = new Decimal(0)
  const otherNCA = getBalance("241")
    .plus(getBalance("242"))
    .plus(getBalance("243"))
    .plus(getBalance("244"))
  const totalNonCurrentAssets = fixedAssets
    ...
    .plus(longTermReceivables)
    .plus(otherNCA)

  const totalAssets = totalCurrentAssets.plus(totalNonCurrentAssets)

  // Liabilities
  const currentLiabilities = getCreditBalance("331")
    .plus(getCreditBalance("333"))
    .plus(getCreditBalance("334"))
    .plus(getCreditBalance("335"))
    .plus(getCreditBalance("336"))
    .plus(getCreditBalance("337"))
    .plus(getCreditBalance("338"))
    .plus(getCreditBalance("352"))
    .plus(getCreditBalance("353"))
  const nonCurrentLiabilities = getCreditBalance("341")
    .plus(getCreditBalance("343"))
    .plus(getCreditBalance("344"))
    .plus(getCreditBalance("347"))
    .plus(getCreditBalance("356"))
  const totalLiabilities = currentLiabilities.plus(nonCurrentLiabilities)

  // Equity
  const ownersEquity = getCreditBalance("411")
  const reserves = getCreditBalance("414")
    .plus(getCreditBalance("417"))
    .plus(getCreditBalance("418"))
  const retainedEarnings = getCreditBalance("421")
  const otherEquity = getCreditBalance("412")
    .plus(getCreditBalance("413"))
    .minus(getBalance("419"))
...
export function buildIncomeStatement(
  ...
  const getPeriodCredit = (prefix: string): Decimal => {
    return trialBalance
      .filter((row) => row.accountNumber.startsWith(prefix))
      .reduce(
        (sum, row) => sum.plus(new Decimal(row.periodCredit)),
        new Decimal(0)
      )
  }
  ...
  const getPeriodDebit = (prefix: string): Decimal => {
    return trialBalance
      .filter((row) => row.accountNumber.startsWith(prefix))
      .reduce(
        (sum, row) => sum.plus(new Decimal(row.periodDebit)),
        new Decimal(0)
      )
  }
...
export function buildCashFlowStatement(
  ...
  const receivablesChange = balanceSheetEnd.currentAssets.shortTermReceivables
    .minus(balanceSheetStart.currentAssets.shortTermReceivables)
    .negated()
  const inventoryChange = balanceSheetEnd.currentAssets.inventories
    .minus(balanceSheetStart.currentAssets.inventories)
    .negated()
  const payablesChange = balanceSheetEnd.liabilities.currentLiabilities.minus(
    balanceSheetStart.liabilities.currentLiabilities
  )
  ...
  const fixedAssetChange = balanceSheetEnd.nonCurrentAssets.fixedAssets
    .minus(balanceSheetStart.nonCurrentAssets.fixedAssets)
    .negated()
  const investingActivities = fixedAssetChange

  // Financing (simplified)
  const equityChange = balanceSheetEnd.equity.totalEquity.minus(
    balanceSheetStart.equity.totalEquity
  )
  const debtChange = balanceSheetEnd.liabilities.nonCurrentLiabilities.minus(
    balanceSheetStart.liabilities.nonCurrentLiabilities
  )
  const financingActivities = equityChange.plus(debtChange)

  const netCashChange = operatingActivities
    .plus(investingActivities)
    .plus(financingActivities)
