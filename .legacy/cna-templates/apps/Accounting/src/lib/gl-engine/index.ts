 ...
 */
export function validateJournalEntry(entry: GLJournalEntry): {
  valid: boolean
  errors: string[]
} {
  const errors: string[] = []
  ...
  if (!entry.lines || entry.lines.length < 2) {
    errors.push(
      "Bút toán phải có ít nhất 2 dòng (Journal entry must have at least 2 lines)"
    )
  }
  ...
  for (let i = 0; i < (entry.lines?.length || 0); i++) {
    ...
    if (!line.accountId && !line.accountNumber) {
      errors.push(
        `Dòng ${i + 1}: Thiếu tài khoản (Line ${i + 1}: Account is required)`
      )
    }
    ...
    if (debit.isNegative()) {
      errors.push(
        `Dòng ${i + 1}: Số tiền Nợ không được âm (Debit amount cannot be negative)`
      )
    }

    if (credit.isNegative()) {
      errors.push(
        `Dòng ${i + 1}: Số tiền Có không được âm (Credit amount cannot be negative)`
      )
    }

    if (debit.isZero() && credit.isZero()) {
      errors.push(
        `Dòng ${i + 1}: Phải có Nợ hoặc Có (Must have debit or credit)`
      )
    }

    if (debit.greaterThan(0) && credit.greaterThan(0)) {
      errors.push(
        `Dòng ${i + 1}: Không được vừa Nợ vừa Có (Cannot have both debit and credit)`
      )
    }
  }
...
export function buildTrialBalance(
  ...
  return accounts.map((acc) => {
    ...
    const openingDebit =
      acc.normalBalance === "DEBIT" && opening.greaterThan(0)
        ? opening
    ...
    const openingCredit =
      acc.normalBalance === "CREDIT" && opening.greaterThan(0)
        ? opening
    ...
    const closingDebit =
      closingBalance.greaterThanOrEqualTo(0) && acc.normalBalance === "DEBIT"
        ? closingBalance
    ...
    const closingCredit =
      closingBalance.greaterThanOrEqualTo(0) && acc.normalBalance === "CREDIT"
        ? closingBalance
