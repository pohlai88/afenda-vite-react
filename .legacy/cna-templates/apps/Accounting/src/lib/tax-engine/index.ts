...
export function calculateVATDeclaration(
  ...
  const vatPayable = totalOutputVAT
    .minus(deductibleInputVAT)
    .minus(carryForward)
  ...
  return {
    ...
    carryForwardAmount: vatPayable.lessThan(0)
      ? vatPayable.abs()
      : new Decimal(0),
...
export const CIT_STANDARD_RATE = 0.2 // 20%
 ...
 */
export function calculateCITDeclaration(
  input: CITDeclarationInput
): CITDeclarationResult {
...
export function calculatePIT(input: PITCalculationInput): PITCalculationResult {
  ...
  const dependentDed = new Decimal(PIT_DEPENDENT_DEDUCTION).times(
  ...
  let assessableIncome = taxableIncome
    .minus(insurance)
    .minus(personalDed)
    .minus(dependentDed)
