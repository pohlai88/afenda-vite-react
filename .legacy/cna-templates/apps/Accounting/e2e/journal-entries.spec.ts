...
test.describe("Journal Entries (Bút toán Nhật ký)", () => {
  test("tạo bút toán nhật ký - create journal entry with balanced debit/credit", async ({
    adminPage,
  }) => {
    ...
    await expect(
      adminPage.getByRole("heading", { name: /bút toán|journal entry/i })
    ...
    const dateInput = adminPage
      .locator('input[type="date"], input[placeholder*="Ngày"]')
      .first()
    ...
    const descInput = adminPage
      .locator(
        'input[placeholder*="Mô tả"], input[placeholder*="Description"], textarea'
      )
      .first()
    await descInput.fill(entryData.description)

    // Fill first line item - Debit
    const accountInput1 = adminPage
      .locator('input[placeholder*="Tài khoản"], input[placeholder*="Account"]')
      .nth(0)
    ...
    const debitInput = adminPage
      .locator('input[placeholder*="Nợ"], input[placeholder*="Debit"]')
      .first()
    await debitInput.fill(entryData.entries[0].debit.toString())

    // Add second line item - Credit
    const addButton = adminPage
      .getByRole("button", { name: /thêm|add|dòng/i })
      .first()
    if (await addButton.isVisible()) {
      await addButton.click()
    }

    const accountInput2 = adminPage
      .locator('input[placeholder*="Tài khoản"], input[placeholder*="Account"]')
      .nth(1)
    await adminPage.waitForTimeout(500)
    ...
    const creditInput = adminPage
      .locator('input[placeholder*="Có"], input[placeholder*="Credit"]')
      .first()
    ...
    const responsePromise = adminPage
      .waitForResponse(
        (resp) =>
          resp.url().includes("/api/journal-entries") &&
  ...
  test("validation: tổng nợ phải bằng tổng có - validate debit equals credit", async ({
    adminPage,
  }) => {
    await adminPage.goto("/accounting/journal-entries/new")
    ...
    const descInput = adminPage
      .locator('textarea, input[placeholder*="Mô tả"]')
      .first()
    ...
    if (await errorMessage.isVisible({ timeout: 5_000 }).catch(() => false)) {
      ...
      expect(await errorMessage.textContent()).toContain(
        /cân bằng|balance|không bằng|không equal/i
      )
    }

    // Should remain on form page
    await expect(adminPage).toHaveURL(
      /.*journal-entries.*new|.*journal-entries.*\d+\/edit/,
      { timeout: 5_000 }
    )
  })
  ...
  test("duyệt bút toán - approve journal entry", async ({ adminPage }) => {
    ...
    const approveButton = firstRow
      .locator(
        'button[aria-label*="Approve"], button:has-text("Duyệt"), button:has-text("Phê duyệt")'
      )
      .first()
    ...
    if (await approveButton.isVisible().catch(() => false)) {
      ...
      await expect(
        firstRow.locator("td").filter({ hasText: /approved|đã duyệt/i })
      )
  ...
  test("huỷ bút toán - cancel journal entry", async ({ adminPage }) => {
    ...
    const cancelButton = firstRow
      .locator(
        'button[aria-label*="Cancel"], button:has-text("Huỷ"), button:has-text("Xoá")'
      )
      .first()
    ...
    if (await cancelButton.isVisible().catch(() => false)) {
      ...
      const confirmButton = adminPage
        .locator(
          'button:has-text("Xác nhận"), button:has-text("Confirm"), button[role="dialog"] button'
        )
        .nth(1)
      if (
      ...
      await expect(
        firstRow
          .locator("td")
          .filter({ hasText: /cancelled|đã huỷ|cancelled/i })
      )
        .toBeVisible({ timeout: 5_000 })
        .catch(() => {})
    }
  ...
  test("tìm kiếm theo ngày - search by date", async ({ adminPage }) => {
    ...
    const dateFilterInput = adminPage
      .locator('input[type="date"], input[placeholder*="Ngày"]')
      .first()
