...
test.describe("Chart of Accounts (TT200 Compliant)", () => {
  test("hiển thị danh sách tài khoản - display chart of accounts list", async ({
    adminPage,
  }) => {
    ...
    // Verify page header
    await expect(
      adminPage.getByRole("heading", { name: /tài khoản|chart of accounts/i })
  ...
  test("tạo tài khoản mới theo TT200 - create new account per TT200", async ({
    adminPage,
  }) => {
    ...
    await expect(
      adminPage.getByRole("heading", {
        name: /thêm.*tài khoản|create account/i,
      })
    ).toBeVisible({ timeout: 10_000 })
    ...
    await adminPage
      .locator(
        'input[placeholder*="Mã tài khoản"], input[placeholder*="Account Code"]'
      )
      .first()
      .fill(testAccount.code)

    // Fill account name
    await adminPage
      .locator(
        'input[placeholder*="Tên tài khoản"], input[placeholder*="Account Name"]'
      )
    ...
    await adminPage
      .locator('[role="option"]')
      .filter({ hasText: /ASSET|TÀI SẢN/i })
      .first()
      .click()
    ...
    const responsePromise = adminPage
      .waitForResponse(
        (resp) =>
          resp.url().includes("/api/chart-of-accounts") &&
    ...
    await expect(adminPage).toHaveURL(/.*chart-of-accounts/, {
      timeout: 10_000,
    })
  })
  ...
  test("sửa tài khoản - edit account", async ({ adminPage }) => {
    ...
    const editButton = firstRow
      .locator(
        'button[aria-label*="Edit"], button[title*="Sửa"], a:has-text("Sửa")'
      )
      .first()
    ...
    await expect(
      adminPage
        .locator('input[placeholder*="Tên"], input[placeholder*="Name"]')
        .first()
    ).toBeVisible({ timeout: 10_000 })
    ...
    await adminPage
      .getByRole("button", { name: /cập nhật|update|save/i })
      .click()
  ...
  test("hiển thị cây tài khoản - display chart tree view", async ({
    adminPage,
  }) => {
    await adminPage.goto("/accounting/chart-of-accounts")

    // Look for tree view toggle button
    const treeViewButton = adminPage
      .locator('button[aria-label*="Tree"], button:has-text("Cây")')
      .first()
    ...
    if (await treeViewButton.isVisible().catch(() => false)) {
      ...
      await expect(
        adminPage.locator('[role="tree"], .tree-view, .account-tree').first()
      ).toBeVisible({ timeout: 10_000 })

      // Verify hierarchical structure (look for indentation or tree nodes)
      const treeNodes = adminPage.locator(
        '[role="treeitem"], .tree-node, li.account-item'
      )
