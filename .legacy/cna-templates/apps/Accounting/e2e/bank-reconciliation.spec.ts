...
test.describe("Bank Reconciliation (Đối soát Ngân hàng)", () => {
  test("nhập sao kê ngân hàng - import bank statement", async ({
    adminPage,
  }) => {
    ...
    await expect(
      adminPage.getByRole("heading", {
        name: /đối soát|bank reconciliation|sao kê/i,
      })
    ).toBeVisible({ timeout: 10_000 })

    // Look for import button
    const importButton = adminPage
      .getByRole("button", { name: /nhập|import|tải/i })
      .first()
    if (await importButton.isVisible()) {
      ...
      await expect(
        adminPage.locator('input[type="file"], textarea').first()
      ...
      // Fill bank name
      const bankInput = adminPage
        .locator('input[placeholder*="Ngân hàng"], input[placeholder*="Bank"]')
        .first()
      ...
      // Fill account number
      const accountInput = adminPage
        .locator(
          'input[placeholder*="Tài khoản"], input[placeholder*="Account"]'
        )
        .first()
      ...
      const openingInput = adminPage
        .locator('input[placeholder*="Số dư"], input[placeholder*="Opening"]')
        .first()
      ...
      const submitButton = adminPage
        .getRole("button", { name: /nhập|import|tải/i })
        .nth(1)
      if (await submitButton.isVisible()) {
        const responsePromise = adminPage
          .waitForResponse(
            (resp) =>
              resp.url().includes("/api/bank-reconciliation") &&
        ...
        if (response && response.status() >= 400) {
          ...
          throw new Error(
            `Bank statement import failed: ${JSON.stringify(body)}`
          )
        }
      }
    }
  ...
  test("đối soát tự động - auto reconciliation", async ({ adminPage }) => {
    ...
    await expect(adminPage.locator("table, button").first()).toBeVisible({
      timeout: 10_000,
    })

    // Look for auto-reconcile button
    const autoButton = adminPage
      .getByRole("button", { name: /tự động|auto|reconcile/i })
      .first()
  ...
  test("xử lý chênh lệch - handle discrepancies", async ({ adminPage }) => {
    ...
    const discrepancyRows = adminPage
      .locator("table tbody tr")
      .filter({ hasText: /chênh lệch|discrepanc|unmatched|không khớp/ })

    if (
      await discrepancyRows
        .first()
        .isVisible()
        .catch(() => false)
    ) {
      ...
      if (await actionButton.isVisible()) {
        ...
        const reasonInput = adminPage
          .locator(
            'textarea, input[placeholder*="Lý do"], input[placeholder*="Reason"]'
          )
          .first()
        if (
          await reasonInput.isVisible({ timeout: 3_000 }).catch(() => false)
        ) {
          await reasonInput.fill(
            `${TEST_PREFIX} Chênh lệch xử lý: Phí ngân hàng`
          )
        }

        // Submit
        const submitButton = adminPage
          .getByRole("button", { name: /xác nhận|confirm|save|lưu/i })
          .last()
        if (await submitButton.isVisible()) {
          const responsePromise = adminPage
            .waitForResponse(
          ...
          if (response && response.status() >= 400) {
            const body = await response.json().catch(() => ({}))
            throw new Error(
              `Discrepancy handling failed: ${JSON.stringify(body)}`
            )
          }
        }
      }
