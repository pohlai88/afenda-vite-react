...
test.describe("Tax Management (Quản lý Thuế)", () => {
  test("kê khai thuế GTGT - VAT tax declaration", async ({ adminPage }) => {
    ...
    // Verify page header
    await expect(
      adminPage.getByRole("heading", { name: /thuế|tax/i })
    ).toBeVisible({ timeout: 10_000 })

    // Look for VAT declaration section
    const vatSection = adminPage
      .locator("text=/GTGT|VAT|Giá trị gia tăng/i")
      .first()
    if (await vatSection.isVisible()) {
      // Click on VAT section or view button
      const viewButton = vatSection
        .locator("..")
        .locator('button:has-text("Xem"), button:has-text("View")')
        .first()
    ...
    await expect(
      adminPage.locator("input, select, textarea").first()
    ).toBeVisible({ timeout: 10_000 })
    ...
    const submitButton = adminPage
      .getByRole("button", { name: /nộp|submit|lưu/i })
      .first()
    if (await submitButton.isVisible()) {
      const responsePromise = adminPage
        .waitForResponse(
          (resp) =>
            resp.url().includes("/api/tax") &&
      ...
      // Verify success
      await expect(adminPage.locator('[role="status"], .success').first())
  ...
  test("tính thuế tự động - auto-calculate tax", async ({ adminPage }) => {
    ...
    await expect(
      adminPage.locator("input, select, button").first()
    ).toBeVisible({ timeout: 10_000 })

    // Enter taxable income
    const incomeInput = adminPage
      .locator('input[placeholder*="Doanh thu"], input[placeholder*="Income"]')
      .first()
    if (await incomeInput.isVisible()) {
      ...
      const taxResult = adminPage
        .locator('input[placeholder*="Thuế"], input[placeholder*="Tax"]')
        .nth(1)
  ...
  test("xuất báo cáo thuế - export tax report", async ({ adminPage }) => {
    ...
    await expect(adminPage.locator("button, table").first()).toBeVisible({
      timeout: 10_000,
    })
