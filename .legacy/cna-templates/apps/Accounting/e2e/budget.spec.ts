...
test.describe("Budget Management (Quản lý Ngân sách)", () => {
  test("tạo ngân sách năm - create annual budget", async ({ adminPage }) => {
    ...
    // Verify page header
    await expect(
      adminPage.getByRole("heading", { name: /ngân sách|budget/i })
    ...
    if (await createButton.isVisible()) {
      ...
      // Wait for form
      await expect(adminPage.locator("input, select").first()).toBeVisible({
        timeout: 5_000,
      })
    }
    ...
    const yearInput = adminPage
      .locator('input[type="number"], input[placeholder*="Năm"]')
      .first()
    ...
    // Fill budget amount
    const amountInput = adminPage
      .locator('input[placeholder*="Ngân sách"], input[placeholder*="Budget"]')
      .first()
  ...
  test("so sánh thực tế vs ngân sách - actual vs budget comparison", async ({
    adminPage,
  }) => {
    await adminPage.goto("/accounting/budget")

    // Wait for page to load
    await expect(adminPage.locator("table, chart, .chart").first()).toBeVisible(
      { timeout: 10_000 }
    )

    // Look for comparison view button or toggle
    const comparisonButton = adminPage
      .getByRole("button", { name: /so sánh|comparison|thực tế/i })
      .first()
  ...
  test("cảnh báo vượt ngân sách - budget overrun alert", async ({
    adminPage,
  }) => {
    await adminPage.goto("/accounting/budget")
    ...
    const alertRows = adminPage
      .locator("table tbody tr")
      .filter({ hasText: /cảnh báo|alert|warning|vượt|over/ })
    ...
    if (
      ...
      const alertIndicator = firstAlert.locator(
        '.text-red-500, .text-yellow-500, [aria-label*="warning"], [aria-label*="alert"]'
      )
      ...
      if (await detailButton.isVisible()) {
        ...
        const detailContent = adminPage
          ...
          .first()
        if (
          await detailContent.isVisible({ timeout: 3_000 }).catch(() => false)
        ) {
          // Details shown
          expect(true).toBe(true)
        }
