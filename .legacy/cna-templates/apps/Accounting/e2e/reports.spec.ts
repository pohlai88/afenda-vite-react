...
test.describe("Financial Reports (Báo cáo Tài chính)", () => {
  test("bảng cân đối kế toán - balance sheet", async ({ adminPage }) => {
    ...
    // Verify page header
    await expect(
      adminPage.getByRole("heading", { name: /bảng cân đối|balance sheet/i })
    ).toBeVisible({ timeout: 10_000 })

    // Select period if period selector exists
    const periodInput = adminPage
      .locator('input[type="month"], input[placeholder*="Kỳ"]')
      .first()
    ...
    const totalRow = adminPage
      .locator("table")
      .locator("text=/tổng|total/i")
      .first()
  ...
  test("báo cáo kết quả kinh doanh - income statement", async ({
    adminPage,
  }) => {
    ...
    await expect(
      adminPage.getByRole("heading", {
        name: /kết quả|income statement|lợi nhuận/i,
      })
    ).toBeVisible({ timeout: 10_000 })
    ...
    const revenueSection = adminPage
      .locator("text=/Doanh thu|Revenue/i")
      .first()
    const expenseSection = adminPage.locator("text=/Chi phí|Expense/i").first()
    const netIncomeSection = adminPage
      .locator("text=/Lợi nhuận|Net Income/i")
      .first()

    if (
      (await revenueSection.isVisible()) ||
      (await expenseSection.isVisible())
    ) {
      expect(true).toBe(true)
    }

    // Verify bottom line (net income/profit)
    const bottomLine = adminPage
      .locator("table")
      .locator("text=/lợi nhuận|net income|lãi/i")
      .last()
  ...
  test("báo cáo lưu chuyển tiền tệ - cash flow statement", async ({
    adminPage,
  }) => {
    ...
    // Verify page header
    await expect(
      adminPage.getByRole("heading", { name: /lưu chuyển|cash flow|tiền tệ/i })
    ).toBeVisible({ timeout: 10_000 })
    ...
    const report = adminPage
      .locator('table, .report, [role="table"], [role="tabpanel"]')
      .first()
    await expect(report).toBeVisible({ timeout: 10_000 })

    // Verify key sections: Operating, Investing, Financing
    const operatingSection = adminPage
      .locator("text=/Hoạt động|Operating/i")
      .first()
    const investingSection = adminPage
      .locator("text=/Đầu tư|Investing/i")
      .first()
    const financingSection = adminPage
      .locator("text=/Tài chính|Financing/i")
      .first()

    if (
      (await operatingSection.isVisible()) ||
      (await investingSection.isVisible())
    ) {
      expect(true).toBe(true)
    }
  ...
  test("sổ cái tổng hợp - general ledger", async ({ adminPage }) => {
    ...
    // Verify page header
    await expect(
      adminPage.getByRole("heading", { name: /sổ cái|general ledger/i })
    ).toBeVisible({ timeout: 10_000 })
    ...
    const totalRow = adminPage
      .locator("table")
      .locator("text=/tổng|total|balance/i")
    if (
      await totalRow
        .first()
        .isVisible()
