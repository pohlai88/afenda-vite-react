...
test.describe("Invoices (Hoá đơn)", () => {
  test("tạo hoá đơn bán hàng - create sales invoice", async ({ adminPage }) => {
    ...
    await expect(
      adminPage.getByRole("heading", { name: /hoá đơn bán|sales invoice/i })
    ).toBeVisible({ timeout: 10_000 })

    const invoiceData = createSalesInvoiceData()

    // Fill invoice number
    const invoiceNoInput = adminPage
      .locator(
        'input[placeholder*="Số hoá đơn"], input[placeholder*="Invoice No"]'
      )
      .first()
    ...
    // Fill customer name
    const customerInput = adminPage
      .locator(
        'input[placeholder*="Khách hàng"], input[placeholder*="Customer"]'
      )
      .first()
    ...
    const taxIdInput = adminPage
      .locator('input[placeholder*="Mã số thuế"], input[placeholder*="Tax ID"]')
      .first()
    ...
    // Add line item
    const addItemButton = adminPage
      .getByRole("button", { name: /thêm.*hàng|add.*item|thêm dòng/i })
      .first()
    ...
    const descInput = adminPage
      .locator('input[placeholder*="Mô tả"], textarea')
      .nth(0)
    ...
    const qtyInput = adminPage
      .locator('input[placeholder*="Số lượng"], input[placeholder*="Qty"]')
      .first()
    await qtyInput.fill(invoiceData.items[0].quantity.toString())

    // Fill unit price
    const priceInput = adminPage
      .locator('input[placeholder*="Đơn giá"], input[placeholder*="Price"]')
      .first()
    ...
    await expect(adminPage).toHaveURL(/.*invoices.*(sales|list)/, {
      timeout: 10_000,
    })
  })

  test("tạo hoá đơn mua hàng - create purchase invoice", async ({
    adminPage,
  }) => {
    ...
    // Verify create form is visible
    await expect(
      adminPage.getByRole("heading", { name: /hoá đơn mua|purchase invoice/i })
    ).toBeVisible({ timeout: 10_000 })
    ...
    // Fill supplier name
    const supplierInput = adminPage
      .locator(
        'input[placeholder*="Nhà cung cấp"], input[placeholder*="Supplier"]'
      )
      .first()
    ...
    if (response && response.status() >= 400) {
      const body = await response.json().catch(() => ({}))
      throw new Error(
        `Purchase invoice creation failed: ${JSON.stringify(body)}`
      )
    }

    // Should redirect to invoices list
    await expect(adminPage).toHaveURL(/.*invoices.*(purchase|list)/, {
      timeout: 10_000,
    })
  })
  ...
  test("xuất hoá đơn điện tử - export e-invoice", async ({ adminPage }) => {
    ...
    const moreButton = firstRow
      .locator(
        'button[aria-label*="more"], button[aria-label*="Thêm"], button:has-text("...")'
      )
      .first()
    ...
    const exportButton = adminPage
      .locator(
        'button:has-text("Xuất"), button:has-text("Export"), button:has-text("Điện tử")'
      )
      .first()
    if (await exportButton.isVisible({ timeout: 3_000 }).catch(() => false)) {
      ...
      const successMsg = adminPage
        .locator('[role="status"], .success, .text-green-500')
        .first()
  ...
  test("tìm kiếm hoá đơn - search invoices", async ({ adminPage }) => {
    ...
    const searchInput = adminPage
      .locator(
        'input[placeholder*="Tìm kiếm"], input[placeholder*="Search"], input[type="search"]'
      )
      .first()
