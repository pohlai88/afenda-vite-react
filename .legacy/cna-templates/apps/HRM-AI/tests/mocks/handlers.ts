...
const employeeHandlers = [
  ...
  http.post(`${BASE_URL}/api/employees`, async ({ request }) => {
    ...
    if (
      body.employeeCode &&
      employees.some((e) => e.employeeCode === body.employeeCode)
    ) {
      return HttpResponse.json(
  ...
  http.put(`${BASE_URL}/api/employees/:id`, async ({ params, request }) => {
    ...
    if (
      body.email &&
      employees.some((e) => e.email === body.email && e.id !== params.id)
    ) {
      return HttpResponse.json(
        { error: "Email đã tồn tại trong hệ thống" },
        { status: 400 }
...
const leaveHandlers = [
  ...
  http.get(`${BASE_URL}/api/leave/balance/:employeeId`, async ({ params }) => {
    ...
    const balances = leaveBalancesMock.filter(
