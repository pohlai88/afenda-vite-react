...
} from "../../utils/security-test-client"
  ...
  describe("Credential Stuffing Prevention", () => {
    it("should not reveal if user exists", async () => {
      ...
      expect(existingBody).not.toMatch(
        /user.*not.*found|no.*user|unknown.*user/i
      )
      expect(nonExistingBody).not.toMatch(
        /user.*not.*found|no.*user|unknown.*user/i
      )
    })
    ...
    it("should return consistent timing for existing and non-existing users", async () => {
      ...
      const avgExisting =
        timings.existing.reduce((a, b) => a + b, 0) / timings.existing.length
      const avgNonExisting =
        timings.nonExisting.reduce((a, b) => a + b, 0) /
...
describe("Authentication Security - Session Management", () => {
  ...
  describe("Session Token Security", () => {
    it("should not accept unsigned/modified tokens", async () => {
      // Create a fake JWT
      const fakeToken =
        "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiJhZG1pbiIsInJvbGUiOiJBRE1JTiJ9.FAKE_SIGNATURE"
