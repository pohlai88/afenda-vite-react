...
export class LeavePage {
  ...
  constructor(page: Page) {
    ...
    this.requestLeaveButton = page.locator(
      '[data-testid="request-leave-button"], button:has-text("Yêu cầu nghỉ phép")'
    )

    // Balance Cards
    this.balanceCards = page.locator('[data-testid="leave-balance-cards"]')
    this.annualLeaveBalance = page.locator(
      '[data-testid="annual-leave-balance"], [data-type="ANNUAL"]'
    )
    this.sickLeaveBalance = page.locator(
      '[data-testid="sick-leave-balance"], [data-type="SICK"]'
    )
    this.unpaidLeaveBalance = page.locator(
      '[data-testid="unpaid-leave-balance"], [data-type="UNPAID"]'
    )

    // Tabs
    this.pendingTab = page.locator(
      '[data-testid="pending-tab"], [role="tab"]:has-text("Chờ duyệt")'
    )
    this.approvedTab = page.locator(
      '[data-testid="approved-tab"], [role="tab"]:has-text("Đã duyệt")'
    )
    this.rejectedTab = page.locator(
      '[data-testid="rejected-tab"], [role="tab"]:has-text("Từ chối")'
    )
    this.allTab = page.locator(
      '[data-testid="all-tab"], [role="tab"]:has-text("Tất cả")'
    )
  ...
  async getLeaveBalance(
    type: "ANNUAL" | "SICK" | "UNPAID" | "MATERNITY" | "WEDDING"
  ): Promise<{
    entitled: number
    used: number
    balance: number
    ...
    const entitled = await card
      .locator('[data-testid="entitled"]')
      .textContent()
...
export class LeaveRequestDialog {
  ...
  constructor(page: Page) {
    ...
    // Form fields
    this.leaveTypeSelect = this.dialog.locator(
      '[data-testid="leave-type-select"], [name="leaveType"]'
    )
    this.startDateInput = this.dialog.locator(
      '[data-testid="start-date-input"], [name="startDate"]'
    )
    this.endDateInput = this.dialog.locator(
      '[data-testid="end-date-input"], [name="endDate"]'
    )
    this.halfDayCheckbox = this.dialog.locator(
      '[data-testid="half-day-checkbox"], [name="isHalfDay"]'
    )
    this.reasonInput = this.dialog.locator(
      '[data-testid="reason-input"], [name="reason"], textarea'
    )
    this.attachmentInput = this.dialog.locator(
      '[data-testid="attachment-input"], input[type="file"]'
    )

    // Summary
    this.totalDays = this.dialog.locator('[data-testid="total-days"]')
    this.remainingBalance = this.dialog.locator(
      '[data-testid="remaining-balance"]'
    )

    // Actions
    this.submitButton = this.dialog.locator(
      'button:has-text("Gửi yêu cầu"), [data-testid="submit-button"]'
    )
    ...
    this.warningMessage = this.dialog.locator(
      '[data-testid="warning-message"], .warning-message'
    )
  }
  ...
  async fillForm(data: {
    ...
    await this.page
      .locator(`[role="option"]:has-text("${data.leaveType}")`)
      .click()
...
export class LeaveApprovalPage {
  ...
  async rejectRequest(index: number, reason: string) {
    ...
    await this.page
      .locator('[data-testid="rejection-reason"], textarea')
      .fill(reason)
    await this.page.locator('button:has-text("Xác nhận")').click()
    await this.page.waitForLoadState("networkidle")
  }
