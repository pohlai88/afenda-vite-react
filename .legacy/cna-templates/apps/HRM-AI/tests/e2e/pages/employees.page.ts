...
export class EmployeesPage {
  ...
  constructor(page: Page) {
    ...
    this.addEmployeeButton = page.locator(
      '[data-testid="add-employee-button"], button:has-text("Thêm nhân viên")'
    )
    this.exportButton = page.locator(
      '[data-testid="export-button"], button:has-text("Xuất")'
    )
    this.importButton = page.locator(
      '[data-testid="import-button"], button:has-text("Nhập")'
    )

    // Search & Filter
    this.searchInput = page.locator(
      '[data-testid="search-input"], input[placeholder*="Tìm kiếm"]'
    )
    this.filterButton = page.locator(
      '[data-testid="filter-button"], button:has-text("Bộ lọc")'
    )
    this.statusFilter = page.locator('[data-testid="status-filter"]')
    this.departmentFilter = page.locator('[data-testid="department-filter"]')
    this.clearFiltersButton = page.locator(
      '[data-testid="clear-filters"], button:has-text("Xóa bộ lọc")'
    )
    ...
    this.loadingState = page.locator(
    ...
    this.prevPageButton = page.locator(
      '[data-testid="prev-page"], button:has-text("Trước")'
    )
    this.nextPageButton = page.locator(
      '[data-testid="next-page"], button:has-text("Sau")'
    )
  ...
  async deleteEmployee(employeeCode: string) {
    ...
    await row
      .locator('[data-testid="delete-button"], button:has-text("Xóa")')
      .click()
  ...
  async editEmployee(employeeCode: string) {
    const row = await this.getRowByEmployeeCode(employeeCode)
    await row
      .locator('[data-testid="edit-button"], button:has-text("Sửa")')
      .click()
    await expect(this.page.locator('[role="dialog"]')).toBeVisible()
  }
  ...
  async viewEmployeeDetail(employeeCode: string) {
    const row = await this.getRowByEmployeeCode(employeeCode)
    await row
      .locator('[data-testid="view-button"], button:has-text("Xem")')
      .click()
  ...
  async goToPage(pageNumber: number) {
    await this.page
      .locator(
        `[data-testid="page-${pageNumber}"], button:has-text("${pageNumber}")`
      )
      .click()
    await this.page.waitForLoadState("networkidle")
  }
  ...
  async expectEmployeeInList(employeeCode: string) {
    await expect(
      this.page.locator(`tr:has-text("${employeeCode}")`)
    ).toBeVisible()
  }
...
export class EmployeeFormDialog {
  ...
  constructor(page: Page) {
    ...
    // Form fields
    this.employeeCodeInput = this.dialog.locator(
      '[name="employeeCode"], [data-testid="employee-code-input"]'
    )
    this.firstNameInput = this.dialog.locator(
      '[name="firstName"], [data-testid="first-name-input"]'
    )
    this.lastNameInput = this.dialog.locator(
      '[name="lastName"], [data-testid="last-name-input"]'
    )
    this.emailInput = this.dialog.locator(
      '[name="email"], [data-testid="email-input"]'
    )
    this.phoneInput = this.dialog.locator(
      '[name="phone"], [data-testid="phone-input"]'
    )
    this.departmentSelect = this.dialog.locator(
      '[data-testid="department-select"]'
    )
    this.positionSelect = this.dialog.locator('[data-testid="position-select"]')
    this.hireDateInput = this.dialog.locator(
      '[name="hireDate"], [data-testid="hire-date-input"]'
    )
    this.statusSelect = this.dialog.locator('[data-testid="status-select"]')

    // Actions
    this.saveButton = this.dialog.locator(
      'button:has-text("Lưu"), [data-testid="save-button"]'
    )
    this.cancelButton = this.dialog.locator(
      'button:has-text("Hủy"), [data-testid="cancel-button"]'
    )

    // Messages
    this.errorMessage = this.dialog.locator(
  ...
  async fillForm(data: {
    ...
    if (data.department) {
      ...
      await this.page
        .locator(`[role="option"]:has-text("${data.department}")`)
        .click()
    }
    ...
    if (data.position) {
      ...
      await this.page
        .locator(`[role="option"]:has-text("${data.position}")`)
        .click()
    }
    ...
    if (data.status) {
      ...
      await this.page
        .locator(`[role="option"]:has-text("${data.status}")`)
        .click()
    }
  }
  ...
  async expectValidationError(fieldName: string, message?: string) {
    const errorLocator = this.dialog.locator(
      `[data-field="${fieldName}"] .error, [name="${fieldName}"] ~ .error`
    )
