...
test.describe("Employee CRUD Operations", () => {
  ...
  test.describe("List Employees", () => {
    ...
    test("should display employee list page with table", async ({
      adminPage,
    }) => {
    ...
    test("should display correct columns", async ({ adminPage }) => {
      ...
      for (const header of headers) {
        await expect(
          adminPage.locator(`th:has-text("${header}")`)
        ).toBeVisible()
      }
    })
  ...
  test.describe("Delete Employee", () => {
    ...
    test("should show delete confirmation dialog", async ({ adminPage }) => {
      ...
      const row = await employeesPage.getRowByEmployeeCode(
        `DEL-${deleteTestId}`
      )
      await row
    ...
    test("should delete employee on confirmation", async ({ adminPage }) => {
      ...
      // Should show success message
      await expect(
        adminPage.locator("text=/xóa thành công|deleted/i")
      ).toBeVisible()
  ...
  test.describe("View Employee Detail", () => {
    ...
    test("should show employee tabs (info, contracts, attendance, etc.)", async ({
      adminPage,
    }) => {
      await employeesPage.viewEmployeeDetail("TEST-EMP-001")
      ...
      for (const tab of tabs) {
        await expect(
          adminPage.locator(`[role="tab"]:has-text("${tab}")`)
        ).toBeVisible()
      }
    })
  })
