...
test.describe("Leave Request Workflow", () => {
  ...
  test.describe("Employee: Submit Leave Request", () => {
    ...
    test("should submit annual leave request successfully", async ({
      employeePage,
    }) => {
      ...
      await expect(
        employeePage.locator("text=/yêu cầu đã được gửi|submitted/i")
      ).toBeVisible()
      ...
      await expect(
        employeePage.locator(`text=${leaveData.startDate}`)
      ).toBeVisible()
    })
    ...
    test("should validate leave balance", async () => {
      ...
      await leaveDialog.expectErrorMessage(
        /vượt quá số ngày phép|exceeds balance/i
      )
    })
    ...
    test("should validate date range", async () => {
      ...
      await leaveDialog.expectErrorMessage(
        /ngày kết thúc.*trước.*bắt đầu|end date.*before.*start/i
      )
    })
    ...
    test("should prevent overlapping leave requests", async ({
      ...
      await leaveDialog.expectErrorMessage(
        /trùng với yêu cầu khác|overlapping|conflict/i
      )
    })
  ...
  test.describe("Employee: View & Manage Requests", () => {
    ...
    test("should cancel pending request", async ({ employeePage }) => {
      ...
      if (itemCount > 0) {
        ...
        await expect(
          employeePage.locator("text=/đã hủy|cancelled/i")
        ).toBeVisible()
      }
    })
  })
  ...
  test.describe("Manager: Approve/Reject Requests", () => {
    ...
    test("should display pending requests for approval", async ({
      managerPage,
    }) => {
      await expect(
        managerPage.locator('h1, [data-testid="page-title"]')
      ).toBeVisible()
    ...
    test("should approve leave request", async ({ managerPage }) => {
      ...
      if (pendingCount > 0) {
        ...
        // Should show success message
        await expect(
          managerPage.locator("text=/đã phê duyệt|approved/i")
        ).toBeVisible()
      }
    })
    ...
    test("should reject leave request with reason", async ({ managerPage }) => {
      ...
      if (pendingCount > 0) {
        await approvalPage.rejectRequest(
          0,
        ...
        // Should show success message
        await expect(
          managerPage.locator("text=/đã từ chối|rejected/i")
        ).toBeVisible()
      }
    })
    ...
    test("should require rejection reason", async ({ managerPage }) => {
      ...
      if (pendingCount > 0) {
        ...
        await expect(
          managerPage.locator("text=/lý do.*bắt buộc|reason.*required/i")
        ).toBeVisible()
      }
    })

    test("should view request details before approval", async ({
      managerPage,
    }) => {
      const pendingCount = await approvalPage.getPendingCount()
      ...
      if (pendingCount > 0) {
        ...
        await expect(
          managerPage.locator("text=/thông tin yêu cầu|request details/i")
        ).toBeVisible()
        await expect(
          managerPage.locator("text=/loại nghỉ phép|leave type/i")
        ).toBeVisible()
  ...
  test.describe("Full Workflow", () => {
    test("complete leave request approval workflow", async ({ browser }) => {
      ...
      await expect(
        employeePage.locator("text=/đã được phê duyệt|approved/i")
      ).toBeVisible()
    ...
    test("complete leave request rejection workflow", async ({ browser }) => {
      ...
      await employeePage.goto("/notifications")
      await expect(
        employeePage.locator("text=/đã bị từ chối|rejected/i")
      ).toBeVisible()
  ...
  test.describe("Leave Calendar", () => {
    test("should display team leave calendar", async ({ managerPage }) => {
      ...
      await expect(
        managerPage.locator('[data-testid="leave-calendar"], .calendar')
      ).toBeVisible()
    })
    ...
    test("should show leave events on calendar", async ({ managerPage }) => {
      ...
      await expect(
        managerPage.locator('[data-testid="calendar-event"], .calendar-event')
      ).toBeVisible()
    })
  })
  ...
  test.describe("Leave Reports (Admin)", () => {
    test("should view leave summary report", async ({ adminPage }) => {
      ...
      await expect(
        adminPage.locator('h1:has-text("Báo cáo nghỉ phép")')
      ).toBeVisible()
    })
