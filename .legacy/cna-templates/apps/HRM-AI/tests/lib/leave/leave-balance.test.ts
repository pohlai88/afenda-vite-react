...
describe("Leave Balance Logic", () => {
  ...
  describe("Balance Calculation", () => {
    function getAvailable(balance: LeaveBalance): number {
      return (
        balance.entitled + balance.carryOver - balance.used - balance.pending
      )
    }
  ...
  describe("Leave Validation", () => {
    ...
    function validateLeaveRequest(
      ...
      if (request.days > available) {
        errors.push(
          `Không đủ số ngày phép (còn lại: ${available}, yêu cầu: ${request.days})`
        )
      }
      ...
      if (request.days > policy.maxConsecutiveDays) {
        errors.push(
          `Vượt quá số ngày nghỉ liên tiếp tối đa (${policy.maxConsecutiveDays} ngày)`
        )
      }
      ...
      const daysDiff = Math.ceil(
        (request.startDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24)
      )
    ...
    it("should reject exceeding max consecutive days", () => {
      ...
      const result = validateLeaveRequest(request, balance, defaultPolicy)
      expect(result.valid).toBe(false)
      expect(result.errors.some((e) => e.includes("ngày nghỉ liên tiếp"))).toBe(
        true
      )
    })
  ...
  describe("Leave Type Specific Rules", () => {
    function getLeaveTypeConfig(type: LeaveType): {
      ...
      const configs: Record<
        LeaveType,
        { requiresDocument: boolean; maxDays: number; isPaid: boolean }
      > = {
  ...
  describe("Accrual Calculation", () => {
    ...
    function calculateAccrual(
      ...
      const totalPeriods =
        frequency === "MONTHLY" ? 12 : frequency === "BIWEEKLY" ? 26 : 1
    ...
    function calculateSummary(balances: LeaveBalance[]): BalanceSummary {
      return {
        ...
        totalAvailable: balances.reduce(
          (sum, b) => sum + b.entitled + b.carryOver - b.used - b.pending,
          0
        ),
        byType: balances,
      }
