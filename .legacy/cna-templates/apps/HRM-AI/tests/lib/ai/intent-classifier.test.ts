...
describe("AI Intent Classifier", () => {
  ...
  describe("Keyword-based Classification", () => {
    ...
    it("should classify leave queries", () => {
      expect(classifyByKeywords("Tôi còn bao nhiêu ngày phép?")).toBe(
        "LEAVE_QUERY"
      )
      expect(classifyByKeywords("Xin nghỉ ngày mai được không?")).toBe(
        "LEAVE_QUERY"
      )
    ...
    it("should classify attendance queries", () => {
      expect(classifyByKeywords("Tôi đã chấm công hôm nay chưa?")).toBe(
        "ATTENDANCE_QUERY"
      )
      expect(classifyByKeywords("Check-in giờ nào?")).toBe("ATTENDANCE_QUERY")
      expect(classifyByKeywords("Attendance summary tuần này")).toBe(
        "ATTENDANCE_QUERY"
      )
    })

    it("should classify payroll queries", () => {
      expect(classifyByKeywords("Lương tháng này là bao nhiêu?")).toBe(
        "PAYROLL_QUERY"
      )
      expect(classifyByKeywords("Thuế TNCN được tính như nào?")).toBe(
        "PAYROLL_QUERY"
      )
    ...
    it("should classify policy queries", () => {
      ...
      expect(classifyByKeywords("Quy định về thời gian làm việc?")).toBe(
        "POLICY_QUERY"
      )
      expect(classifyByKeywords("Chính sách bảo mật thông tin?")).toBe(
        "POLICY_QUERY"
      )
  ...
  describe("Confidence Scoring", () => {
    ...
    it("should boost confidence for Vietnamese text", () => {
      ...
      const vietnameseConf = calculateConfidence(
        "số ngày phép",
        "LEAVE_QUERY",
        1
      )
  ...
  describe("Vietnamese Text Processing", () => {
    ...
    it("should handle mixed case", () => {
      expect(normalizeVietnamese("NGHỈ PHÉP")).toBe(
        normalizeVietnamese("nghỉ phép")
      )
    })
  })
})
