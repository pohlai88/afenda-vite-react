...
describe("canAccessEmployeeData", () => {
  it("should allow HR_STAFF to access any employee data", () => {
    expect(
      canAccessEmployeeData(Role.HR_STAFF, "user-1", "emp-2", "emp-1")
    ).toBe(true)
  })

  it("should allow employee to access their own data", () => {
    expect(
      canAccessEmployeeData(Role.EMPLOYEE, "user-1", "emp-1", "emp-1")
    ).toBe(true)
  })

  it("should NOT allow employee to access other employee data", () => {
    expect(
      canAccessEmployeeData(Role.EMPLOYEE, "user-1", "emp-2", "emp-1")
    ).toBe(false)
  })

  it("should allow ADMIN to access any employee data", () => {
    expect(canAccessEmployeeData(Role.ADMIN, "admin-1", "emp-999", null)).toBe(
      true
    )
  })
})
