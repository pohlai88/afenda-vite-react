...
describe("Overtime Service", () => {
  ...
  describe("findAll", () => {
    it("should return paginated overtime requests", async () => {
      ...
      const result = await overtimeService.findAll("tenant-1", {
        page: 1,
        pageSize: 20,
      })
  ...
  describe("create", () => {
    it("should create overtime request with calculated hours", async () => {
      vi.mocked(db.overtimeRequest.aggregate).mockResolvedValue({
        _sum: { plannedHours: 10 },
      } as never)
      vi.mocked(db.overtimeRequest.create).mockResolvedValue({
        id: "new-1",
        plannedHours: 3,
      } as never)
  ...
  describe("update", () => {
    it("should update pending request", async () => {
      ...
      vi.mocked(db.overtimeRequest.update).mockResolvedValue({
        id: "1",
      } as never)

      await overtimeService.update("tenant-1", "1", {
  ...
  describe("delete", () => {
    it("should delete pending request", async () => {
      ...
      vi.mocked(db.overtimeRequest.delete).mockResolvedValue({
        id: "1",
      } as never)

      await overtimeService.delete("tenant-1", "1")

      expect(db.overtimeRequest.delete).toHaveBeenCalledWith({
        where: { id: "1" },
      })
    })
    ...
    it("should throw if request is approved", async () => {
      ...
      await expect(overtimeService.delete("tenant-1", "1")).rejects.toThrow(
        "Không thể xóa đơn tăng ca đã được duyệt"
      )
    })
  })
  ...
  describe("approve", () => {
    it("should approve pending request", async () => {
      ...
      const result = await overtimeService.approve(
        "tenant-1",
        "1",
        "approver-1"
      )
  ...
  describe("cancel", () => {
    ...
    it("should throw if approved", async () => {
      ...
      await expect(overtimeService.cancel("tenant-1", "1")).rejects.toThrow(
        "Không thể hủy đơn tăng ca đã được duyệt"
      )
    })
  })
  ...
  describe("bulkApprove", () => {
    it("should approve multiple pending requests", async () => {
      vi.mocked(db.overtimeRequest.updateMany).mockResolvedValue({
        count: 3,
      } as never)

      await overtimeService.bulkApprove(
        "tenant-1",
        ["1", "2", "3"],
        "approver-1"
      )
  ...
  describe("getEmployeeOTStats", () => {
    it("should calculate OT statistics for an employee", async () => {
      ...
      const result = await overtimeService.getEmployeeOTStats(
        "tenant-1",
        "emp-1",
        2026,
        1
  ...
  describe("getMyPendingRequests", () => {
    it("should return pending requests for an employee", async () => {
      ...
      const result = await overtimeService.getMyPendingRequests(
        "tenant-1",
        "emp-1"
      )
