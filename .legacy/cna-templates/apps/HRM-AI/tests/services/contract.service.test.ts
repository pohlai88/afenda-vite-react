...
describe("Contract Service", () => {
  ...
  describe("findAll", () => {
    it("should return paginated contracts", async () => {
      ...
      const result = await contractService.findAll("tenant-1", {
        page: 1,
        pageSize: 20,
      })
  ...
  describe("update", () => {
    it("should update contract if exists", async () => {
      ...
      vi.mocked(db.contract.update).mockResolvedValue({
        id: "1",
        baseSalary: 20000000,
      } as never)

      const result = await contractService.update("tenant-1", "1", {
        baseSalary: 20000000,
      })
    ...
    it("should throw if contract not found", async () => {
      ...
      await expect(
        contractService.update("tenant-1", "non-existent", {
          baseSalary: 20000000,
        })
  ...
  describe("delete", () => {
    ...
    it("should respect tenant isolation on delete", async () => {
      ...
      await expect(
        contractService.delete("tenant-1", "contract-from-tenant-2")
      ).rejects.toThrow()
