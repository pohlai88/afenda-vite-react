...
describe("Shift Service", () => {
  ...
  describe("create", () => {
    it("should create shift", async () => {
      vi.mocked(db.shift.create).mockResolvedValue({
        id: "new-1",
        name: "New Shift",
      } as never)
  ...
  describe("update", () => {
    it("should update shift if exists", async () => {
      ...
      const result = await shiftService.update("tenant-1", "1", {
        name: "Updated",
      })
  ...
  describe("delete", () => {
    ...
    it("should throw if shift has assignments", async () => {
      ...
      await expect(shiftService.delete("tenant-1", "1")).rejects.toThrow(
    ...
    it("should throw if shift has attendance records", async () => {
      ...
      await expect(shiftService.delete("tenant-1", "1")).rejects.toThrow(
        "Không thể xóa ca làm việc đã có dữ liệu chấm công"
      )
    })
  })
  ...
  describe("assignShift", () => {
    it("should create new shift assignment", async () => {
      vi.mocked(db.shiftAssignment.create).mockResolvedValue({
        id: "new-1",
      } as never)
    ...
    it("should remove primary flag from other assignments when assigning primary", async () => {
      vi.mocked(db.shiftAssignment.updateMany).mockResolvedValue({
        count: 1,
      } as never)
      vi.mocked(db.shiftAssignment.create).mockResolvedValue({
        id: "new-1",
  ...
  describe("deleteAssignment", () => {
    it("should delete assignment if exists", async () => {
      ...
      vi.mocked(db.shiftAssignment.delete).mockResolvedValue({
        id: "1",
      } as never)
  ...
  describe("getEmployeeShift", () => {
    it("should return shift for employee on given date", async () => {
      ...
      const result = await shiftService.getEmployeeShift(
        "emp-1",
        new Date("2026-01-15")
      )
  ...
  describe("bulkAssignShift", () => {
    it("should assign shift to multiple employees", async () => {
      ...
      vi.mocked(db.shiftAssignment.createMany).mockResolvedValue({
        count: 3,
      } as never)
