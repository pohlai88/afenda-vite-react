...
describe("Holiday Service", () => {
  ...
  describe("findByDate", () => {
    it("should find holiday matching date", async () => {
      ...
      const result = await holidayService.findByDate(
        "tenant-1",
        new Date("2026-01-01")
      )
  ...
  describe("create", () => {
    it("should create holiday with required fields", async () => {
      vi.mocked(db.holiday.create).mockResolvedValue({
        id: "new-1",
        name: "Company Day",
      } as never)
  ...
  describe("update", () => {
    it("should update holiday if exists", async () => {
      ...
      vi.mocked(db.holiday.update).mockResolvedValue({
        id: "1",
        name: "Updated",
      } as never)

      const result = await holidayService.update("tenant-1", "1", {
        name: "Updated",
      })
