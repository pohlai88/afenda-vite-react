...
describe("useEmployees Hook Logic", () => {
  ...
  describe("URL Building", () => {
    ...
    it("should build URL with page and limit", () => {
      expect(buildEmployeeUrl({ page: 1, limit: 20 })).toBe(
        "/api/employees?page=1&limit=20"
      )
    })

    it("should build URL with search", () => {
      expect(buildEmployeeUrl({ search: "nguyen" })).toBe(
  ...
  describe("Filter State Management", () => {
    ...
    function updateFilter(
      ...
      const isFilterChange =
        "search" in update || "departmentId" in update || "status" in update
      return {
        ...state,
        ...update,
        page:
          isFilterChange && !("page" in update) ? 1 : update.page || state.page,
      }
    }
  ...
  describe("Error Handling", () => {
    ...
    it("should translate error codes", () => {
      expect(getErrorMessage({ message: "Err", code: "UNAUTHORIZED" })).toBe(
  ...
  describe("Sorting", () => {
    ...
    it("should toggle to descending", () => {
      ...
      expect(toggleSort(current, "name")).toEqual({
        field: "name",
        direction: "desc",
      })
    })
    ...
    it("should reset to ascending when clicking different field", () => {
      ...
      expect(toggleSort(current, "date")).toEqual({
        field: "date",
        direction: "asc",
      })
    })
    ...
    it("should build query param", () => {
      expect(sortToQueryParam({ field: "name", direction: "asc" })).toBe(
        "name:asc"
      )
    })
