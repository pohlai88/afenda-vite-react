...
export class ComplianceChecker {
  ...
  async runFullCheck(): Promise<ComplianceReport> {
    ...
    console.log(
      "         LAC VIET HR - Compliance Checker                       "
    )
    console.log(
      "════════════════════════════════════════════════════════════════"
    )
  ...
  private async checkAccessControl(): Promise<void> {
    ...
    this.addResult(
      "OWASP-A01-01",
      unauthResponse.status === 401 ? "PASS" : "FAIL",
  ...
  private async checkInjectionPrevention(): Promise<void> {
    ...
    const sqlResponse = await this.makeRequest(
      "GET",
      `/api/employees?search=${encodeURIComponent(sqlPayload)}`
    )
    const sqlBody = await sqlResponse.text()
    const sqlVulnerable =
      sqlBody.toLowerCase().includes("sql") || sqlResponse.status === 500
    this.addResult(
      "OWASP-A03-01",
      sqlVulnerable ? "FAIL" : "PASS",
  ...
  private async checkConfiguration(): Promise<void> {
    ...
    this.addResult(
      "OWASP-A05-02",
      exposesDetails ? "FAIL" : "PASS",
      exposesDetails
        ? "Error response may expose stack traces"
  ...
  private async checkAuthentication(): Promise<void> {
    ...
    const weakPwdResponse = await this.makeRequest(
      "POST",
      "/api/auth/register",
      {
    ...
    this.addResult(
      "OWASP-A07-01",
      weakPwdResponse.status === 400 ? "PASS" : "FAIL",
      weakPwdResponse.status === 400
    ...
    this.addResult(
      "OWASP-A07-03",
      hasHttpOnly && hasSameSite ? "PASS" : "WARNING",
  ...
  private async checkIntegrity(): Promise<void> {
    ...
    this.addResult(
      "OWASP-A08-01",
      csrfResponse.status === 403 ? "PASS" : "FAIL",
  ...
  private addResult(
    ...
    this.results.push({
      ...
      recommendation:
        status === "FAIL"
          ? `Review ${check.requirement} implementation`
          : undefined,
    })
  }
  ...
  private generateReport(): ComplianceReport {
    ...
    const notApplicable = this.results.filter(
      (r) => r.status === "NOT_APPLICABLE"
    ).length
    const applicable = this.results.length - notApplicable
    const complianceScore =
      applicable > 0 ? Math.round((passed / applicable) * 100) : 0
