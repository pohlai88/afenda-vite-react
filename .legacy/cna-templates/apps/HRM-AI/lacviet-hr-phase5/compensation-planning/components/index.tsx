    ...
    <div className="space-y-6">
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              FY {cycle.fiscalYear} • {cycle.cycleType} • Effective:{" "}
              {new Date(cycle.effectiveDate).toLocaleDateString("vi-VN")}
            </p>
          </div>
          <span
            className={`px-3 py-1 rounded-full text-sm font-medium ${statusColors[cycle.status]}`}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <SummaryCard
          ...
          value={analytics.adjustmentSummary.byType
            ...
            .map((type) => (
              <div key={type.type} className="bg-gray-50 rounded-lg p-4">
                <div className="text-sm text-gray-500">
                  {formatAdjustmentType(type.type)}
                </div>
...
const CycleTimeline: React.FC<CycleTimelineProps> = ({ cycle }) => {
  const phases = [
    {
      name: "Planning",
      start: cycle.planningStartDate,
      end: cycle.planningEndDate,
      status: "PLANNING",
    },
    {
      name: "Manager Review",
      start: cycle.managerReviewStartDate,
      end: cycle.managerReviewEndDate,
      status: "REVIEW",
    },
    {
      name: "Approval",
      start: cycle.approvalStartDate,
      end: cycle.approvalEndDate,
      status: "APPROVAL",
    },
  ]
...
export const MeritMatrixGrid: React.FC<MeritMatrixGridProps> = ({
  ...
  const getCellValue = (
    perfRating: string,
    rangeId: string
  ): MeritMatrixCell | undefined => {
    return matrix.cells.find(
      (c) =>
        c.performanceRating === perfRating && c.compaRatioRangeId === rangeId
    )
  }
                      ...
                      >
                        ...
                        {cell && (
                          <div className="text-xs text-gray-500 mt-1">
                            {cell.minIncreasePercentage}% -{" "}
                            {cell.maxIncreasePercentage}%
                          </div>
                        )}
                      </div>
                    </td>
...
export const AdjustmentForm: React.FC<AdjustmentFormProps> = ({
  ...
  const newSalary =
    employee.currentSalary + (employee.currentSalary * increasePercentage) / 100
    ...
    >
      ...
      <div>
        ...
        >
          ...
          <option value={AdjustmentType.MARKET_ADJUSTMENT}>
            Market Adjustment
          </option>
          <option value={AdjustmentType.EQUITY_ADJUSTMENT}>
            Equity Adjustment
          </option>
          <option value={AdjustmentType.ONE_TIME_BONUS}>One-Time Bonus</option>
          <option value={AdjustmentType.RETENTION_BONUS}>
            Retention Bonus
          </option>
        </select>
      </div>
      ...
      {Math.abs(varianceFromMatrix) > 2 && (
        ...
        >
          ...
          <div className="text-sm">
            ...
            {varianceFromMatrix.toFixed(1)}% from recommendation. Justification
            required.
          </div>
        </div>
      )}
      ...
      {[
        AdjustmentType.ONE_TIME_BONUS,
        AdjustmentType.RETENTION_BONUS,
        AdjustmentType.SPOT_BONUS,
        ...
        <div>
          ...
          <input
            ...
            value={bonusAmount || ""}
            onChange={(e) =>
              setBonusAmount(parseInt(e.target.value) || undefined)
            }
            className="w-full border border-gray-300 rounded-md shadow-sm p-2"
          />
        </div>
          ...
          {adjustments.map((adjustment) => (
            <tr key={adjustment.id} className="hover:bg-gray-50">
              <td className="px-6 py-4">
                ...
                >
                  {(adjustment as any).employee?.firstName}{" "}
                  {(adjustment as any).employee?.lastName}
                </div>
...
const BudgetUtilizationChart: React.FC<BudgetUtilizationChartProps> = ({
  analytics,
}) => {
    ...
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="flex justify-between mt-2 text-sm">
        <span className="text-gray-600">
          Used: {formatCurrency(budgetSummary.usedBudget)} (
...
const CompaRatioDistributionChart: React.FC<
  CompaRatioDistributionChartProps
> = ({ analytics }) => {
  ...
  const maxCount = Math.max(
    ...compaRatioAnalysis.compaRatioDistribution.map((d) => d.count)
  )
