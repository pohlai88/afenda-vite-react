...
export async function createLearningPath(
  ...
  data: {
    ...
    stages?: {
      ...
      description?: string
      targetMonths?: number
      courses?: { courseId: string; isRequired?: boolean }[]
    }[]
  }
) {
      ...
      if (s.courses) {
        for (let j = 0; j < s.courses.length; j++) {
          await db.learningPathCourse.create({
            data: {
              stageId: stage.id,
              courseId: s.courses[j].courseId,
              isRequired: s.courses[j].isRequired ?? true,
              order: j,
            },
          })
        }
...
export async function getLearningPaths(
  ...
  const [paths, total] = await Promise.all([
    db.learningPath.findMany({
      where,
      include: {
        stages: { include: { courses: true }, orderBy: { order: "asc" } },
...
export async function getMyPathEnrollments(
  tenantId: string,
  employeeId: string
) {
