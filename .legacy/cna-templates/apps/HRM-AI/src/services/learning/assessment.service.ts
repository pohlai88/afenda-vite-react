...
export async function startAttempt(
  ...
  const assessment = await db.assessment.findFirst({
    where: { id: assessmentId, tenantId, isActive: true },
  })
...
export async function submitAttempt(
  ...
  for (const r of responses) {
    ...
    if (
      ...
      isCorrect =
        r.response === correctAnswer ||
        JSON.stringify(r.selectedOptions?.sort()) ===
          JSON.stringify((JSON.parse(correctAnswer || "[]") as string[]).sort())
  ...
  const percentageScore =
    (totalScore / Number(attempt.assessment.totalPoints)) * 100
