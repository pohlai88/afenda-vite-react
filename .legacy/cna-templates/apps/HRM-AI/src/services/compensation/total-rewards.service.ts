...
export async function getTotalRewardsStatement(
  tenantId: string,
  employeeId: string,
  year?: number
) {
...
export async function generateTotalRewardsStatement(
  tenantId: string,
  employeeId: string,
  year?: number
) {
  ...
  const totalBenefitsValue =
    enrollments.reduce((sum, e) => sum + Number(e.employerAmount || 0), 0) * 12
  ...
  return db.totalRewardsStatement.upsert({
    where: {
      tenantId_employeeId_year: { tenantId, employeeId, year: targetYear },
    },
    update: {
      baseSalary,
      totalAllowances,
...
export async function getTotalRewardsStatements(
  tenantId: string,
  year?: number
) {
  const targetYear = year || new Date().getFullYear()
