...
export async function upsertMeritMatrix(
  tenantId: string,
  data: {
  ...
  if (existing) {
    return db.meritMatrix.update({
      ...
      data: {
        meritIncreasePercent: data.meritIncreasePercent,
        compaRatioLabel: data.compaRatioLabel,
      },
    })
  }
...
export async function bulkUpsertMeritMatrix(
  tenantId: string,
  entries: {
    performanceRating: number
    compaRatioMin: number
