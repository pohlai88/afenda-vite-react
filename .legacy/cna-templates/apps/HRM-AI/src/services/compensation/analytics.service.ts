...
export async function getCompensationAnalytics(tenantId: string) {
  ...
  const salaries = withComp.map((e) =>
    Number(e.employeeCompensations[0].baseSalary)
  )
  ...
  const avgCompaRatio =
    compaRatios.length > 0
      ? compaRatios.reduce((s, v) => s + v, 0) / compaRatios.length
      : 0
  ...
  const deptMap = new Map<
    string,
    { headcount: number; totalSalary: number; compaRatios: number[] }
  >()
  withComp.forEach((e) => {
    ...
    if (e.employeeCompensations[0].grade) {
      entry.compaRatios.push(
        salary / Number(e.employeeCompensations[0].grade.midSalary)
      )
    }
  ...
  const departmentStats = Array.from(deptMap.entries()).map(
    ([department, data]) => ({
      ...
      avgCompaRatio:
        data.compaRatios.length > 0
          ? Number(
              (
                data.compaRatios.reduce((s, v) => s + v, 0) /
                data.compaRatios.length
              ).toFixed(2)
            )
          : 0,
    })
