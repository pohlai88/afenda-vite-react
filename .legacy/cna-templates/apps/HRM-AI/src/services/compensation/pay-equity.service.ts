...
export async function getPayEquityAnalyses(
  tenantId: string,
  departmentId?: string
) {
...
export async function runPayEquityAnalysis(
  tenantId: string,
  departmentId?: string
) {
  ...
  const getMaleSalary = (e: any) =>
    Number(e.employeeCompensations[0]?.baseSalary || 0)
  const avgMale =
    maleEmployees.length > 0
      ? maleEmployees.reduce((s, e) => s + getMaleSalary(e), 0) /
        maleEmployees.length
      : null
  const avgFemale =
    femaleEmployees.length > 0
      ? femaleEmployees.reduce((s, e) => s + getMaleSalary(e), 0) /
        femaleEmployees.length
      : null
  const genderGap =
    avgMale && avgFemale ? ((avgMale - avgFemale) / avgMale) * 100 : null

  const allSalaries = withSalary
    .map((e) => getMaleSalary(e))
    .sort((a, b) => a - b)
  const medianSalary =
