...
export async function getSalaryDistribution(
  ...
  const ranges = rangeDefinitions.map((rd) => {
    ...
    return {
      ...
      percentage:
        salaries.length > 0
          ? Math.round((count / salaries.length) * 1000) / 10
          : 0,
    }
  })
  ...
  const byDepartment = Array.from(deptMap.entries())
    .map(([department, deptSalaries]) => ({
      ...
      avgSalary: Math.round(
        deptSalaries.reduce((a, b) => a + b, 0) / deptSalaries.length
      ),
...
export async function getPayEquity(tenantId: string): Promise<PayEquityResult> {
  const employees = await db.employee.findMany({
    where: {
      tenantId,
      status: { in: ["ACTIVE", "PROBATION"] },
  ...
  const overallGap =
    maleAvg > 0 ? Math.round(((femaleAvg - maleAvg) / maleAvg) * 1000) / 10 : 0
  ...
  const gapByDepartment = Array.from(deptGenderMap.entries())
    .map(([department, data]) => {
      ...
      const gap =
        deptMaleAvg > 0
          ? Math.round(((deptFemaleAvg - deptMaleAvg) / deptMaleAvg) * 1000) /
            10
          : 0
      return {
        department,
  ...
  if (benchmarks.length > 0) {
    const positionDeptMap = new Map<
      string,
      { salaries: number[]; position: string; department: string }
    >()
    employeesWithSalary.forEach((e) => {
      const key = `${e.position}:${e.department}`
      if (!positionDeptMap.has(key)) {
        positionDeptMap.set(key, {
          salaries: [],
          position: e.position,
    ...
    positionDeptMap.forEach((data) => {
      const benchmark = benchmarks.find(
        (b) =>
          ...
          (b.department === data.department || !b.department)
      )
      if (benchmark) {
        const avgSalary =
        ...
        const compaRatio =
          marketMid > 0 ? Math.round((avgSalary / marketMid) * 100) / 100 : 0
        compaRatios.push({
