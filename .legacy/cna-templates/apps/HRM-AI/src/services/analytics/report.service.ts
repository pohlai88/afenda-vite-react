  ...
  const report = await db.analyticsReport.update({
    ...
    data: {
      ...
      ...(data.aggregations !== undefined && {
        aggregations: data.aggregations as object,
      }),
...
export async function runReport(
  ...
  const sorting =
    (report.sorting as Array<{ field: string; direction: "asc" | "desc" }>) ||
    []
  ...
  switch (report.dataSource) {
    case "employees": {
      ...
      const employees = await db.employee.findMany({
        ...
        orderBy:
          Object.keys(orderBy).length > 0 ? orderBy : { fullName: "asc" },
      })
      ...
      data = employees.map((emp) => {
        ...
        columns.forEach((col) => {
          switch (col.field) {
            case "fullName":
              row[col.field] = emp.fullName
              break
            case "employeeCode":
              row[col.field] = emp.employeeCode
              break
            case "department":
              row[col.field] = emp.department?.name || ""
              break
            case "position":
              row[col.field] = emp.position?.name || ""
              break
            case "status":
              row[col.field] = emp.status
              break
            case "hireDate":
              row[col.field] = emp.hireDate
              break
            case "gender":
              row[col.field] = emp.gender
              break
            case "phone":
              row[col.field] = emp.phone
              break
            case "workEmail":
              row[col.field] = emp.workEmail
              break
            default:
              row[col.field] = (emp as Record<string, unknown>)[col.field]
          }
        })
        return row
      })
    ...
    case "attendance": {
      ...
      data = records.map((rec) => {
        ...
        columns.forEach((col) => {
          switch (col.field) {
            case "employeeName":
              row[col.field] = rec.employee.fullName
              break
            case "employeeCode":
              row[col.field] = rec.employee.employeeCode
              break
            case "department":
              row[col.field] = rec.employee.department?.name || ""
              break
            case "date":
              row[col.field] = rec.date
              break
            case "checkIn":
              row[col.field] = rec.checkIn
              break
            case "checkOut":
              row[col.field] = rec.checkOut
              break
            case "status":
              row[col.field] = rec.status
              break
            default:
              row[col.field] = (rec as Record<string, unknown>)[col.field]
          }
        })
        return row
      })
    ...
    case "contracts": {
      ...
      data = contracts.map((contract) => {
        ...
        columns.forEach((col) => {
          switch (col.field) {
            case "employeeName":
              row[col.field] = contract.employee.fullName
              break
            case "employeeCode":
              row[col.field] = contract.employee.employeeCode
              break
            case "contractNumber":
              row[col.field] = contract.contractNumber
              break
            case "contractType":
              row[col.field] = contract.contractType
              break
            case "startDate":
              row[col.field] = contract.startDate
              break
            case "endDate":
              row[col.field] = contract.endDate
              break
            case "baseSalary":
              row[col.field] = Number(contract.baseSalary)
              break
            case "status":
              row[col.field] = contract.status
              break
            default:
              row[col.field] = (contract as Record<string, unknown>)[col.field]
          }
        })
        return row
      })
    ...
    case "payroll": {
      ...
      data = payrolls.map((payroll) => {
        ...
        columns.forEach((col) => {
          switch (col.field) {
            case "employeeName":
              row[col.field] = payroll.employee.fullName
              break
            case "employeeCode":
              row[col.field] = payroll.employee.employeeCode
              break
            case "department":
              row[col.field] = payroll.employee.department?.name || ""
              break
            case "grossSalary":
              row[col.field] = Number(payroll.grossSalary)
              break
            case "netSalary":
              row[col.field] = Number(payroll.netSalary)
              break
            default:
              row[col.field] = (payroll as Record<string, unknown>)[col.field]
          }
        })
        return row
      })
