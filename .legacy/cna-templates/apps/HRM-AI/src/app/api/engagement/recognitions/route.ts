...
import {
  createRecognition,
  listRecognitions,
} from "@/services/engagement/recognition.service"
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to list recognitions" },
      { status: 500 }
    )
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { success: true, data: recognition },
      { status: 201 }
    )
  } catch (error: unknown) {
    const message =
      error instanceof Error ? error.message : "Failed to create recognition"
