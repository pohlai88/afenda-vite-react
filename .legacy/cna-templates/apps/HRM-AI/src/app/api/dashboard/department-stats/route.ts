...
export async function GET() {
  try {
    ...
    const departmentStats = departments.map((dept) => {
      const deptEmployees = employeesByDept.filter(
        (e) => e.departmentId === dept.id
      )
      const deptGender = genderByDept.filter((g) => g.departmentId === dept.id)
      const deptNewHires =
        newHiresByDept.find((n) => n.departmentId === dept.id)?._count || 0
      const deptResignations =
        resignationsByDept.find((r) => r.departmentId === dept.id)?._count || 0

      const activeCount =
        deptEmployees.find((e) => e.status === "ACTIVE")?._count || 0
      const probationCount =
        deptEmployees.find((e) => e.status === "PROBATION")?._count || 0
      const resignedCount =
        deptEmployees.find((e) => e.status === "RESIGNED")?._count || 0

      const maleCount = deptGender.find((g) => g.gender === "MALE")?._count || 0
      const femaleCount =
        deptGender.find((g) => g.gender === "FEMALE")?._count || 0

      // Today's attendance for this department
      const deptAttendance = attendanceByDept.filter(
        (a) => a.employee?.departmentId === dept.id
      )
      const presentCount = deptAttendance.filter(
        (a) => a.status === "PRESENT"
      ).length
      const lateCount = deptAttendance.filter((a) =>
        ["LATE", "LATE_AND_EARLY"].includes(a.status)
      ).length
      const absentCount = deptAttendance.filter(
        (a) => a.status === "ABSENT"
      ).length
      const onLeaveCount = deptAttendance.filter(
        (a) => a.status === "ON_LEAVE"
      ).length
    ...
    return NextResponse.json({
      ...
      totals: {
        ...
        attendanceRate:
          totals.headcount > 0
            ? Math.round(
                ((totals.present + totals.late) / totals.headcount) * 100
              )
            : 0,
      },
