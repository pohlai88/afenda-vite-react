...
export async function GET() {
  try {
    ...
    if (employeesWithHireDate.length > 0) {
      ...
      avgTenure =
        Math.round((totalMonths / employeesWithHireDate.length) * 10) / 10
    }
    ...
    const statusDistribution = {
      ...
      terminated:
        statusStats.find((s) => s.status === "TERMINATED")?._count || 0,
    }
    ...
    if (
      ["ADMIN", "HR_MANAGER", "HR_STAFF", "MANAGER"].includes(session.user.role)
    ) {
    ...
    const turnoverRate =
      Math.round((resignationsLast12Months / avgHeadcount) * 100 * 10) / 10
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const rate =
        totalEmployees > 0
          ? Math.round((dayAttendance / totalEmployees) * 100)
          : 0
    ...
    if (latestPayrolls.length > 0) {
      ...
      const periodPayrolls = latestPayrolls.filter(
        (p) => p.periodId === latestPeriodId
      )
      totalPayroll = periodPayrolls.reduce(
