          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b">
                ...
                <th className="text-right p-3">P90</th>
              ...
              {benchmarks?.map((b: any) => (
                <tr key={b.id} className="border-b hover:bg-muted/50">
                  ...
                  <td className="p-3 text-right font-mono">
                    {b.percentile25
                      ? formatCurrency(Number(b.percentile25))
                      : "-"}
                  </td>
                  <td className="p-3 text-right font-mono">
                    {b.percentile50
                      ? formatCurrency(Number(b.percentile50))
                      : "-"}
                  </td>
                  <td className="p-3 text-right font-mono">
                    {b.percentile75
                      ? formatCurrency(Number(b.percentile75))
                      : "-"}
                  </td>
                  <td className="p-3 text-right font-mono">
                    {b.percentile90
                      ? formatCurrency(Number(b.percentile90))
                      : "-"}
                  </td>
