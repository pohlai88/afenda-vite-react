...
export default function CompensationHistoryPage() {
  ...
  useEffect(() => {
    fetch("/api/compensation/changes")
      .then((r) => r.json())
      .then((data) => {
              ...
              <CardContent className="p-4 flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <Badge variant="outline" className="text-xs">
                      {COMPENSATION_CHANGE_TYPE[change.changeType]?.label}
                    </Badge>
                ...
                <div className="text-right">
                  <p className="text-sm font-mono">
                    {formatCurrency(Number(change.previousSalary))} →{" "}
                    {formatCurrency(Number(change.newSalary))}
                  </p>
                  <p className="text-xs font-mono text-primary">
                    {formatPercent(Number(change.changePercent))}
                  </p>
                </div>
              </CardContent>
            </Card>
