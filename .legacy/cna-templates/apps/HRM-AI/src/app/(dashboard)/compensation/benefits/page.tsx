    ...
    <div className="p-6 space-y-6">
      ...
      {isLoading ? (
        ...
        <Tabs defaultValue="all">
          <TabsList>
            <TabsTrigger value="all">Tất cả ({plans?.length || 0})</TabsTrigger>
            <TabsTrigger value="mandatory">
              Bắt buộc ({mandatory.length})
            </TabsTrigger>
            <TabsTrigger value="optional">
              Tùy chọn ({optional.length})
            </TabsTrigger>
            <TabsTrigger value="perks">Đãi ngộ ({perks.length})</TabsTrigger>
          </TabsList>
          <TabsContent
            value="all"
