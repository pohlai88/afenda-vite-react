...
export default function OneOnOneDetailPage() {
  ...
  const handleAddAgenda = async () => {
    ...
    try {
      const res = await fetch(
        `/api/performance/one-on-ones/${params.id}/agenda`,
        {
          method: "POST",
  ...
  const handleComplete = async () => {
    try {
      const res = await fetch(
        `/api/performance/one-on-ones/${params.id}/complete`,
        {
          method: "POST",
        }
      )
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-zinc-400 mt-1">
            {meeting.employee?.fullName} & {meeting.manager?.fullName} -{" "}
            ...
            <div className="space-y-2">
              {meeting.actionItems.map((item, idx) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <p
                      className={`text-sm ${item.completed ? "text-zinc-500 line-through" : "text-zinc-200"}`}
                    >
                      {item.task}
                    </p>
