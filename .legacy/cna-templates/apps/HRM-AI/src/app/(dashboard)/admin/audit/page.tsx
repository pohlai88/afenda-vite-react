  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button
          variant="outline"
          onClick={() => window.open("/api/admin/export/audit", "_blank")}
        >
          Xuất Excel
        </Button>
      </div>
              ...
              <tbody>
                {logs.map((log) => (
                  <tr key={log.id} className="border-b hover:bg-muted/50">
                    ...
                    <td className="p-2">
                      <span
                        className={`px-2 py-0.5 rounded text-xs ${AUDIT_ACTIONS[log.action]?.color === "green" ? "bg-green-100 text-green-700" : AUDIT_ACTIONS[log.action]?.color === "red" ? "bg-red-100 text-red-700" : AUDIT_ACTIONS[log.action]?.color === "blue" ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-700"}`}
                      >
                        {AUDIT_ACTIONS[log.action]?.label || log.action}
                      </span>
                    </td>
                    <td className="p-2">
                      {ENTITY_TYPES[log.entityType] || log.entityType}
                    </td>
