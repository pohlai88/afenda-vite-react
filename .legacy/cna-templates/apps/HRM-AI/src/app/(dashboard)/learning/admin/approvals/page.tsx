...
export default function AdminApprovalsPage() {
  ...
  const handleApprove = async (id: string) => {
    try {
      const res = await fetch(`/api/learning/admin/approvals/${id}/approve`, {
        method: "POST",
      })
      if (res.ok) {
        setRequests((prev) =>
  ...
  const handleReject = async (id: string) => {
    try {
      const res = await fetch(`/api/learning/admin/approvals/${id}/reject`, {
        method: "POST",
      })
      if (res.ok) {
        setRequests((prev) =>
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Xem xet va phe duyet cac yeu cau dao tao
        </p>
      </div>
      ...
      <div className="flex items-center gap-2">
        <Badge variant="secondary" className="text-sm">
          {pendingRequests.length} cho duyet
        </Badge>
        <Badge variant="outline" className="text-sm">
          {processedRequests.length} da xu ly
        </Badge>
      </div>
      ...
      {pendingRequests.length === 0 && processedRequests.length === 0 ? (
        ...
        <>
          {pendingRequests.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle>Cho phe duyet</CardTitle>
              </CardHeader>
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-4 text-sm text-muted-foreground">
                          ...
                          <span className="flex items-center gap-1">
                            ...
                            {req.estimatedCost?.toLocaleString()} VND
                          </span>
                        </div>
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {req.employeeName} - {req.requestDate}
                      </p>
                    </div>
                    <Badge
                      variant={
