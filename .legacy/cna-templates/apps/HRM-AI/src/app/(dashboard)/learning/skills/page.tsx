      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Đánh giá và theo dõi phát triển kỹ năng
          </p>
        </div>
        ...
        <Card>
          <CardContent className="p-12 text-center text-muted-foreground">
            ...
            <p className="text-sm mt-2">
              Liên hệ quản lý để bắt đầu đánh giá kỹ năng
            </p>
          </CardContent>
        </Card>
      ) : (
                  ...
                  <div className="text-right">
                    <div className="flex items-center gap-1">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star
                          key={i}
                          className={`h-4 w-4 ${i < skill.currentLevel ? "text-yellow-500 fill-yellow-500" : "text-muted"}`}
                        />
                      ))}
                    </div>
                    <p className="text-xs text-muted-foreground mt-1">
                      Mục tiêu: {skill.targetLevel}/5
                    </p>
                  </div>
                </div>
                <div className="flex items-center justify-between text-sm text-muted-foreground">
                  <span>
                    Cấp độ: {skill.currentLevel}/{skill.targetLevel}
                  </span>
