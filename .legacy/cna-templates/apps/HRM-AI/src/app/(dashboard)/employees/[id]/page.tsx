    ...
    return (
      <div className="space-y-6">
        <PageHeader
          ...
          description="Nhân viên không tồn tại hoặc đã bị xóa"
        >
        ...
        <Card>
          ...
          <CardContent className="space-y-1">
            ...
            <InfoRow
              ...
              value={formatDate(employee.socialInsuranceDate)}
            />
          </CardContent>
        </Card>
                  ...
                  <div>
                    <p className="font-medium text-sm">
                      {(contract.contractNumber as string) ||
                        (contract.contractType as string)}
                    </p>
          ...
          <CardContent>
            <div className="space-y-3">
              {employee.dependents.map((dep: Record<string, unknown>) => (
                <div
                  key={dep.id as string}
                  className="flex items-center justify-between border-b pb-3 last:border-0 last:pb-0"
                >
                  <div>
                    <p className="font-medium text-sm">
                      {dep.fullName as string}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {dep.relationship as string}{" "}
                      {dep.dateOfBirth
                        ? `• ${formatDate(dep.dateOfBirth as string)}`
                        : ""}
                    </p>
                  </div>
                </div>
