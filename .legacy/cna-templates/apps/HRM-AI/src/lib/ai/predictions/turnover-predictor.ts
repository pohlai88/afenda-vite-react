...
export class TurnoverPredictor {
  ...
  private calculateTenureFactor(employee: EmployeeData): TurnoverFactor {
    ...
    if (monthsWorked < 6) {
      score = 80
      description =
        "Nhân viên mới (< 6 tháng), rủi ro cao trong giai đoạn thử việc"
  ...
  private calculateLeaveUsageFactor(employee: EmployeeData): TurnoverFactor {
    ...
    const totalEntitlement = leaveBalances.reduce(
      (sum, lb) => sum + Number(lb.entitlement),
      0
    )
    const totalUsed = leaveBalances.reduce(
      (sum, lb) => sum + Number(lb.used),
      0
    )
    const usageRate =
      totalEntitlement > 0 ? (totalUsed / totalEntitlement) * 100 : 0
   ...
   */
  private async calculateSalaryFactor(
    employee: EmployeeData
  ): Promise<TurnoverFactor> {
   ...
   */
  private async calculatePerformanceFactor(
    employee: EmployeeData
  ): Promise<TurnoverFactor> {
   ...
   */
  private async calculateManagerRelationFactor(
    employee: EmployeeData
  ): Promise<TurnoverFactor> {
    ...
    const approvedRequests =
      employee.leaveRequests?.filter((lr) => lr.status === "APPROVED").length ||
      0
    const totalRequests = employee.leaveRequests?.length || 0
    const approvalRate =
      totalRequests > 0 ? (approvedRequests / totalRequests) * 100 : 50
   ...
   */
  private calculateCareerProgressFactor(
    employee: EmployeeData
  ): TurnoverFactor {
    const hireDate = new Date(employee.hireDate)
    const yearsWorked =
...
Trả về JSON array: ["đề xuất 1", "đề xuất 2", "đề xuất 3"]`
  ...
  private calculateConfidence(factors: TurnoverFactor[]): number {
    ...
    const factorsWithData = factors.filter(
      (f) =>
        !f.description.includes("Không đủ dữ liệu") &&
        !f.description.includes("Không có")
    )
  ...
  ): TurnoverAnalysisResult["summary"] {
    const total = predictions.length
    const avgScore =
      total > 0
        ? Math.round(
            predictions.reduce((sum, p) => sum + p.riskScore, 0) / total
          )
        : 0
  ...
  private generateInsights(
    predictions: TurnoverPrediction[]
  ): TurnoverAnalysisResult["insights"] {
    ...
    for (const p of predictions) {
      const topFactor = p.factors.sort(
