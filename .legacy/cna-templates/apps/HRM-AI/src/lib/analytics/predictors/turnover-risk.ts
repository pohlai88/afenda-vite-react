...
const RISK_FACTORS = {
  TENURE: { weight: 0.2, label: "Tenure Risk" },
  ATTENDANCE: { weight: 0.15, label: "Attendance Pattern" },
  LEAVE_USAGE: { weight: 0.15, label: "Leave Usage" },
  SALARY_COMPETITIVENESS: { weight: 0.2, label: "Salary Competitiveness" },
...
export async function predictTurnoverRisk(
  ...
  try {
    ...
    const departmentTurnoverRates =
      await calculateDepartmentTurnoverRates(tenantId)
    ...
    for (const emp of employees) {
      ...
      const tenureMonths = emp.hireDate
        ? differenceInMonths(new Date(), emp.hireDate)
        : 0
...
function calculateAttendanceRisk(
  ...
  if (attendanceRate < 0.9) return 70
...
function calculateSalaryRisk(
  employeeSalary: number,
  avgPositionSalary: number
): number {
...
function calculateDepartmentTurnoverRisk(
  ...
  for (const factor of topRiskFactors) {
    ...
    switch (factor.factor) {
      ...
      case RISK_FACTORS.ATTENDANCE.label:
        recommendations.push(
          "Review attendance pattern and conduct engagement check-in"
        )
        break
      case RISK_FACTORS.LEAVE_USAGE.label:
        recommendations.push(
          "Assess work-life balance and discuss workload management"
        )
        break
      case RISK_FACTORS.SALARY_COMPETITIVENESS.label:
        recommendations.push(
          "Review compensation against market benchmarks and consider adjustment"
        )
        break
      case RISK_FACTORS.OVERTIME.label:
        recommendations.push(
          "Evaluate workload distribution and consider hiring or reassignment"
        )
        break
      case RISK_FACTORS.DEPARTMENT_TURNOVER.label:
        recommendations.push(
          "Investigate department culture and management effectiveness"
        )
        break
    }
  }
...
async function calculateDepartmentTurnoverRates(
  tenantId: string
...
async function getAvgSalaryByPosition(
  tenantId: string
): Promise<Record<string, number>> {
...
async function savePredictions(
  tenantId: string,
  predictions: TurnoverRiskPrediction[]
): Promise<void> {
  try {
    ...
    for (const prediction of predictions) {
      await db.turnoverPrediction.create({
        data: {
          ...
          riskLevel: prediction.riskLevel as
            | "LOW"
            | "MEDIUM"
            | "HIGH"
            | "CRITICAL",
...
function getLeaveRiskDescription(score: number): string {
  if (score > 60)
    return "Unusual leave usage pattern - may indicate burnout or exit planning"
...
function getSalaryRiskDescription(score: number): string {
  if (score > 70)
    return "Salary significantly below market rate - high risk of competing offers"
  if (score > 40)
    return "Salary slightly below market - monitor for competitor offers"
...
function getDepartmentRiskDescription(score: number): string {
  if (score > 60)
    return "Department has high historical turnover - systemic issues likely"
