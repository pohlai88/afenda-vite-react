...
import {
  calculateHeadcountMetrics,
  getHeadcountTrend,
} from "../calculators/headcount"
...
export async function aggregateAllMetrics(
  ...
  try {
    ...
    const [headcount, turnover, attendance, laborCost, compensation] =
      await Promise.all([
...
export async function getExecutiveDashboard(
  ...
  try {
    ...
    const [prevHeadcount, prevTurnover, prevAttendance, prevLaborCost] =
      await Promise.all([
...
async function storeMetrics(
  ...
  try {
    const metricEntries: Array<{
      metricType: "HEADCOUNT" | "TURNOVER" | "ATTENDANCE" | "LABOR_COST"
      value: number
      breakdown: unknown
    }> = [
      {
        metricType: "HEADCOUNT",
        value: metrics.headcount.total,
        breakdown: metrics.headcount,
      },
      {
        metricType: "TURNOVER",
        value: metrics.turnover.rate,
        breakdown: metrics.turnover,
      },
      {
        metricType: "ATTENDANCE",
        value: metrics.attendance.rate,
        breakdown: metrics.attendance,
      },
      {
        metricType: "LABOR_COST",
        value: metrics.laborCost.total,
        breakdown: metrics.laborCost,
      },
    ]
