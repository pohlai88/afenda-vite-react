...
export async function calculateCompensationMetrics(
  params: CompaRatioParams
...
function calculatePayEquity(
  ...
  const maleAvg =
    maleEmployees.length > 0
      ? maleEmployees.reduce((sum, e) => sum + (Number(e.baseSalary) || 0), 0) /
        maleEmployees.length
      : 0

  const femaleAvg =
    femaleEmployees.length > 0
      ? femaleEmployees.reduce(
  ...
  const genderGap =
    maleAvg > 0
      ? Math.round(((maleAvg - femaleAvg) / maleAvg) * 100 * 100) / 100
      : 0

  // By department gap
  const byDepartment: Record<string, number> = {}
  const departmentEmployees: Record<
    string,
  ...
  for (const [dept, data] of Object.entries(departmentEmployees)) {
    ...
    if (deptMaleAvg > 0 && deptFemaleAvg > 0) {
      byDepartment[dept] =
        Math.round(((deptMaleAvg - deptFemaleAvg) / deptMaleAvg) * 100 * 100) /
        100
    }
  }
...
> {
  try {
    ...
    for (const emp of employees) {
      const level =
        emp.position?.level != null ? String(emp.position.level) : "Unknown"
    ...
    for (const benchmark of benchmarks) {
      const level =
        (benchmark as { level?: string }).level ||
        (benchmark as { positionLevel?: string }).positionLevel ||
        "Unknown"
      const marketMin =
        Number(
          (benchmark as { minSalary?: unknown }).minSalary ||
            (benchmark as { marketMin?: unknown }).marketMin
        ) || 0
      const marketMax =
        Number(
          (benchmark as { maxSalary?: unknown }).maxSalary ||
            (benchmark as { marketMax?: unknown }).marketMax
        ) || 0
      const marketMid =
        Number(
          (benchmark as { midSalary?: unknown }).midSalary ||
            (benchmark as { marketMid?: unknown }).marketMid
      ...
      const ratio =
        marketMid > 0 ? Math.round((internalAvg / marketMid) * 100) / 100 : 0
