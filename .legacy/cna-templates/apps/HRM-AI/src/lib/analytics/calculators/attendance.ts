...
export async function calculateAttendanceMetrics(
  ...
  try {
    ...
    const lateRate =
      summaries.length > 0 ? (lateCount / summaries.length) * 100 : 0
    ...
    for (const [dept, data] of Object.entries(departmentData)) {
      const deptRate =
        data.workDays > 0 ? (data.actualDays / data.workDays) * 100 : 0
      const deptLateRate =
        data.count > 0 ? (data.absentDays / data.count) * 100 : 0
...
function generateHeatmapData(
  ...
  for (let day = 1; day <= 5; day++) {
    for (let hour = 7; hour <= 18; hour++) {
      ...
      const totalStandard = summaries.reduce(
        (sum, s) => sum + (Number(s.workingDays) || 0),
        0
      )
      const totalActual = summaries.reduce(
        (sum, s) => sum + (Number(s.actualWorkDays) || 0),
        0
      )
