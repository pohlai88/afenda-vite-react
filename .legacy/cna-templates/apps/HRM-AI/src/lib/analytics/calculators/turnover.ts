...
export async function calculateTurnoverMetrics(
  ...
  try {
    ...
    const endingCount = await db.employee.count({
      where: {
        ...
        OR: [{ resignationDate: null }, { resignationDate: { gt: periodEnd } }],
      },
    })
    ...
    const voluntaryCount = terminatedEmployees.filter(
      (emp) =>
        ...
        voluntaryReasons.includes(emp.resignationReason)
    ).length
    const involuntaryCount = terminatedCount - voluntaryCount

    const voluntaryRate =
      avgHeadcount > 0 ? (voluntaryCount / avgHeadcount) * 100 : 0
    const involuntaryRate =
      avgHeadcount > 0 ? (involuntaryCount / avgHeadcount) * 100 : 0
    ...
    for (const dept of departments) {
      ...
      const deptRate =
        deptHeadcount > 0 ? (deptTerminated / deptHeadcount) * 100 : 0
