...
export function useCompensationCycles(
  options: UseCompensationCyclesOptions = {}
) {
    ...
    async (page = 1) => {
      ...
      try {
        ...
        if (options.fiscalYear)
          params.append("fiscalYear", options.fiscalYear.toString())
  ...
  const updateCycleStatus = useCallback(
    async (cycleId: string, status: CompensationCycleStatus) => {
      const updated = await apiCall<CompensationCycle>(
...
export function useAdjustments(filters: AdjustmentFilters) {
  ...
  const approveAdjustment = useCallback(async (data: ApproveAdjustmentDto) => {
    const updated = await apiCall<CompensationAdjustment>(
      `/adjustments/${data.adjustmentId}/approve`,
      {
        method: "POST",
        body: JSON.stringify(data),
      }
    ...
    setAdjustments((prev) =>
      prev.map((a) => (a.id === data.adjustmentId ? updated : a))
    )
    return updated
  }, [])

  const bulkApprove = useCallback(
    async (adjustmentIds: string[], comments?: string) => {
      const result = await apiCall<{ approvedCount: number }>(
...
export function useMeritMatrix(cycleId: string | undefined) {
  ...
  const createMatrix = useCallback(
    async (data: Partial<MeritMatrix>) => {
      if (!cycleId) return null

      const created = await apiCall<MeritMatrix>(
        `/cycles/${cycleId}/merit-matrix`,
        {
          method: "POST",
          body: JSON.stringify(data),
        }
  ...
  const getRecommendation = useCallback(
    async (matrixId: string, performanceRating: string, compaRatio: number) => {
      const params = new URLSearchParams({
        performanceRating,
        compaRatio: compaRatio.toString(),
      })
      return apiCall<{ targetIncreasePercentage: number }>(
        `/merit-matrix/${matrixId}/recommend?${params}`
      )
    },
    []
  )
...
export function useBudget(cycleId: string | undefined) {
  ...
  const updateAllocation = useCallback(
    async (departmentId: string, data: Partial<any>) => {
      if (!cycleId) return
...
export function useSalaryGrades() {
  ...
  const updateGrade = useCallback(
    async (id: string, data: Partial<SalaryGrade>) => {
...
async function successionApiCall<T>(
  endpoint: string,
  options?: RequestInit
): Promise<T> {
...
export function useCriticalPositions(
  options: UseCriticalPositionsOptions = {}
) {
  ...
  const fetchPositions = useCallback(
    async (page = 1) => {
      ...
      try {
        ...
        if (options.hasSuccessors !== undefined)
          params.append("hasSuccessors", String(options.hasSuccessors))
        if (options.riskLevel) params.append("riskLevel", options.riskLevel)
        params.append("page", page.toString())

        const response = await fetch(
          `${SUCCESSION_API_BASE}/critical-positions?${params}`
        )
        const data = await response.json()
  ...
  const createPosition = useCallback(
    async (data: CreateCriticalPositionDto) => {
      const position = await successionApiCall<CriticalPosition>(
        "/critical-positions",
        {
          method: "POST",
          body: JSON.stringify(data),
  ...
  const assessRisk = useCallback(async (positionId: string) => {
    return successionApiCall<{
      vacancyRisk: RiskLevel
      benchStrength: number
      riskFactors: string[]
    }>(`/critical-positions/${positionId}/assess-risk`, { method: "POST" })
  }, [])
...
export function useTalentProfiles(filters: TalentSearchFilters = {}) {
    ...
    async (page = 1) => {
      ...
      try {
        ...
        if (filters.nineBoxPositions?.length)
          params.append("nineBoxPositions", filters.nineBoxPositions.join(","))
        if (filters.talentCategories?.length)
          params.append("talentCategories", filters.talentCategories.join(","))
        if (filters.minPerformanceRating)
          params.append(
            "minPerformanceRating",
            filters.minPerformanceRating.toString()
          )
        if (filters.flightRiskLevels?.length)
          params.append("flightRiskLevels", filters.flightRiskLevels.join(","))
        if (filters.searchTerm) params.append("searchTerm", filters.searchTerm)
        params.append("page", page.toString())

        const response = await fetch(
          `${SUCCESSION_API_BASE}/talent-profiles?${params}`
        )
        const data = await response.json()
  ...
  const updateProfile = useCallback(
    async (profileId: string, data: UpdateTalentProfileDto) => {
      const updated = await successionApiCall<TalentProfile>(
        `/talent-profiles/${profileId}`,
        {
          method: "PUT",
          body: JSON.stringify(data),
        }
...
export function useTalentProfile(employeeId: string | undefined) {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      const data = await successionApiCall<TalentProfile>(
  ...
  const update = useCallback(
    async (data: UpdateTalentProfileDto) => {
      if (!profile) return null
      const updated = await successionApiCall<TalentProfile>(
        `/talent-profiles/${profile.id}`,
        {
          method: "PUT",
          body: JSON.stringify(data),
        }
...
export function useSuccessionPlan(planId: string | undefined) {
  ...
  const addSuccessor = useCallback(
    async (data: Omit<AddSuccessorDto, "successionPlanId">) => {
      if (!planId) return null
      const successor = await successionApiCall<SuccessorCandidate>(
        `/plans/${planId}/successors`,
        {
          method: "POST",
          body: JSON.stringify(data),
        }
  ...
  const updateSuccessorReadiness = useCallback(
    async (
      successorId: string,
      readiness: SuccessorReadiness,
      notes?: string
    ) => {
      const updated = await successionApiCall<SuccessorCandidate>(
        `/successors/${successorId}/readiness`,
        {
          method: "PUT",
  ...
  const removeSuccessor = useCallback(
    async (successorId: string, reason: string) => {
...
export function useNineBox(departmentId?: string) {
  ...
  const fetchDistribution = useCallback(async () => {
    ...
    try {
      ...
      const data = await successionApiCall<typeof distribution>(
        `/nine-box/distribution${params}`
      )
  ...
  const assessEmployee = useCallback(
    async (
      ...
      const profile = await successionApiCall<TalentProfile>(
        "/nine-box/assess",
        {
          method: "POST",
          body: JSON.stringify({
...
export function useDevelopmentPlan(planId: string | undefined) {
  ...
  const createPlan = useCallback(async (data: CreateDevelopmentPlanDto) => {
    const created = await successionApiCall<DevelopmentPlan>(
      "/development-plans",
      {
        method: "POST",
        body: JSON.stringify(data),
  ...
  const addActivity = useCallback(
    async (data: Omit<any, "developmentPlanId">) => {
      if (!planId) return null
      const activity = await successionApiCall(
        `/development-plans/${planId}/activities`,
        {
          method: "POST",
          body: JSON.stringify(data),
        }
      ...
      const updated = await successionApiCall<DevelopmentPlan>(
...
export function useSuccessionDashboard() {
  const [dashboard, setDashboard] = useState<SuccessionDashboardData | null>(
    null
  )
