...
export function getCompaRatioLabel(compaRatio: number): {
  label: string
  color: string
} {
  const range = COMPA_RATIO_RANGES.find(
    (r) => compaRatio >= r.min && compaRatio < r.max
  )
  return range || { label: "N/A", color: "gray" }
}

export function getMeritIncrease(
  performanceRating: number,
  compaRatio: number,
  matrix?: typeof DEFAULT_MERIT_MATRIX
): number {
  const data = matrix || DEFAULT_MERIT_MATRIX
  const entry = data.find(
    (m) =>
      m.performanceRating === performanceRating &&
      compaRatio >= m.compaRatioMin &&
      compaRatio < m.compaRatioMax
  )
...
export function calculateTotalInsurance(
  salary: number,
  role: "employer" | "employee"
): number {
  return (
...
export function getSalaryRangePosition(
  salary: number,
  min: number,
  max: number
): number {
