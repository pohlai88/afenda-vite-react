...
export const nationalIdSchema = z
  .string()
  .regex(/^[0-9]{9}$|^[0-9]{12}$/, "CCCD/CMND phải có 9 hoặc 12 chữ số")
...
export const bankAccountSchema = z
  .string()
  .regex(/^[0-9]{6,20}$/, "Số tài khoản ngân hàng không hợp lệ")
...
export const employeeListQuerySchema = z.object({
  ...
  search: searchSchema,
  status: z
    .enum(["ACTIVE", "INACTIVE", "PROBATION", "TERMINATED", "ON_LEAVE", "ALL"])
    .optional(),
  departmentId: uuidSchema.optional(),
  positionId: uuidSchema.optional(),
  sortBy: z
    .enum(["name", "employeeCode", "hireDate", "createdAt", "department"])
    .optional(),
...
export const leaveCreateSchema = z
  ...
  .refine((data) => !data.isHalfDay || data.halfDayPeriod !== undefined, {
    message: "Phải chọn buổi sáng hoặc chiều khi nghỉ nửa ngày",
    path: ["halfDayPeriod"],
  })
  .refine((data) => !data.isHalfDay || data.startDate === data.endDate, {
    message: "Nghỉ nửa ngày chỉ áp dụng cho cùng một ngày",
    path: ["endDate"],
  })
