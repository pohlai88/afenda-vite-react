...
const invalidationStrategies: Record<EntityType, InvalidationHandler> = {
  ...
  leave: async (event) => {
    ...
    if (event.metadata?.employeeId) {
      patterns.push(
        ...InvalidationPatterns.leave(event.metadata.employeeId as string)
      )
    }
...
export class CacheInvalidator {
  ...
  async invalidate(event: InvalidationEvent): Promise<InvalidationResult> {
    ...
    try {
      ...
      if (!strategy) {
        console.warn(
          `[CacheInvalidator] No strategy for entity: ${event.entity}`
        )
        return {
          success: false,
          keysInvalidated: 0,
      ...
      async (channel: string, message: string) => {
        if (channel === "cache:invalidation") {
          try {
            ...
            console.error(
              "[CacheInvalidator] Error processing pub/sub message:",
              error
            )
          }
        }
