  ...
  constructor() {
    ...
    this.cleanupInterval = setInterval(() => {
      ...
      Array.from(this.store.entries()).forEach(([key, value]) => {
        if (
          value.windowStart + SecurityConfig.rateLimit.global.windowMs <
          now
        ) {
    ...
    try {
      ...
      if (await memoryStore.isBlocked(fullKey)) {
        ...
        return {
          ...
          remaining: 0,
          resetTime: new Date(
            blockInfo?.blockedUntil || Date.now() + this.config.windowMs
          ),
          retryAfter: Math.ceil(
            ((blockInfo?.blockedUntil || 0) - Date.now()) / 1000
          ),
        }
      }
      ...
      const isSensitive =
        this.config.keyPrefix?.includes("login") ||
