...
export async function runSecurityAudit(): Promise<SecurityAuditResult> {
  ...
  checks.push({
    ...
    recommendation:
      secretLength < 32
        ? "Use at least 32 character secret (openssl rand -base64 32)"
        : undefined,
    severity: "critical",
  })
  ...
  checks.push({
    ...
    category: "authentication",
    status: authUrl?.startsWith("https://")
      ? "PASS"
      : authUrl
        ? "WARN"
        : "FAIL",
    message: authUrl
      ? `NEXTAUTH_URL is set to ${authUrl}`
      : "NEXTAUTH_URL is not set",
    recommendation: !authUrl?.startsWith("https://")
  ...
  checks.push({
    ...
    message: vcbClientId
      ...
      : "Bank API not configured",
    recommendation: !vcbClientId
      ? "Configure bank API for payment features"
      : undefined,
    severity: "medium",
  })
  ...
  checks.push({
    ...
    message: vnptPartnerId
      ? "E-Signature API configured"
      : "E-Signature API not configured",
    recommendation: !vnptPartnerId
      ? "Configure VNPT-CA for e-signature features"
      : undefined,
    severity: "medium",
  })
  ...
  try {
    ...
    const suspiciousAdmins = adminUsers.filter(
      (u) => u.createdAt.getTime() === u.updatedAt.getTime()
    )
    ...
    checks.push({
      ...
      recommendation:
        suspiciousAdmins.length > 0
          ? "Force password change for admin accounts"
          : undefined,
      severity: "high",
    })
  } catch {
  ...
  checks.push({
    ...
    message: alertWebhook
      ? "Alert webhook is configured"
      : "Alert webhook not configured",
    recommendation: !alertWebhook
      ? "Configure ALERT_WEBHOOK_URL for production alerts"
      : undefined,
    severity: "medium",
  })
...
export function getGoLiveChecklist(): GoLiveChecklistItem[] {
  return [
    // Pre-Deployment
    {
      ...
      item: "All environment variables set",
      status: "pending",
    },
    {
      category: "Pre-Deployment",
    ...
    {
      category: "Security",
      item: "Security audit passed (score >= 80%)",
      status: "pending",
    },
    {
      category: "Security",
      item: "Admin passwords changed",
      status: "pending",
    },
    ...
    { category: "Integrations", item: "VCB sandbox tested", status: "pending" },
    {
      ...
      item: "VCB production credentials obtained",
      status: "pending",
    },
    {
      category: "Integrations",
      item: "VNPT-CA sandbox tested",
      status: "pending",
    },
    {
      category: "Integrations",
      item: "VNPT-CA production contract signed",
      status: "pending",
    },
    {
      category: "Integrations",
      item: "Attendance devices tested",
      status: "pending",
    },
    ...
    {
      category: "Monitoring",
      item: "Health check endpoint working",
      status: "pending",
    },
    { category: "Monitoring", item: "Logging configured", status: "pending" },
    {
      category: "Monitoring",
      item: "Alert webhooks set up",
      status: "pending",
    },
    {
      category: "Monitoring",
    ...
    {
      category: "Documentation",
      item: "Support contact established",
      status: "pending",
    },
  ]
}
