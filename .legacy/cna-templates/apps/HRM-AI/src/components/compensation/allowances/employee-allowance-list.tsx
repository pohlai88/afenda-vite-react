...
interface EmployeeAllowanceListProps {
  allowances: EmployeeAllowance[]
}

export function EmployeeAllowanceList({
  allowances,
}: EmployeeAllowanceListProps) {
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-medium font-mono">
            {formatCurrency(totalMonthly)}/tháng
          </span>
        </div>
      </CardHeader>
      <CardContent className="space-y-2">
        {allowances.length === 0 && (
          <p className="text-sm text-muted-foreground">Chưa có phụ cấp.</p>
        )}
        {allowances.map((a) => (
          <div
            key={a.id}
          ...
          >
            <div>
              <p className="text-sm font-medium">{a.allowanceType?.name}</p>
              <span className="text-xs text-muted-foreground">
                {ALLOWANCE_FREQUENCY[a.frequency]?.label}
              </span>
            </div>
            <span className="text-sm font-medium font-mono">
