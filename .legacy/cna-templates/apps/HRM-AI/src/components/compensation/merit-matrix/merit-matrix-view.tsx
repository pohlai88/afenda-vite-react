...
interface MeritMatrixViewProps {
  data: MeritMatrix[]
}
...
export function MeritMatrixView({ data }: MeritMatrixViewProps) {
  ...
  const getValue = (rating: number, rangeIdx: number) => {
    ...
    const entry = data.find(
      (d) =>
        d.performanceRating === rating &&
        Number(d.compaRatioMin) >= range.min - 0.01 &&
        Number(d.compaRatioMax) <= range.max + 0.01
    )
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">Ma trận Merit (5x5)</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="overflow-x-auto">
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr>
                <th className="border p-2 bg-muted text-left text-xs">
                  Hiệu suất \ Compa-ratio
                </th>
                {ranges.map((r, i) => (
                  <th
                    key={i}
                    className="border p-2 bg-muted text-center text-xs"
                  >
                    {r.label}
                    <br />
                    <span className="text-muted-foreground">
                      {r.min}-{r.max === 999 ? "∞" : r.max}
                    </span>
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {ratings.map((rating) => (
                <tr key={rating}>
                  <td className="border p-2 font-medium text-xs">
                    {rating} - {PERFORMANCE_RATING_LABELS[rating]}
                  </td>
                  {ranges.map((_, rangeIdx) => {
                    ...
                    return (
                      <td
                        key={rangeIdx}
                        className={`border p-2 text-center font-medium ${getCellColor(value)}`}
                      >
                        {value > 0 ? `${value}%` : "-"}
                      </td>
                    )
