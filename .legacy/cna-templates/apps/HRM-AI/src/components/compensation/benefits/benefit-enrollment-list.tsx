...
interface BenefitEnrollmentListProps {
  enrollments: BenefitEnrollment[]
}

export function BenefitEnrollmentList({
  enrollments,
}: BenefitEnrollmentListProps) {
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">Phúc lợi đã đăng ký</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        {enrollments.length === 0 && (
          <p className="text-sm text-muted-foreground">
            Chưa đăng ký phúc lợi nào.
          </p>
        )}
            ...
            >
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {enrollment.employerAmount
                    ? `Công ty: ${formatCurrency(enrollment.employerAmount)}`
                    : ""}
                </p>
              </div>
