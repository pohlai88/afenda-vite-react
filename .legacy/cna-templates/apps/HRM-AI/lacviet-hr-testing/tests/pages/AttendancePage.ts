...
export class AttendancePage extends BasePage {
  ...
  async checkInWithPhoto(): Promise<void> {
    ...
    await expect(
      this.page.locator('[data-testid="camera-modal"]')
    ).toBeVisible()
  ...
  async rejectOvertime(reason: string): Promise<void> {
    ...
    await this.fillInput(
      this.page.locator('[data-testid="reject-reason"]'),
      reason
    )
  ...
  async createShift(data: {
    ...
    await this.fillInput(
      this.page.locator('[data-testid="shift-name"]'),
      data.name
    )
    await this.fillInput(
      this.page.locator('[data-testid="shift-start"]'),
      data.startTime
    )
    await this.fillInput(
      this.page.locator('[data-testid="shift-end"]'),
      data.endTime
    )

    if (data.breakStart && data.breakEnd) {
      await this.fillInput(
        this.page.locator('[data-testid="shift-break-start"]'),
        data.breakStart
      )
      await this.fillInput(
        this.page.locator('[data-testid="shift-break-end"]'),
        data.breakEnd
      )
    }
  ...
  async assignShiftToEmployee(
    ...
    await this.fillInput(
      this.page.locator('[data-testid="date-from"]'),
      dateRange.from
    )
    await this.fillInput(
      this.page.locator('[data-testid="date-to"]'),
      dateRange.to
    )
  ...
  async expectLateWarning(): Promise<void> {
    await expect(
      this.page.locator('[data-testid="late-warning"]')
    ).toBeVisible()
  }
  ...
  async getWeekStartDate(): Promise<string> {
    return (
      (await this.page.locator('[data-testid="week-start"]').textContent()) ||
      ""
    )
  }
