...
test.describe("Attendance Management", () => {
  ...
  test.describe("Check-In/Check-Out", () => {
    test("@smoke should check in successfully", async () => {
      ...
      await expect(
        page.locator('[data-testid="attendance-status"]')
      ).toContainText("Đã vào ca")
    })
    ...
    test("should record GPS location on check-in", async () => {
      ...
      await page
        .context()
        .setGeolocation({ latitude: 10.762622, longitude: 106.660172 })
      ...
      await expect(
        page.locator('[data-testid="check-in-location"]')
      ).toBeVisible()
    })
    ...
    test("should show warning for late check-in", async () => {
      ...
      await expect(page.locator('[data-testid="late-warning"]')).toContainText(
        "Đi muộn"
      )
    })
    ...
    test("should show office zones for valid check-in", async () => {
      ...
      await expect(
        page.locator('[data-testid="zone-item"]')
  ...
  test.describe("Timesheet", () => {
    ...
    test("should display monthly timesheet", async () => {
      ...
      // Verify monthly view
      await expect(
        page.locator('[data-testid="timesheet-calendar"]')
      ).toBeVisible()
    ...
    test("should calculate working hours correctly", async () => {
      ...
      // Get total hours
      const totalHours = await page
        .locator('[data-testid="total-hours"]')
        .textContent()
      const regularHours = await page
        .locator('[data-testid="regular-hours"]')
        .textContent()
      const overtimeHours = await page
        .locator('[data-testid="overtime-hours"]')
        .textContent()
    ...
    test("should navigate between weeks", async () => {
      ...
      const currentStartDate = await page
        .locator('[data-testid="week-start"]')
        .textContent()

      // Go to previous week
      await page.locator('[data-testid="prev-week"]').click()

      const prevStartDate = await page
        .locator('[data-testid="week-start"]')
        .textContent()
      ...
      const nextStartDate = await page
        .locator('[data-testid="week-start"]')
        .textContent()
  ...
  test.describe("Overtime", () => {
    test("should create overtime request", async () => {
      ...
      await page
        .locator('[data-testid="ot-reason"]')
        .fill("Hoàn thành dự án ABC")
  ...
  test.describe("Reports", () => {
    test("@critical should generate attendance summary report", async () => {
      ...
      await expect(
        page.locator('[data-testid="total-working-days"]')
      ).toBeVisible()
    ...
    test("should generate late arrival report", async () => {
      ...
      await expect(
        page.locator('[data-testid="late-report-table"]')
      ).toBeVisible()
    })
    ...
    test("should generate overtime report", async () => {
      ...
      await expect(
        page.locator('[data-testid="ot-by-department"]')
      ).toBeVisible()
    })
  ...
  test.describe("Corrections", () => {
    test("should submit attendance correction request", async () => {
      ...
      await page
        ...
        .fill("Quên checkout do họp kéo dài")
    ...
    test("should list pending corrections for approval", async () => {
      ...
      await expect(
        page.locator('[data-testid="corrections-list"]')
      ).toBeVisible()
    })
    ...
    test("should approve correction with time verification", async () => {
      ...
      await expect(
        page.locator('[data-testid="correction-details"]')
      ).toBeVisible()
  ...
  test.describe("Device Integration", () => {
    ...
    test("should add new fingerprint device", async () => {
      ...
      await page
        .locator('[data-testid="device-name"]')
        .fill("Máy chấm công Tầng 1")
      await page
        .locator('[data-testid="device-type"]')
        .selectOption("FINGERPRINT")
