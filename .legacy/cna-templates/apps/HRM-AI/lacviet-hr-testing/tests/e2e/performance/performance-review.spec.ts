...
test.describe("Performance Management", () => {
  ...
  test.describe("Goals & OKRs", () => {
    ...
    test("should create new goal", async ({ page }) => {
      ...
      await page
        ...
        .fill("Improve code review efficiency")
      await page
        .locator('[data-testid="field-description"]')
        .fill("Reduce average code review time by 30%")
      await page
        .locator('[data-testid="field-category"]')
      ...
      await page
        .locator('[data-testid="kr-title-0"]')
        .fill("Average review time under 2 hours")
      await page.locator('[data-testid="kr-target-0"]').fill("2")
      await page.locator('[data-testid="kr-unit-0"]').fill("hours")

      await page.locator('[data-testid="add-key-result"]').click()
      await page
        .locator('[data-testid="kr-title-1"]')
        .fill("Review 20 PRs per week")
    ...
    test("should update goal progress", async ({ page }) => {
      ...
      await page
        .locator('[data-testid="progress-notes"]')
        .fill("Completed initial phase")
    ...
    test("should align goal with company objectives", async ({ page }) => {
      ...
      await page
        .locator('[data-testid="objective-list"]')
        .locator("li")
        .first()
        .click()
  ...
  test.describe("Performance Reviews", () => {
    test.describe("As Manager", () => {
      ...
      test("should display team reviews", async ({ page }) => {
        ...
        await expect(
          page.locator('[data-testid="team-reviews-table"]')
        ).toBeVisible()
      })
      ...
      test("should create review cycle", async ({ page }) => {
        ...
        await page
          .locator('[data-testid="field-start-date"]')
          .fill("2025-03-15")
        ...
        await expect(
          page.locator('[data-testid="toast-message"]')
        ).toContainText("Tạo chu kỳ đánh giá thành công")
      })
      ...
      test("should submit manager review", async ({ page }) => {
        ...
        const pendingReview = page
          .locator('[data-testid="review-status"]:has-text("Chờ đánh giá")')
          .first()

        if (await pendingReview.isVisible()) {
          await pendingReview
            .locator("..")
            .locator('[data-testid="start-review"]')
            .click()

          // Fill review form
          await page
            .locator('[data-testid="rating-performance"]')
            .locator("button")
            .nth(3)
            .click()
          await page
            .locator('[data-testid="rating-skills"]')
            .locator("button")
            .nth(4)
            .click()
          await page
            .locator('[data-testid="rating-teamwork"]')
            .locator("button")
            .nth(3)
            .click()
          await page
            .locator('[data-testid="rating-leadership"]')
            .locator("button")
            .nth(3)
            .click()

          await page
            .locator('[data-testid="achievements"]')
            .fill("Successfully led project X, improved team velocity")
          await page
            .locator('[data-testid="improvements"]')
            .fill("Could improve documentation practices")
          await page
            .locator('[data-testid="goals-next-period"]')
            .fill("Lead 2 major projects, mentor junior developers")
          await page
            .locator('[data-testid="overall-rating"]')
            .selectOption("EXCEEDS_EXPECTATIONS")

          await page.locator('[data-testid="submit-review"]').click()

          await expect(
            page.locator('[data-testid="toast-message"]')
          ).toContainText("Đã gửi đánh giá")
        }
      })
      ...
      test("should calibrate team ratings", async ({ page }) => {
        ...
        await expect(
          page.locator('[data-testid="calibration-grid"]')
        ).toBeVisible()
    ...
    test.describe("As Employee", () => {
      ...
      test("should submit self-review", async ({ page }) => {
        ...
        const pendingReview = page
          .locator('[data-testid="self-review-pending"]')
          .first()
        ...
        if (await pendingReview.isVisible()) {
          ...
          await page
            .locator('[data-testid="self-achievements"]')
            .fill("Completed 15 projects, learned new technologies")
          await page
            .locator('[data-testid="self-challenges"]')
            .fill("Time management during multiple projects")
          await page
            .locator('[data-testid="self-improvements"]')
            .fill("Better documentation, more testing")
          await page
            .locator('[data-testid="self-goals"]')
            .fill("Get certified in AWS, lead a team")
          await page
            .locator('[data-testid="self-rating"]')
            .selectOption("MEETS_EXPECTATIONS")

          await page.locator('[data-testid="submit-self-review"]').click()

          await expect(
            page.locator('[data-testid="toast-message"]')
          ).toContainText("Đã gửi tự đánh giá")
        }
      })
      ...
      test("should view my review history", async ({ page }) => {
        ...
        await expect(
          page.locator('[data-testid="review-history-list"]')
        ).toBeVisible()
      })
    })
  })
  ...
  test.describe("360 Feedback", () => {
    ...
    test("should submit peer feedback", async ({ page }) => {
      ...
      const pendingFeedback = page
        .locator('[data-testid="feedback-request"]')
        .first()
      ...
      if (await pendingFeedback.isVisible()) {
        ...
        await page
          .locator('[data-testid="strengths"]')
          .fill("Excellent problem solving, great mentor")
        await page
          .locator('[data-testid="improvements"]')
          .fill("Could communicate more proactively")
        await page
          .locator('[data-testid="additional-comments"]')
          .fill("Great team player, always willing to help")

        await page.locator('[data-testid="submit-feedback"]').click()

        await expect(
          page.locator('[data-testid="toast-message"]')
        ).toContainText("Cảm ơn bạn đã gửi feedback")
      }
    })
    ...
    test("should view 360 feedback report", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="feedback-comments"]')
      ).toBeVisible()
    })
  })
  ...
  test.describe("Competency Assessment", () => {
    test.beforeEach(async ({ page }) => {
      ...
      await loginPage.goto()
      await loginPage.login(
        testUsers.hrManager.email,
        testUsers.hrManager.password
      )
      await loginPage.expectLoginSuccess()
    })
    ...
    test("should manage competency framework", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="competency-tree"]')
      ).toBeVisible()
    })
    ...
    test("should create competency", async ({ page }) => {
      ...
      await page
        .locator('[data-testid="field-description"]')
        .fill("Ability to lead technical decisions")

      // Add proficiency levels
      await page
        .locator('[data-testid="level-1-desc"]')
        .fill("Basic understanding")
      await page
        .locator('[data-testid="level-2-desc"]')
        .fill("Can apply with guidance")
      await page
        .locator('[data-testid="level-3-desc"]')
        .fill("Works independently")
      await page
        .locator('[data-testid="level-4-desc"]')
        .fill("Can mentor others")
      await page
        .locator('[data-testid="level-5-desc"]')
        .fill("Expert, sets standards")

      await page.locator('[data-testid="save-competency"]').click()

      await expect(page.locator('[data-testid="toast-message"]')).toContainText(
        "Tạo năng lực thành công"
      )
    })
  ...
  test.describe("Performance Reports", () => {
    ...
    test("should display performance dashboard", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="review-completion-rate"]')
      ).toBeVisible()
      await expect(
    ...
    test("should export performance report", async ({ page }) => {
      ...
      await page
        ...
        .selectOption("PERFORMANCE_SUMMARY")
