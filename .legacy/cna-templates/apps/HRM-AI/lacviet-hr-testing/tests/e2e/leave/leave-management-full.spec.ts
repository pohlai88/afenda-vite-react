...
test.describe("Leave Management", () => {
  ...
  test.describe("Leave Requests", () => {
    ...
    test("should cancel pending leave request", async () => {
      ...
      const requestRow = leavePage.page
        .locator('[data-testid="leave-table"] tbody tr')
        .first()
    ...
    test("should upload supporting document", async ({ page }) => {
      ...
      await leavePage.attachmentUpload.setInputFiles(
        "./tests/fixtures/medical-cert.pdf"
      )
    ...
    test("should view leave history", async ({ page }) => {
      ...
      const approvedRequests = page.locator(
        '[data-testid="leave-table"] tbody tr'
      )
  ...
  test.describe("Leave Approvals", () => {
    ...
    test("should display pending approvals", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="pending-approvals-table"]')
      ).toBeVisible()
    })
    ...
    test("should reject leave request with reason", async ({ page }) => {
      ...
      if (await pendingRequest.isVisible()) {
        ...
        if (requestId) {
          await leavePage.rejectLeave(
            requestId,
            "Dự án đang trong giai đoạn quan trọng, vui lòng chọn thời điểm khác."
          )
    ...
    test("should bulk approve multiple requests", async ({ page }) => {
      ...
      if (count >= 2) {
        ...
        await leavePage.expectToastMessage(
          "Phê duyệt thành công 2 đơn",
          "success"
        )
      }
    })
  ...
  test.describe("Leave Policies", () => {
    ...
    test("should display leave types", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="leave-types-table"]')
      ).toBeVisible()
    })
    ...
    test("should set leave blackout dates", async ({ page }) => {
      ...
      await page
        .locator('[data-testid="blackout-reason"]')
        .fill("Year-end closing")

      await page.locator('[data-testid="save-blackout"]').click()

      await expect(page.locator('[data-testid="toast-message"]')).toContainText(
        "Thêm ngày nghỉ bị chặn"
      )
    })
  })
  ...
  test.describe("Leave Reports", () => {
    ...
    test("should display leave dashboard", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="total-leaves-taken"]')
      ).toBeVisible()
      await expect(
        page.locator('[data-testid="pending-requests"]')
      ).toBeVisible()
      await expect(
        page.locator('[data-testid="leave-by-type-chart"]')
      ).toBeVisible()
    })
    ...
    test("should generate leave utilization report", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="utilization-chart"]')
      ).toBeVisible()
    })
  })
})
