    ...
    for (const line of data.deliveredLines) {
      await prisma.deliveryLine.update({
        ...
        data: {
          ...
          status:
            line.deliveredQty >=
            order.lines.find((l) => l.id === line.lineId)!.quantity
              ? "DELIVERED"
              : line.deliveredQty > 0
                ? "PARTIAL"
                : "PENDING",
    ...
    <div className="space-y-6">
      ...
      {view === "list" ? (
        ...
        <DeliveryCalendar
          ...
          year={selectedYear}
          onDateClick={(date) =>
            navigate(`/operations/delivery/new?date=${date}`)
          }
        />
      )}
    </div>
...
async function importSellData(data: ImportRequest, userId: string) {
  ...
  for (const row of data.data) {
    try {
      ...
      results.errors.push(
        `Row ${row.customerId}-${row.productId}: ${error.message}`
      )
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          variant={
            comparison?.analysis.avgSellThroughRate >= 70
              ? "success"
              : "warning"
          }
          icon={<Activity />}
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <Button
            variant="outline"
            onClick={() => navigate("/operations/inventory/snapshots")}
          >
      ...
      {alerts?.data.filter((a) => a.severity === "CRITICAL").length > 0 && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            {alerts.data.filter((a) => a.severity === "CRITICAL").length}{" "}
