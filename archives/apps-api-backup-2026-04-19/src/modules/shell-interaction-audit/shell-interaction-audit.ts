/**
 * POST `/v1/audit/shell-interaction` — shell UI interaction evidence (governed audit, not domain events).
 *
 * Parses the client payload, maps phases to audit outcomes, and writes via `insertGovernedAuditLog`.
 * Requires session + tenant; uses `c.var.audit` from platform audit middleware.
 *
 * @module modules/shell-interaction-audit/shell-interaction-audit
 */
import {
