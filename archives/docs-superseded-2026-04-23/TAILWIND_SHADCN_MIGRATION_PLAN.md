...
relatedDomain: design-system-migration
---

# Tailwind + shadcn Migration Plan

Historical execution record for the Tailwind CSS v4, shadcn/ui, and ERP app-shell adoption in `apps/web`.

This archived document was removed from the active `docs/` surface during docs-policy Wave C.5 because the migration is complete and the governing truths now live in canonical docs.
