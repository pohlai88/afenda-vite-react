---
name: hono
description: Use when building Hono web applications or when the user asks about Hono APIs, routing, middleware, JSX, validation, testing, or streaming. TRIGGER when code imports from 'hono' or 'hono/*', or user mentions Hono. In this monorepo, `@afenda/api` ships `@hono/cli`; use `pnpm --filter @afenda/api exec hono …`. If the `hono-docs` MCP server is configured in `.cursor/mcp.json`, prefer its tools for the latest docs over this inline reference.
---

# Hono Skill

Upstream source: [yusukebe/hono-skill](https://github.com/yusukebe/hono-skill) (copied into `.agents/skills/hono/`; sync periodically). Build Hono web applications with inline API knowledge for AI. Use `pnpm --filter @afenda/api exec hono request` (or `npx hono request`) to hit routes without starting a server, when your entry file matches the CLI’s export expectations. If the `hono-docs` MCP server is configured, prefer its tools for the latest documentation over the inline reference below.

## Afenda `apps/api` layout (anti-dumping)

Keep the Node API from turning into a flat junk drawer. Align with Hono’s [larger-app guidance](https://hono.dev/docs/guides/best-practices): modular routes via `app.route()` / mount functions, **not** Rails-style controller classes.

**Allowed at `apps/api/src/` root**

- `index.ts` — process entry; `serve()` from `@hono/node-server`.
- `app.ts` — `createApp()` factory only: global middleware and **`registerRoutes(app)`**.

**Structured elsewhere**

- `routes/` — Sub-apps (`new Hono()`, then `app.route("/", subApp)` in `routes/index.ts`): e.g. `health.ts`, `auth.ts`.
- `middleware/` — Shared middleware (`request-id`, `error-handler`, etc.); use **`hono/logger`** (or Pino later) from `app.ts` as needed.
- `lib/` — `env.ts`, `errors.ts`, `response.ts` (no business logic).
- `modules/<feature>/` — Vertical slices as you add them: services, Zod `contracts/`, repos, tests. Older trees also used `platform/` and `*-adapters/` — valid if you reintroduce them.

**Forbidden**

- New miscellaneous `.ts` files beside `app.ts`.
- Hand-written public DTO interfaces that duplicate Zod contracts — contracts folder is canonical.

**Tests** — Prefer `app.request()` in Vitest (see “Testing with app.request()” below); that matches how `createApp` is already tested.
