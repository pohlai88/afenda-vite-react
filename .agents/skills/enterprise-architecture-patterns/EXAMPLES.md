...
class Order {
  ...
  addItem(product: ProductDetails, price: Money, quantity: number): void {
    ...
    if (existingItem) {
      ...
      this.items.set(product.productId, new OrderItem(product, price, quantity));
    }

    this.addEvent(new OrderItemAddedEvent(this.orderId, product.productId, quantity, price));
  }
  ...
  confirm(): void {
    ...
    this.addEvent(new OrderConfirmedEvent(this.orderId, this.calculateTotal(), new Date()));
  }
  ...
  cancel(reason: string): void {
    ...
    this.addEvent(new OrderCancelledEvent(this.orderId, reason, new Date()));
  }
  ...
  ship(trackingNumber: string): void {
    ...
    this.addEvent(new OrderShippedEvent(this.orderId, trackingNumber, new Date()));
  }
...
class OrderCommandHandler {
  ...
  async handleCreateOrder(command: CreateOrderCommand): Promise<void> {
    const order = new Order(command.orderId, command.customerId, command.shippingAddress);
...
class OrderQueryService {
  ...
  async getCustomerOrders(customerId: string, status?: string): Promise<OrderSummaryReadModel[]> {
  ...
  async getRecentOrders(limit: number = 10): Promise<OrderSummaryReadModel[]> {
    return await this.db.orderSummaries.find({}).sort({ createdAt: -1 }).limit(limit);
  }
}
```
...
class BankAccount {
  ...
  open(initialDeposit: Money): void {
    ...
    this.applyEvent(
      new AccountOpenedEvent(this.accountNumber, this.accountHolder, initialDeposit, new Date())
    );
  }
  ...
  deposit(amount: Money, description: string): void {
    ...
    this.applyEvent(new MoneyDepositedEvent(this.accountNumber, amount, description, new Date()));
  }
  ...
  withdraw(amount: Money, description: string): void {
    ...
    this.applyEvent(new MoneyWithdrawnEvent(this.accountNumber, amount, description, new Date()));
  }
  ...
  close(): void {
    ...
    this.applyEvent(new AccountClosedEvent(this.accountNumber, new Date()));
  }

  freeze(reason: string): void {
    this.ensureAccountActive();

    this.applyEvent(new AccountFrozenEvent(this.accountNumber, reason, new Date()));
  }
  ...
  unfreeze(): void {
    ...
    this.applyEvent(new AccountUnfrozenEvent(this.accountNumber, new Date()));
  }
  ...
  private apply(event: DomainEvent): void {
    if (event instanceof AccountOpenedEvent) {
      ...
      this.balance = new Money(this.balance.amount - event.amount.amount, this.balance.currency);
  ...
  static fromEvents(events: DomainEvent[]): BankAccount {
    ...
    const account = new BankAccount(firstEvent.accountNumber, firstEvent.accountHolder);
...
class TransactionQueryService {
  ...
  async getTransactionHistory(
    ...
    return await this.db.transactions.find(filter).sort({ timestamp: -1 });
  }
  ...
  async getAccountStatement(
    ...
    const transactions = await this.getTransactionHistory(accountNumber, startDate, endDate);
    ...
    const closingBalance = transactions.length > 0 ? transactions[0].balanceAfter : openingBalance;
...
class ResourceQuotaService {
  ...
  async checkQuota(tenantId: string, resourceType: ResourceType): Promise<boolean> {
  ...
  async recordUsage(tenantId: string, resourceType: ResourceType, amount: number): Promise<void> {
...
class PaymentSaga {
  ...
  private async sendConfirmation(): Promise<void> {
    await notificationService.sendPaymentConfirmation(this.customerId, this.orderId, this.amount);
  }

  private async publishSuccess(): Promise<void> {
    await eventBus.publish(
      new PaymentCompletedEvent(this.paymentId, this.orderId, this.amount, new Date())
    );
  }

  private async publishFailure(error: any): Promise<void> {
    await eventBus.publish(
      new PaymentFailedEvent(this.paymentId, this.orderId, error.message, new Date())
    );
  }

  private async logCompensationFailure(stepName: string, error: any): Promise<void> {
...
class EventStreamProcessor {
  ...
  private async handlePageView(event: any): Promise<void> {
    ...
    await this.aggregationStore.increment(`pageviews:${minuteKey}`, 1);

    await this.aggregationStore.increment(`pageviews:${event.page}:${minuteKey}`, 1);
  ...
  private async handlePurchase(event: any): Promise<void> {
    ...
    await this.aggregationStore.increment(`revenue:${minuteKey}`, event.amount);

    await this.aggregationStore.increment(`orders:${minuteKey}`, 1);
...
class TimeSeriesAggregator {
  async aggregateHourly(metric: string, date: Date): Promise<HourlyAggregation[]> {
    ...
    for (let hour = 0; hour < 24; hour++) {
      ...
      const value = await this.aggregationStore.sum(metric, hourStart, hourEnd);
  ...
  async aggregateDaily(
    ...
    while (currentDate <= endDate) {
      ...
      const value = await this.aggregationStore.sum(metric, dayStart, dayEnd);
...
_[Continue with 16 more examples covering the remaining topics...]_
