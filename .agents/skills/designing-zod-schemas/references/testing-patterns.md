...
// Usage in tests
const category = new CategoryTestBuilder().withName("Electronics").withSlug("electronics").build();
```
...
```typescript
const validateInput = <T>(schema: z.ZodSchema<T>, data: unknown, context: string): T => {
  ...
  if (!result.success) {
    ...
    throw new ValidationError(`Validation failed for ${context}`, errorDetails);
  }
