...
const UniqueSlugArraySchema = z
  ...
  .refine((items) => new Set(items).size === items.length, "Duplicate slugs found");
```
...
const NormalizedNameSchema = z.string().transform((val) => val.trim().toLowerCase());

// Generate slug from name
const AutoSlugSchema = z.string().transform((name) =>
  name
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "")
...
// Parse JSON string
const JsonObjectSchema = z.string().transform((val, ctx) => {
  try {
...
const ProductSchema = z
  ...
  .refine((data) => data.isDigital || data.weight !== undefined, {
