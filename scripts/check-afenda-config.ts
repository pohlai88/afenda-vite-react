...
async function assertRootTopologyGovernance(config: Awaited<ReturnType<typeof loadAfendaConfig>>) {
...
async function assertFeatureTemplateGovernance(
  ...
  const featuresRootPath = path.join(workspaceRoot, featureTemplate.featuresRoot)
  const featuresRootStats = await fs.stat(featuresRootPath).catch(() => undefined)
  ...
  if (!featureTemplate.enforceWhenFeatureExists || featureDirectories.length === 0) {
    return
  }
...
async function assertSharedPackageTemplateGovernance(
  ...
  const sharedPackagePath = path.join(workspaceRoot, sharedPackageTemplate.packagePath)
...
async function assertWebClientSrcGovernance(
  ...
  const expected = [...webClientSrc.allowedTopLevelDirectories].sort((left, right) =>
